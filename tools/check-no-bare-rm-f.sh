...
SKILL_DIRS=(meeting git-diary-workflow todo-update meeting-cross relay projects decision-brief hooks statusline tools tracker)
...
for f in "${files[@]:-}"; do
  ...
  for ((i = 0; i < n; i++)); do
    ...
    if grep -qE "$VIOLATION_RE" < <(printf '%s' "$line") ; then

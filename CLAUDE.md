...
make install-<skill>    # one skill (meeting, meeting-cross, git-diary-workflow, todo-update, relay, projects, decision-brief)
...
|---|---|---|
| `relay/references/executor-contract.md` | `contract vN` HTML-comment marker (currently v18) | `/relay executor` + the `## Relay contract` pointer must agree on `vN`; bump discipline documented in-file |
...
| `inflownistration/` | The `/inflownistration` skill (id:32c3; skeleton seam id:68c1 = SKILL.md alone, modes (b)/(c) add their scripts under id:d846/id:a187). **This skill is PUBLIC and the concept's home repo is PRIVATE**, which is the constraint that shapes the whole file: it carries POINTERS and PROCEDURE, never the concept's content, never an instance name, never a populated table schema. Mode (a) therefore points at the four topical docs (`docs/{provenance,grammar,instances,prior-art}.md`) instead of summarizing them -- a restatement living in a second repo is exactly the drift shape mode (b) exists to detect, so the surface mode must not mint new instances of it, and the absent-repo branch is a STATED non-fatal degradation that must never reconstruct the missing content from memory. The parameter-less router is audit-then-RECOMMEND, never act, and it never chains into the mode it offers. No `/infln` alias, deliberately |
| `decision-brief/` | SKILL.md + `docket.sh` (id:6fda) — turns open OWNER-lane ledger decisions into a brief + recommendation + ONE batched `AskUserQuestion`. It DELEGATES enumeration/bucketing to `relay/scripts/gather-human-backlog.sh` rather than minting a third copy of that contract, so it requires `relay` installed alongside it; what it adds on top is an ANCHORED per-candidate lane re-resolution, load-bearing because that collector matches lane tags with an UNANCHORED awk `match()` and does not know `[ROUTINE]`. **Recommend-never-decide is MECHANICAL, not prose:** the single write path only ever DRAFTS `@owner-answered` through `md-merge.py`'s flock, exits 4 without `--owner-confirmed`, and exits 5 in any unattended context (`RELAY_AFK`/`RELAY_RUN_ID`/`RELAY_POOL`/`DECISION_BRIEF_UNATTENDED`) EVEN WITH the flag — the unattended refusal is checked FIRST and deliberately outranks the confirmation, because a confirmation raised where no owner exists can only have been set by an agent. `calibrate()` runs ten both-polarity controls before any row or count is emitted and exits 3 emitting nothing countable if any fails. Stated design limit: `--owner-confirmed` is agent-settable in an ATTENDED session, so forgery is REDUCED to a deliberate act, not eliminated |
| `relay/` | The reviewer/executor relay skill itself (this contract comes from there). `references/executor-contract.md` is the versioned executor contract loaded by `/relay executor`; the `## Relay contract` pointer below must match its `vN` marker. `scripts/lib-private-remote.sh` is THE single "is this remote a PRIVATE/LAN host?" predicate — sourced by BOTH `hooks/pre-push-privacy-gate.sh` and `scripts/integrate.sh`'s per-remote push narrowing (id:4d44); it reads `private-host:` directives from the PRIVATE, never-committed pattern file at runtime, and `git-lock-push.sh`'s `is_ssh_url()` is NOT a substitute (it is an SSH-AUTH predicate and fails toward auto-publish). `scripts/lib-clean-tree.sh` is THE single "is this working tree really dirty?" predicate (`tree_clean_probe`, created by id:68e2 out of the id:3016 fix) — sourced by `worktree-retire.sh`, `verify-isolation.sh` (BOTH its main path and its id:1b13 breach branch, id:0fad) and `gather-repo-state.sh` (id:fac7). **A bare `git status --porcelain` is NOT a substitute and minting a fifth copy is the recurring defect** — the class has now produced four separate false-dirty incidents, because a git-annex pointer file reads ` M` in porcelain while `git diff` is empty, so cosmetic dirt blocks dispatch, reads as a residue, or reads as an isolation BREACH. Untracked files MUST stay dirty (`git diff` never sees them), and since the owner's 2026-09-11 rc=2 ratification a probe where git ITSELF failed reads DIRTY, not clean. `scripts/self-transcript.sh` is THE single "where is the CALLING agent's own transcript?" resolver (id:ff30) — `$CLAUDE_SESSION_ID` names the top-level session, which is the *directory* holding a child's `subagents/agent-<id>.jsonl`; siblings are disambiguated by a `--marker` string from the child's own dispatch prompt. `context-budget.sh --self` is its only consumer today and is what makes executor-contract rule 2c runnable at all. `scripts/heartbeat.sh` is the run-liveness marker (id:e149) the outage watchdog (id:98f0) + auto-reconcile-on-restart (`relay-reconcile.sh --auto`, id:7809) both read. `scripts/review-box-tick.py` is THE `/relay human` 3(a) apply path (id:7c75) — it is what makes that step performable at all, since `md-merge.py` addresses a line only by an anchored `<!-- id:XXXX -->` marker or by a `## ` heading whose unit is the WHOLE section, and the common REVIEW_ME shape is one coarse heading over many boxes. It extracts the section verbatim, flips EXACTLY ONE checkbox, emits the caller's rationale as its own paragraph at the END of that box, and hands the section back to `md-merge.py update-sections` under its flock; it never writes a ledger itself (no md-merge, no write). **The rationale is NEVER appended to the checkbox line, and that placement is the whole of the 2026-09-10 defect fix — a REVIEW_ME head line is a PHYSICAL line, not a semantically complete unit.** Measured that day, 188 of 431 open boxes fleet-wide (44%) have a title that WRAPS: this repo's own `REVIEW_ME.md:1286` opens `- [ ] **id:6446 was worked ... carried TWO independent` and continues `first-class dispatch exclusions ...` on the next line, with the `**` closing two lines later, so the first version spliced its rationale mid-sentence and left it inside an unterminated bold run. That is the id:2964 class one layer up (there a regex crossed a marker boundary; here a physical line was read as a logical one). Appending at the end of the LOGICAL title only moves the bug, because the title also ends mid-line; end-of-box is UNIFORM for wrapped and unwrapped boxes alike, so no branch can pick wrong, and it leaves the checkbox line byte-identical apart from its checkbox character so an anchored marker stays line-final. Refusals are loud and code-distinguished: 0 matches (2) and 2+ matches (3) get DIFFERENT messages because the remedies are opposite (the id:6d7e ruling), no `## ` heading above the box is 4, a heading that REPEATS in the file is 5 — md-merge keys on heading TEXT and rewrites every match, so taking the first would duplicate one section into all of them — and an inline-markup run still OPEN where the paragraph would land is 9 (measured: fires on 3 of 431 boxes, all genuinely malformed today). `--dry-run` is inert BY CONSTRUCTION, not by a re-checked flag (the id:f563 defect shape): one fork picks the write target and the dry side of it picks a copy in a private temp dir, so the real ledger path is unreachable from the dry branch, and the printed diff is byte-exact because the real md-merge produced it |
...
| `tools/` | `allowlist.py` (settings.json allowlist generator) + `allow-extra.txt`; `ctx-budget.sh` (advisory SKILL.md token-budget audit); `settings-env.py` (settings.json env-block applier, used by `make install-relay-env`); `model-probe.sh` + `model-probe.battery.jsonl` (standing model-quality probe, id:dba3); `quota-sample.sh` + `quota-sample.{service,timer}` + `quota-report.py` (idle-resilient usage-quota sampler → git-versioned JSONL in `~/src/claude-diary/quota/`, `make install-quota-timer`, id:d267); `relay-watchdog.sh` + `relay-watchdog.{service,timer}` (outage watchdog — notifies when a local relay loop died without a clean stop, via the shared run-heartbeat; NO `claude -p`; `make install-relay-watchdog`, id:98f0); `memory-index.py` (GENERATES the auto-memory `MEMORY.md`/`MEMORY.archive.md` index from per-file frontmatter — `--check` exits non-zero on drift, `--write` regenerates atomically; the derived-index/SSOT source-of-truth for the `hooks/memory-index-sync.py` PostToolUse hook, id:2e6d); `privacy-audit.sh` (TREE-scoped privacy audit over `git ls-files`, id:9bfc part (a) — the companion to the DIFF-scoped `hooks/pre-push-privacy-gate.sh`, which structurally cannot see already-published exposure. Prints pattern INDICES, never the pattern: the gate's `pattern<TOKEN>` output spells out the PRIVATE pattern, and pasting it into a tracked file in this PUBLIC repo publishes it. Resolved report goes to `~/.claude/logs/privacy-audit.log`, and the script REFUSES to write that log anywhere git could commit it); `ledger-shrink.py` (the id:0d7c line-shrink GENERATOR -- relocates an item's body into `docs/ledger-notes/<id>.md` and plants the `-- detail:` pointer; carries a structural keep-token catch-all that is open-ended over marker NAMES but ANCHORED to a marker SHAPE (id:2964 -- it was spelled `<!--[^>]*-->`, which ran across PROSE from a backticked `<!--` to the next `>` and spliced body text onto two head lines; do NOT re-spell it as an enumeration of today's marker names, which is the id:d35a regression the catch-all exists to prevent), the `lane-would-change` invariance refusal and the id:a580 `bold-run-to-anchor` refusal, and emits a relocated lane in the LEADING run so `primary_lane()` stays anchored); `ledger-continuations.py` (the CONTINUATION-line mover, and the tool that decides whether a body may move at all: `cited_by` COMPUTES whether a live consumer greps the body text, and the id:1447 construct-to-corpus dataflow pass narrows that per READ SITE rather than per file. **There is no `REFUSE_IDS` constant and there must never be one again** -- `selftest_predicate()` runs before every scan and the tool exits 4 if either half of the predicate stops discriminating, so a refactor cannot silently re-vacuum it. `scan()` runs in TWO passes and that shape is LOAD-BEARING, not stylistic (id:0176): pass 1 does the structural split (no-id/foreign-id/unowned) and collects the surviving candidates, pass 2 runs `cited_by` once per candidate against ONE shared `batch_rest` -- the document with EVERY candidate's body removed and everything else, head lines included, left standing. A single `scan()` call relocates the whole batch, so the surviving-text escape must be scored against the state the batch actually leaves behind; the per-block `lines[:i+1] + lines[j:]` it replaced modelled "this block gone, every OTHER candidate still in place", under which two blocks citing only each other's body both read as safe and `tracker/ledger-map.py:493` -- the one genuine continuation-body consumer on this tree, and the consumer whose existence justified the id:1447 untraced amendment -- was silenced. Do NOT collapse the two passes back into one loop) |
...
- **The dispatch prompt spells a child's worktree with a LITERAL TILDE, and the child's
  `$(pwd)` is absolute -- so a substring self-marker never matches.** `worktreePathFor()`
  (`relay/scripts/relay-loop.js:2957`) builds the path as the template literal
  `` `~/.cache/relay/worktrees/${repo}/${runId}-${key}` ``; nothing expands it, so the

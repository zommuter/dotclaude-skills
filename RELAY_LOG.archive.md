# Relay log archive <!-- merge=union; append-only — never edit or reorder past entries -->
...
## 2026-07-20 20:07 — reviewer (claude-opus-4-8)

Review of relay-ckpt-20260720-1621..HEAD (commits 3473131 meeting + d8f9354 TODO-archive) — DOCS/LEDGER-ONLY window: .gitignore (+persona-events/), TODO.md (3 new items + archive prune), TODO.archive.md, one meeting note. NO executor/code commits, so §2b gaming-residue checks (resurrection, fixture-special-casing, faked-clean-tree, refactor-claim) are N/A. Mechanical passes CLEAN: gaming-scan.sh exit 0 (no DELETED_TEST/ADDED_SKIP/REMOVED_ASSERT), roadmap-lint.sh exit 0, relay-doctor 0 per-repo issues (2 report-only cross-repo inbox dead-letters routed:6025→zelegator, routed:4975→chidiai, pre-existing, unrelated to window). §5b: the 3 new open items id:0ee1(routed:4361 lease-scope-fix), id:65f5(routed:1c08a classifier-not-executor-ready), id:8089(routed:1c08b user-visible-close+bump-gate) are all [HARD — meeting] owner-ratified design-ledger items from today's meeting — qualified design-judgment work, correctly LEFT in TODO awaiting a separate RED-spec handoff; NOT forced into ROADMAP, NOT re-litigated. No ROADMAP re-derivation needed (window touched no ROADMAP). PRE-EXISTING finding surfaced to owner (OUTSIDE this window, present at the baseline tag, NOT my regression): full make test = 282 passed / 2 failed — (1) test_git_lock_push_slash_branch.sh is an order-dependent flake (PASSES in isolation on main and at baseline); (2) test_drain_driver_stop.sh (# roadmap:cd7a, item ticked [x] DONE) fails reason=quota-stop rounds=0 because it lacks the DRAIN_QUOTA_CMD stub that id:5eb8 added to the heartbeat/quota specs — the id:838d quota-gate now fires the real quota-stop.sh before round 1. Recommend a tiny follow-up: stub DRAIN_QUOTA_CMD in test_drain_driver_stop.sh, mirroring test_drain_driver_heartbeat.sh:45-52. Checkpoint is bookkeeping-only; no ledger edits made.
...
## 2026-07-21 16:25 — executor (sonnet)

Worked id:7681 — built the shared arg-guard `relay/scripts/validate-flags.sh` (runtime warn-and-drop with arity-aware value consumption and near-miss escalation via a plain-bash Levenshtein check against `--afk`/`--cross`/`--fabled`/`-d`) plus `--coverage` drift-check mode, and two manifests (`relay/scripts/known-flags-relay.tsv`, `known-flags-meeting.tsv`). Wired it into `meeting/SKILL.md` (new step 1b) and `relay/SKILL.md` (new step 0pre, before 0a) as the required displayed warning artifact. `--coverage` grep-scoping (REVIEW_ME id:7681, left OPEN for reviewer confirmation): for /relay, scans the `Invocation:` code fence excluding the `inject`/`stop` lines (their bracketed tokens are subcommand-owned args — `--item`/`--verdict`/`--prompt`/`--now` — not top-level guard flags) plus the `## Configuration knobs` table's FIRST column only (excludes the Effect/description prose column, which mentions unrelated flags in passing text, e.g. `--only`'s effect text saying "the old `--exclude`-everything-else workaround"); for /meeting, scans only lines matching `grep -i "skill argument"` (today just the `--cross` line) since meeting/SKILL.md has no formal invocation fence and a naive whole-file grep pulls in dozens of helper-script prose flags (`--mode`, `--apply`, `--query`, …) exactly as REVIEW_ME warned. Verified both scopings empirically against the live SKILL.md files before finalizing the regex. `test_unknown_switch_guard.sh` (17/17) passes; full suite 292/0/0 (was 291/0/1-xred at handoff). Also registered `validate-flags.sh` + both `.tsv` manifests in the Makefile's `relay_FILES`/`relay_EXEC`/`relay_ALLOW` (caught by `test_relay_install_manifest.sh`, which failed once before the manifest files were registered — fixed in the same session). Ticked ROADMAP.md + TODO.md twins for id:7681 (single-id-two-views, flock'd md-merge). Did NOT tick the REVIEW_ME id:7681 box — left open for the next `/relay review` to confirm the coverage scoping per its own instruction.
Friction: none — the ROADMAP's pinned interface + the REVIEW_ME box's explicit recommendation made the scoping design mostly mechanical; the only real judgment call was excluding inject/stop's subcommand-owned flags from the relay fence scan, which I verified empirically rather than guessing.
refactor: none needed — first implementation of a new script, no pre-existing duplication to clean up; the manifest/script split (data vs logic) was the design from the start, not an afterthought extraction.


## 2026-07-21 20:49 — probe id:94b8 (claude-opus-4-8)

Ran the revised Workflow probe (script `probe-94b8`, runId wf_b65f71af-227) under `ANTHROPIC_BASE_URL`→`mechanical-proxy.py`. SPLIT result — decisive premise PASSES, `model:"bash"` mechanism does NOT work as-tested:

- **(b) base-URL transit — PASS (the decisive premise).** A NORMAL `agent()` returned `OK` end-to-end and its traffic APPEARS at the proxy: `/tmp/mechanical-proxy.log` is all `{"event":"passthrough", ..., "upstream_hit":true}` (200s). So a plain Workflow `agent()` DOES transit the global base URL — consolidation is NOT off; the off-Workflow line (id:65f9) is not forced by this premise.
- **(a) `model:"bash"` short-circuit — FAIL as-tested.** `agent(…, {model:"bash"})` errored ("issue with the selected model (bash) — may not exist / no access"), returned `null`. The proxy is a PURE passthrough: it forwarded `model:"bash"` unmodified to the upstream, which 404'd it (visible in the log as the two ~132 KB `404` lines + retry). The proxy did NOT intercept/short-circuit `"bash"`, and there is no real `bash` model upstream.

**Recommendation (evidence, NOT a verdict — consolidate-vs-coexist remains the owner's per the two same-day directives):** the `model:"bash"` dispatch handle is viable ONLY if the proxy is taught to INTERCEPT-and-run the sentinel model id locally (return the allowlisted hop's stdout, zero upstream) — bare passthrough will never short-circuit it. Wiring id:176f into the Workflow substrate + converting `echo-runner`/the ~12 Haiku hops therefore depends on building that proxy-side intercept first; it is not free from the current proxy. The 436c-vs-8ba1 consolidate/coexist call is unchanged by this probe and stays open for owner ratification.


## 2026-07-21 21:09 — proxy shape-robust intercept (claude-opus-4-8) id:94b8/id:176f

Diagnosed WHY (a) fell open (root cause, not "the proxy is pure passthrough"): `do_POST` (mechanical-proxy.py:300) intercepts only if `_command_allowed(last-user-message-text)` — i.e. the user message must BE a bare allowlisted relay command. `test_mechanical_proxy.sh` passed because it hand-fed exactly that synthetic shape (`req("bash", [{"role":"user","content": ok_cmd}])`). A real Workflow `agent()` WRAPS the task in subagent scaffolding, so the raw last-user text is prose → `_command_allowed` rejects it → fail-open → `model:"bash"` forwarded upstream → 404. The intercept was never exercised against a real `agent()` shape. Nothing regressed; the unit test had a coverage hole. (Silver lining: the harness DID carry `model:"bash"` to the wire — Fable's "does agent() accept an arbitrary model string" resolves to yes; the failure was purely extraction.)
...
## 2026-07-21 — executor (Opus apex) id:6176
...
## 2026-07-23 — review (claude-opus-4-8)

Reviewed relay-ckpt-20260723-1301..HEAD (5 commits: id:99a4 probe, id:69f6 service+manifest, integrate merge, claude-relay launcher). VERDICT: clean, no gaming.
- id:99a4 discriminator is REAL, not fixture-special-cased: `is_loopback_base_url` exact-match `http://127.0.0.1:<port>`, `port_alive` a genuine `/dev/tcp` connect; the RED test triangulates 4 cases on RANDOMIZED ports so special-casing is impossible. `tests/test_mech_proxy_probe.sh` re-run PASS; `gaming-scan.sh` clean.
- id:69f6 unit correct: `ExecStart` worktree-independent (`%h/src/dotclaude-skills/...` absolute, NOT a worktree path), `Restart=always`/`RestartSec=5`, loopback per the script's own posture. Installed unit is a symlink to the committed file and is `active`+`enabled`; proxy confirmed LISTEN 127.0.0.1:61843; live probe → `healthy`. Makefile install/status/uninstall-mech-proxy + install/status/uninstall-claude-relay mirror the relay-watchdog precedent and are idempotent (grep-guarded rc source, `ln -sf`).
...
## 2026-07-23 — strong-execute (claude-opus-4-8)

Worked id:bc49 — orphan-suppress is now ITEM-scoped/ADDITIVE, not REPO-scoped. `discover-repo.sh` step 1 no longer zeroes a repo's units the moment reconcile's `surfaced` is non-empty; it disposes surfaced BY CLASS. When every surfaced entry is an orphan-suppress notification (reason prefix `suppressed re-dispatch:`, reconcile-repo.sh:203/id:1f53) the repo STILL classifies and emits its execute unit ALONGSIDE the suppress surface (additive), with an A4-ii item-scoped `orphan-parked (id:X) — reconcile-first, do NOT work id:X` note injected into `unit.reason`; the SAME-ITEM carve-out (D1) drops the duplicate execute unit only when every open executable `[ROUTINE]` id is bound to a suppressed orphan (fail-open: never wrong-suppresses when it parses no routine ids). Any repo-level block (in-flight/claimed id:ebfb, diverged id:c3f7, e3ad fail-closed refusal, discover-error) stays SUBSTITUTIVE (units:[]) — the dc5b cross-run collision guard. Ambiguous-binding orphans (no id token, A3) are additive-surface. Twin surfaces in relay-loop.js updated to match: the :1163 CASE-A shard-prompt routing (orphan-suppress-only → delegate to the live discover-repo.sh which applies the additive routing mechanically; repo-level classes stay substitutive), the :1172 "each repo appears exactly once" invariant (a repo MAY now appear in both units and surfaced), and the surfaced schema comment (reused by SHARD_SCHEMA). RED spec `tests/test_orphan_additive_surface_bc49.sh` cases A-E all pass; full `make test` 299 pass / 0 fail / 4 expected-red (the other open items 1048/86a2/24ec/dfb9). node --check + lint-workflow-templates + both exec-smoke harnesses clean.
Friction: none — the classify unit carries no explicit selected-item id, so the same-item correlation is done by parsing open `[ROUTINE]` ids from ROADMAP.md and subtracting the suppressed-orphan ids (fail-open when unparseable).
...
## 2026-07-24 21:22 — executor (claude-opus-4-8, c5ed chain)

Executor for id:c5ed (install-drift guard, routed:35eb): implemented relay/scripts/check-install-drift.sh — detects direct drift (canonical script with no installed symlink) AND source-target drift (a script whose source/. target sibling is uninstalled — the routed:35eb false-green shape), exit 1 naming each missing basename, exit 0 on parity. RED spec green (4 assertions); wired into Makefile relay_FILES/EXEC/ALLOW; full suite 307/0/0 (independently re-verified). c5ed ticked in both ledgers.


## 2026-07-24 21:04 — review (claude-opus-4-8, c5ed audit)

Independent anti-gaming review of the id:c5ed install-drift guard chain (diff 3b4891f..HEAD). VERDICT: GENUINE. RED spec is real — moving `check-install-drift.sh` aside makes `test_install_drift_guard.sh` FAIL (exit 1), confirming it is not a no-op/tautology; restored, suite back to green. The guard is a real source-follower, NOT a blind canonical∖installed set-diff: assertion (d) (both dirs = {a.sh}, a.sh sources an uninstalled non-canonical helper.sh) passes only because the impl parses `source`/`.` lines — verified live. Adversarial fixtures beyond the shipped tests: tab-indented `source`, `source "$dir"/x.sh` split-quote form, and removing the REAL `lib-state-claim.sh` symlink from a mirror of relay/scripts (the exact routed:35eb target) are all caught with correct basenames; a `source` of non-`.sh` `/etc` paths is correctly ignored (no false-positive). Two theoretical edges do NOT manifest on the real corpus and are out of c5ed's stated scope, so NOT flagged (D3 anti-spam): (i) an absolute non-sibling `.sh` source would false-positive by basename; (ii) `source "$LIB_OWN_REPOS"` (full path in a variable) is a source-parser false-negative — but every such target is itself a canonical `relay/scripts/*.sh` caught by the DIRECT enumeration, so no real gap. gaming-scan.sh: clean. Allowlist: 8 entries for check-install-drift.sh. roadmap-lint: exit 0 (only pre-existing baselined WARNs id:6b35/id:659c, unrelated). Full suite: 307 passed, 0 failed, 0 expected-red. c5ed `[x]` tick JUSTIFIED in both ledgers; not reopened.


## 2026-07-24 21:30 — review (claude-opus-4-8, c5ed chain)

Review for id:c5ed (install-drift guard, routed:35eb): verdict GENUINE. RED spec confirmed genuinely failing without impl (not a tautology); guard verified to parse source/. lines and follow sibling targets (not a blind set-diff — assertion d); adversarial fixtures (tab-indent, split-quote, real lib-state-claim.sh removal, /etc non-sibling) show no false pos/neg on the real corpus; gaming-scan clean; suite 307/0/0; roadmap-lint clean. Tick justified, not reopened; no REVIEW_ME boxes.


## 2026-07-26 — executor (sonnet)

Worked id:78e1 — word-boundary-anchored `STATE_CLAIM_TERMINAL_RE` in `relay/scripts/lib-state-claim.sh`. Defect: a bare `\b`/`\<...\>` boundary still fires inside a hyphen-joined compound (e.g. "fail-CLOSED"), since a hyphen is a non-word character — this was the LIVE false positive on id:6b35's own ROADMAP prose ("fail-CLOSED is the key property"). Fix: introduced `STATE_CLAIM_TERMINAL_WORDS` (bare alternation, unchanged) for the existing "id:XXXX is <WORD>" scoped-strip context (where the preceding "is " already consumes the separator, so a boundary group there would be non-zero-width and silently fail to strip), and `STATE_CLAIM_TERMINAL_RE` (boundary-anchored, excluding hyphen from the boundary-char class) for the two direct whole-string `=~` checks in `state_claim_direction_i`/`state_claim_direction_ii`. New RED spec `tests/test_state_claim_word_boundary_78e1.sh` (`# roadmap:78e1`) covers the hyphenated-compound non-fire cases, the id:6b35 ground-truth regression, and confirms standalone terminal words still fire. Full suite: 308 passed, 0 failed, 0 expected-red.
Friction: my first fix attempt (a single anchored `STATE_CLAIM_TERMINAL_RE` reused verbatim in the "is <WORD>" sed-strip site) broke `test_contradiction_predicate_5533.sh` — the anchored boundary group is not zero-width except at true string-start, so embedding it right after an already-consumed "is[[:space:]]+" separator left nothing for the boundary group to match, and the strip silently no-op'd. Split into two variables (bare vs anchored) to fix.
refactor: none needed — a two-variable split plus one fixture, no new duplication introduced.


## 2026-07-26 15:32 — executor (sonnet, relay-loop)
...
## 2026-07-26 15:04 — review (claude-opus-4-8, relay-loop)

Anti-gaming review of the id:78e1 close (diff relay-ckpt-20260724-2130..HEAD). VERDICT: GENUINE GREEN, not reopened. The word-boundary anchoring of `lib-state-claim.sh` is a real red-green fix: running the NEW `tests/test_state_claim_word_boundary_78e1.sh` against the OLD lib (from relay-ckpt-20260724-2130) FAILS at the first assertion ("'fail-CLOSED' wrongly fired direction (i)"), proving the RED spec is not a tautology; it passes against the new lib. Ground-truth confirmed: `state_claim_violation` on id:6b35's actual ROADMAP line ("fail-CLOSED is the key property") now returns empty (was firing 'i' before) — the live false positive is genuinely resolved. gaming-scan.sh: clean (0 DELETED_TEST/ADDED_SKIP/REMOVED_ASSERT). Only ONE test file touched (the new 78e1 spec, +104/-0); no existing test modified — `test_contradiction_predicate_5533.sh` unchanged (kept passing by the two-variable STATE_CLAIM_TERMINAL_WORDS/RE split the executor's friction note describes, not by editing the test). No `@owner-accepted` introduced by any executor commit. refactor: `none needed` claim matches the diff (clean two-variable split + one fixture, no leftover cruft). Contract pointer v10 == canonical v10 (no drift). Full suite independently re-run: 308 passed, 0 failed, 0 expected-red. cross-ledger clean; roadmap-lint exit 0 (only pre-existing baselined WARNs id:6b35/id:659c). id:78e1 `[x]` in ROADMAP justified; no TODO twin (nothing to co-tick). No REVIEW_ME boxes. relay-doctor: report-only findings are cross-repo (it-infra parked orphan) or the pre-existing id:82c4 shadow-parity backlog — none new for this repo.

Reverse-handoff (§5b): the sole genuinely-new open ledger line this window is id:27e3, ingested into TODO.md from cross-project inbox (routed:609e from loderite) by commit 3a62ea5 — a relay bug report: `MECH_FALLBACK=fallback-haiku` does not cover the discover-prelude hop (dispatched `model:"bash"`, hard-aborts the pool at round 0 before the mid-loop haiku rewrite applies). QUALIFIED as a handoff-spec candidate, NOT promoted to ROADMAP: the fix is relay-loop.js prelude-dispatch internals entangled with the gated model:bash proxy substrate cluster (id:94b8/6b35), and a Workflow-dispatch bug is not hermetically RED-spec'able in-sandbox (sandbox-2ec4). Left in TODO under its existing id:27e3 for a strong handoff turn to author the spec. routine_open after re-derivation: 0 open [ROUTINE] items in ROADMAP.
...
## 2026-07-28 11:15 — review (claude-opus-4-8, relay-loop)

Anti-gaming review of the diff window relay-ckpt-20260726-2020..HEAD (7 commits). One substantive code commit — id:18e2, the quota-gate BOOTSTRAP DEADLOCK fix in `relay/scripts/quota-stop.sh` — plus owner `/relay human` ledger housekeeping (REVIEW_ME/ROADMAP/TODO), the prior session checkpoint, and a CHANGELOG line. VERDICT: GENUINE GREEN, nothing reopened. gaming-scan.sh: clean (0 DELETED_TEST/ADDED_SKIP/REMOVED_ASSERT). The one modification to an existing test (`tests/test_quota_extrapolation_fallback.sh`: pinning `RELAY_QUOTA_SAMPLES_FALLBACK=""` in `run_extrap`) is a legitimate HERMETICITY input change, not an assertion weakening — resurrection check holds: all pre-existing cases (a)–(e2) still pass against the new implementation, and the recency-guard case (c2) still fail-safes to exit 2 when no fallback is configured (only the input env pin changed, the assertion logic is untouched — the §2b.1 negative control). The three new cases carry a genuine anti-bypass guard: (f) stale-primary→fresh-fallback proceeds (exit 0), (f2) fresh fallback with HIGH burn still STOPS (exit 3 — proving a data-source swap, not a bypass), (f3) both-stale keeps the fail-safe exit 2 and names both sources. Implementation is a real logic change (candidate-loop with the recency check moved inside so a stale primary falls THROUGH), not fixture special-casing. id:18e2 already `[x]` in TODO.md (session fix, no ROADMAP twin — nothing to co-tick). Full suite independently re-run: 309 passed, 0 failed, 0 expected-red. Contract pointer v10 == canonical v10 (no drift). Health: relay-doctor per-repo clean (the 174 surfaced are the standing id:82c4 relay-core shadow-parity backlog, cross-repo, not introduced this window); roadmap-lint exit 0; orphan-scan --cross-ledger clean.

Reverse-handoff (§5b): four open ledger lines added this window, none force-promoted. id:659c (TODO + ROADMAP twin) — already `[HARD — decision gate]` + auto-GATED (id:3801, route:decision-gate) awaiting a /meeting; qualified, skip. id:194e — sharpened (not new) design item (general failure-ledger, enumerated design qs a/b/c), /meeting candidate; leave. id:7142 `[ROUTINE]` (commit-ledger.sh path-vs-name doc drift) — carries a genuine (a) fix-script-to-resolve-bare-name vs (b) fix-the-doc fork; per §5b design-judgment, left in TODO, not forced into ROADMAP. id:bf19 `[ROUTINE]` (roadmap-lint DECIDED-LEFT-OPEN two false-positive classes) — execution-ready but owner-filed TODAY by the `/relay human` session and requires a coordinated multi-consumer change (lib-state-claim.sh + roadmap-lint.sh + todo-conformance.sh twin), so left in TODO under its own id for a strong handoff turn to author the RED spec (respecting the owner's authorial placement + avoiding the under-specified-promotion size-out class id:194e just named). routine_open after re-derivation: 0 open [ROUTINE] items in ROADMAP. One pre-existing open REVIEW_ME box (id:659c inline-gate DECIDED-LEFT-OPEN decision) left for the owner; no new boxes added this pass.


## 2026-07-28 11:20 — reviewer (claude-opus-4-8, fable-standin, relay-loop)
...
Worked id:44a1 — extended id:3801's seam emitter (`handback-followup.py`, `relay-loop.js` REPORT_SCHEMA) so a hard-split `proposed_split` seam must carry `acceptance`/`done_check`/`file` alongside `title`. `seam_line()` now renders each seam with `**Acceptance**`/`**Done-check**`/`**Context**` sub-bullets instead of a bare title one-liner, and a new pre-write guard rejects (exit 2, writes nothing) any seam missing one of the 3 required fields — same fail-LOUD-write-nothing shape as md-merge's id:1b1a. New `tests/test_seam_emitter_acceptance_44a1.sh` (roadmap:44a1) covers reject-on-missing/partial, conforming-render, and an integration check against id:213a's `roadmap-lint.sh` (rendered seam passes NO-ACCEPTANCE-NO-TWIN without a TODO.md twin). Updated `tests/test_handback_followup.sh`'s hard-split fixtures to the new required-field shape.
Friction: both this test and the pre-existing `test_handback_followup.sh` previously resolved `handback-followup.py` via `$HOME/.claude/skills/...` first, falling back to `$ROOT` only if absent — since that path is a symlink to the MAIN checkout, running either test from a worktree (this session) silently exercised STALE code instead of the worktree's edits. Fixed both tests to resolve `$ROOT` unconditionally (worktree isolation, id:6f1c/f682). Did not sweep other test files for the same pattern — out of scope for this item, flagging in case it recurs elsewhere.
refactor: none needed — the change is additive (new required schema fields + a new guard function); no existing duplication to clean up.
...
Worked id:907e — amended the id:c919 `workCreated` producer (`relay/scripts/relay-loop.js`, handback branch of `integrate()`) from the route-only test to "did THIS ROUND change THIS REPO's verdict class". All three mandatory clauses landed: (i) the "after" class comes from a fresh `classify-repo.sh --repo/--path` mechanical `model:'bash'` hop dispatched at the site, which never reads `state.discoverCache` — the in-source comment says so and cites id:c3a6, because reading the cache would answer "unchanged" forever and ship the amendment as a silent no-op; (ii) the named repo-scoped pair is `verdictClassBefore` (discovery's class for the repo when it dispatched the unit) vs `verdictClassAfter` (a whole-repo re-classification, so a sibling unit's flip under id:1f4f is counted too), not per-handback causality; (iii) a module-level oscillation guard (`recordVerdictClass`, window 5 / ≥3 flips) makes a flapping repo's class change stop counting as work — so the round scores dry again — AND names the flap in the exit reason on both the drain path (`drained:verdict-oscillation:<repos>`) and the previously-null MAX_ROUNDS seatbelt (`max-rounds:verdict-oscillation:<repos>`), so a livelock can no longer read as an ordinary termination. `isDryRound`'s `substantive`/`surfaced` terms are untouched; this widens `workCreated` only. Friction: the ±40-line window the RED spec scopes its source-shape greps to is tight — the first draft's rationale comment pushed the c3a6 citation out of range and had to be restated compactly at the predicate line; separately, an early rewrite of the drain-exit line broke `test_relay_loop_drain_vs_blocked.sh`'s literal `stopReason = stopReason || 'drained'` assertion, fixed by preserving that literal and qualifying the reason on the next line rather than weakening the test. On a classifier hop failure the predicate falls back to c919's route-only answer and LOGS loudly — an unreadable class is not evidence of a flip, and defaulting a repeated failure to "keep going" is the livelock clause (iii) exists to prevent.
refactor: extracted the two reusable pieces instead of inlining them — `parseVerdictClass()` (MECH-ERROR / id:3557 MECH-OK-sentinel / malformed-JSON tolerant, returns null rather than inventing a class) and `recordVerdictClass()` (the windowed flip counter + `oscillatingRepos` set, consumed by both exit-reason sites); no duplication left at the predicate site.
Worked id:49e0 — added roadmap-lint.sh DOCTRINE rule 3(d) DEAD-GATE: for every comment-anchored `<!-- gated-on:CSV -->` on an OPEN, active-section, top-level ROADMAP item, each target token must exist as a checkbox item in ROADMAP.md itself; a target that resolves only in TODO.md (never promoted), only in TODO.archive.md (retired — gate permanent), or nowhere at all fires LOUD on stderr naming BOTH the gated id and the target id plus the remedy. Extraction goes through the SHARED id:46f6 engine (lib-typed-edges.sh `typed_edges_gated_of_line` / `typed_edges_build_state_map`), so a backticked/prose `gated-on:` mention is never read as an edge (the id:4da4/0d58 trap) — this is the engine's third caller, no fourth copy written. Tiering mirrors 3(a)/3(b)/3(c) exactly: WARN + report-only by default, ERROR + nonzero under `--strict`; no third tier invented, and it therefore flows into relay-doctor.sh unchanged. Three maps are built rather than one merged map because WHERE the target lives IS the finding (resolve-gates.sh correctly merges them for its different question). RED spec `tests/test_roadmap_lint_dead_gate.sh` authored first and proved red ("expected EXACTLY 2 DEAD-GATE findings, got 0"), then green; suite 331 passed / 0 failed / 15 expected-red. **A fourth live instance of the trap, previously undetected:** the rule immediately found `id:ebbe` gated-on `id:0534`, whose target is `[x]` archived at `TODO.archive.md:432` and was never a ROADMAP item — same permanent-gate shape as f6d5→8ba1. NOT fixed here (out of this item's scope, and re-targeting is a judgement call); surfaced for triage. Friction: none — but note the fixture-authoring gotcha that ids must be 4 HEX digits (`g001` silently reads as "no id" and trips the grammar clause instead of the rule under test).
refactor: none needed — the rule reuses the existing shared typed-edge engine rather than adding an extractor, and introduces no duplication to hoist; the only new state is the three resolution maps, each read once.
...
Worked id:8af2 — the pool's execute unit silently took `actionable_routine_ids[0]` (id:b09e) and no surface recorded which id that was nor how many were eligible, so a mid-list primary fix was invisible until a human read the classifier JSON by hand. Added `dispatchChoiceFor(unit)` to `relay/scripts/relay-loop.js` — a pure REPORTER over the existing `dispatchItemFor`/`namedItemsFor` selection (`{item, itemRank, eligibleCount}`; rank 0 marks a user-injected `--item` so "1 of N" is never faked; `eligibleCount` is the post-orphan-suppression set the selection actually ranges over). It is computed ONCE at the dispatch site and stamped on BOTH surfaces from that one value, so the live view and the forensic log can never disagree: the `state.inFlight` row now renders `- dotclaude-skills  mode=execute  agent=unit-1  → id:cbd2 (1 of 21 actionable)` in RELAY_STATUS.md's In-flight section, and `pushEvent('dispatch', …)` carries `item`/`itemRank`/`eligibleCount` into relay-events.jsonl. The sandbox `log()` line got the same suffix. RED spec `tests/test_dispatch_choice_visible_8af2.sh` was authored first and failed on 6 clauses; it pins the count as MANDATORY on both surfaces (the id alone was the invisibility), pins the CALL SITE not just the token (the [[relay-builtgreen-but-unreferenced]] class), fails OPEN for units with no named item, and carries a SCOPE GUARD asserting the b09e selection rule string is unchanged — so an implementation that "fixes" ordering instead of visibility fails the test. Verified the whole read chain rather than assuming it: inFlight fields survive `snapshotState`'s array spread, `relay-status-publish.sh` passes both the content and the post-sentinel event block through verbatim to `relay-state-write.sh event-append`. Friction: none — the item was well-sized. One honest gap worth naming for the reviewer: nothing programmatically CONSUMES the new event fields today (`handback-summary.mjs` reads only handback events; `relay-doctor.sh:281` notes a dispatched-verdict read as a future upgrade), so the event half's consumer is a human's `jq` — which is what the acceptance asks for, but it is one step short of the mechanized check that would keep it honest.
refactor: extracted the row decoration into a named `inFlightChoice(r)` helper instead of inlining a third nested ternary into the `state.inFlight.map` template — the row expression already carried repo/mode/agent and a fourth clause inline would have been unreadable. Deliberately did NOT share one formatter between the status row and the `log()` line: their prefixes genuinely differ (`→ id:` vs `item=id:`) and `buildRelayStatus` must stay self-contained so the hermetic test can extract and evaluate it without dragging in module-level state.


## 2026-07-31 16:13 — reviewer (claude-opus-5)

wave 2: id:8123 chain-end classifier re-ask ENDS review starvation (chain_ended in discover-sig blob so the c3a6 cache can't no-op it); id:8af2 dispatch-choice visibility; id:ebbe dead gate dropped


## 2026-07-31 — ANTI-GAMING REVIEW (independent audit, window `relay-ckpt-20260730-2018..HEAD`)

Audited the 46-commit window that a single session both wrote and integrated, using the explicitly-supplied window (the derived one via `relay.toml`'s `last_strong_ckpt = relay-ckpt-20260731-1613` yields 0 commits — that is the already-filed `id:ecce`, and deriving it would have produced a vacuous clean report).

**Test integrity: clean.** `gaming-scan.sh <repo> relay-ckpt-20260730-2018` → no output (no `DELETED_TEST`, `ADDED_SKIP`, or `REMOVED_ASSERT`). Exactly ONE pre-existing test file was modified in the window — `tests/test_roadmap_scope_table_consistency_c480.sh` (`263b9f4`, id:6a15 part a) — and the change is legitimate: it widens a PRECONDITION grep from `model: 'bash'` to `model: ('bash'|MECH_MODEL)` because `490ac6e` (id:4239) genuinely replaced the literal with the `MECH_MODEL` indirection; the §1 contradiction assertion itself is unchanged, and `grep -n "label: \`release:"` confirms exactly one release hop so the added `head -1` is a no-op. The reported mid-work breakage of `tests/test_relay_loop_drain_vs_blocked.sh:112` was indeed fixed by restoring the implementation literal, not by editing the test: that file has ZERO commits in the window and `relay-loop.js:2924` carries `stopReason = stopReason || 'drained'` verbatim. Full suite re-run from the main checkout: **335 passed / 0 failed / 13 expected-red** (single tier — this repo declares one, `tests/run-tests.sh` via `make test`; no e2e/integration tier exists to skip). The known `test_git_lock_push_slash_branch.sh` flake did not fire.

**Both `id:c3a6` cache traps verified EMPIRICALLY, not by reading the comments.** (a) `id:907e`'s "after" class comes from an `agent()` hop running `classify-repo.sh --repo/--path` directly; `state.discoverCache` is untouched at that site, and `grep -n "CACHE\|cache"` across `classify-repo.sh` / `classify-verdict.sh` / `gather-repo-state.sh` finds no cache read (the only hit is `RELAY_WORKTREE_BASE`). Ran the script live on this repo: exit 0, `{"verdict":"execute",…,"actionable_routine_open":21}` — so the hop is functional and `parseVerdictClass` has real JSON to parse, not a dead branch that silently falls back to route-only. `unit.verdict` and the script's `verdict` share one vocabulary (`relay-loop.js:446` enum). (b) `chain_ended` genuinely participates in the hash AND is genuinely emitted: `discover-sig.sh:135` prints a labelled `== chain_ended ==` blob section fed by the `:146` jq read, `relay-loop.js:2298` passes `"chain_ended":true` into the postSig call, and `:2639` sets `unit.chainEnded`. Proved end-to-end by running `discover-sig.sh` twice on this repo — `8921…c643` without the flag vs `8228…9432` with it. Neither half is the banked built-green-but-unreferenced shape.

**The cascade branch works and is NOT unconditional.** Drove `classify-verdict.sh` directly across seven cases: `chain_ended`+`substantive_unaudited`+21 open `[ROUTINE]` → `review` (the starvation is broken); `chain_ended` without unaudited → `execute`; unaudited without `chain_ended` → `execute` (byte-equivalent pre-8123 behaviour); explicit `chain_ended:false` → `execute`; empty `chain_end_reason` → `execute`; bare `chain_end_reason` string alone → `review`. Both rank-0 parity guards still outrank it: `dirty` → `blocked`, and `has_upstream`+`upstream_ahead_behind` (TAB-separated) → `blocked`.

**Spec-drift: none found** against `docs/meeting-notes/2026-07-31-1231-execute-review-cadence-starvation.md`, read at the AMENDED text (A1/D2b/A3), not the superseded D1/D2. A1's two false-premise sites are both corrected — `ROADMAP.md:2273` carries the struck-through *"exactly as a single execute's are today"* with its correction, and the `relay-loop.js` *"reviewed next pool"* comment is gone (pinned by the RED spec's clause 6). D3's three mandatory clauses are all present, and clause (iii) is implemented on BOTH sides where the decision said *"or at minimum"*. One deviation worth recording as deliberate, not drift: D3(i) says *"direct `classify-verdict.sh` invocation"* and the code calls `classify-repo.sh`, which is the wrapper that assembles the input and pipes it to `classify-verdict.sh` — `classify-verdict.sh` alone cannot be called without an assembled blob, so this is the same behaviour through the correct entry point, and A1's "classifier remains sole verdict authority" holds. §2d over-reach check: no item's diff is a superset of what its cited source authorized. `id:8af2` in particular carries its own scope guard asserting the `id:b09e` selection rule string is unchanged, so it cannot have widened from visibility into re-ordering.

**Spot-checked the four riskiest of the 13 TODO closes by re-running the cited check, not by reading the item's prose.** `27e3`: zero non-comment `model: 'bash'` option literals remain (the one `grep` hit at `:1340` is inside a `log()` string), and the prelude dispatches `model: MECH_MODEL`. `94b8`: wiring child `id:6176` is `[x]` at `ROADMAP.md:112`, and `RELAY_LOG.md:3100` reads verbatim *"DECISION: consolidate to Workflow+proxy (owner-ratified)"* — its scoping is also right, since the still-live proxy-launch hazard is `id:6b35`, which correctly stays OPEN. `2b23`: all eleven named children (`9f1b 8ba1 23d8 2ca6 93fe b3cc 2238 ec6f 9f19 de4b cd7a`) verified `[x]`, satisfying the parent's own *"stays open until children close"* rule. `ad7c`: the only close resting on evidence outside the repo — surfaced to `REVIEW_ME.md`, see below.

**`id:1a34` and `id:49e0` verified live.** `ckpt-tag.sh`'s documented format (`:5`, `:9`) and detected pattern (`:119` `claude-[a-z0-9.-]+`) now agree, and `:125` warns on stderr quoting the offending label. `roadmap-lint.sh`'s rule 3(d) fires on a synthetic `gated-on:fffe` probe (*"resolves NOWHERE … a dangling gate target"*) with a clean control on the unmodified file — so the rule is not a no-op, and no dead gates remain after `76a6e1d`.

**REOPENED: nothing. Gaming flags: none.**

**Fixed in-pass — real cross-ledger drift on all five cluster items.** `orphan-scan.sh --cross-ledger` reported `907e`, `8123`, `1a34`, `49e0`, `8af2` as `TODO:[ ] ROADMAP:[x]`. Each TODO line was confirmed to be the same work as its ROADMAP twin (single-id-two-views), so all five were ticked per review.md §5; the scan is now silent. This is the predictable cost of a session integrating its own work — the ROADMAP half gets ticked at merge and the TODO half is forgotten.

**Process note, not a flag: two RED specs were self-authored by their implementation commit** — `tests/test_roadmap_lint_dead_gate.sh` (in `5dbdbea`, id:49e0) and `tests/test_chain_end_sig_8123.sh` (in `46f45ff`). That is the `id:9b9b` "executor graded its own exam" class. Its own stated remediation is that independent verification of the behaviour makes it cheap, and this review supplied exactly that: the dead-gate rule was driven with a synthetic probe plus control, and the `chain_ended` signature was proved by a two-run hash diff. The other four specs (`907e`, `8123` re-ask, `c500`/`1a34`, `8af2`) were handoff-authored ahead of their implementations (`5878350`, `97ef235`), which is the intended separation.

**relay-doctor:** `roadmap-lint` reports exactly one warning, the known `DECIDED-LEFT-OPEN` false positive on `id:5f31` (the `id:bf19` class) — no new grammar violations. `todo-conformance`: clean. `scan-routed` surfaced 8 inbox dead-letters, 6 addressed to this repo, all arriving after the window's 8 ingest commits — boxed in `REVIEW_ME.md` rather than auto-routed, per `id:678e`'s report-only gate. No mechanical orphans, no parked-orphan findings.
...
## 2026-07-31 19:03 — executor (sonnet, relay-loop)
...
Worked id:0af4 — `meeting/md-merge.py update_ids()` no longer replaces a matched id's line unconditionally: a replacement now MUST carry its own anchored `<!-- id:XXXX -->` marker, and MUST stay a `- [ ]`/`- [x]` checkbox line when the target it's replacing is one (a non-checkbox target, e.g. a `## [LANE] Title <!-- id -->` heading-as-item, is unaffected — the checkbox rule is conditional on the target's shape). A violation exits non-zero, names the offending id and reason, and writes NOTHING — the same fail-loud shape id:1b1a already gave the unmatched-id path. Added the explicit APPEND mode (`{"id":"XXXX","append":"<text>"}`) that adds text to the existing line while preserving every original byte and re-anchoring the id marker as the last thing on the line, so a caller that only wants to append is no longer stuck once (1) refuses their old partial-line payload. `tests/test_md_merge_append_guard_0af4.sh` was a pre-existing hermetic RED spec from an earlier handoff (C3, run relay-20260729-133054-23284) — it went RED→GREEN unmodified in its assertions; the only change needed was a one-line `grep -qF -- "$LONG"` fix (missing `--` before a pattern starting with `-`, a GNU-grep option-parsing bug in the fixture itself, unrelated to the implementation). The specified regression suite (`test_md_merge_update_ids_strict.sh`, `test_md_merge_commit.sh`) stayed green unmodified. Full suite: 341 passed, 0 failed, 9 expected-red (unrelated open items).
Friction: none — the RED spec already existed from a prior handoff session (module the one-line grep portability fix), so this unit was mostly an implement-against-spec turn.
refactor: none needed — the new validation/append helpers are small, single-purpose, and don't duplicate existing logic; the id marker regex is a documented mirror of `lib-anchored-id.sh`'s per the item's own instruction.

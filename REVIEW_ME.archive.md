...
- [x] **id:7a5e -- parked orphan `relay/orphan/relay-20260831-190008-13032-execute-e8d4-0` carries UNVERIFIED partial S4 work, and it contains a concrete regression.** The execute child for id:e8d4 (S4) died with an agent-error; `f272` commit-and-park saved 3 of the 12 files (`lane-convert.sh`, `todo-conformance.sh`, `unpromoted-scan.sh`) as `345982f4`. Do NOT merge it as-is. Its `unpromoted-scan.sh` hunk normalizes the WHOLE line (`line="${line//—/-}"`) rather than only the delimiter inside a lane tag, and its own comment asserts the downstream em-dash-aside check "reads identically either way". That is FALSE: `unpromoted-scan.sh:179` tests `[[ "$after" =~ ^[[:space:]]*— ]]` for the clause-boundary aside, so after the blanket substitution that branch is DEAD for every line and the id:4da4 "first recognized tag wins" behaviour it exists to preserve silently regresses. A comment claiming code behaviour is a derived doc (CLAUDE.md); this one was never verified because the child died before running the suite. The next S4 executor should either start clean or fix that hunk to substitute inside the tag only. **RESOLVED 2026-08-31.** The regression described here was real and was NOT merged. The FIRST residue (`345982f4`, blanket `${line//—/-}`) was discarded unreviewed. The SECOND residue (`c8b33ece`, 4 files) was reviewed hunk by hunk and salvaged in `f035a5bc`: `classify-repo.sh` / `lane-convert.sh` / `todo-conformance.sh` were sound as authored, and `unpromoted-scan.sh` carried a DIFFERENT defect of the same class -- it added `[HARD - pool]` to `primary_lane`'s tag list but left the promote-disposition regex em-dash-only, so such an item fell to verdict-neutral `laned` (the id:4b64 lodelore bug in the new delimiter). Fixed before merge and covered by case (p) of `tests/test_unpromoted_scan_newvocab.sh`, with a negative control. Both orphans are now discarded; remaining 8 files tracked as `id:6bf5`. <!-- id:7a5e -->
...
## Review 2026-09-08 (id:62c9 -- Workflow-aware node-check helper, window `relay-ckpt-20260908-0859`..HEAD)
Adversarial audit of the `id:62c9` fix. **One declared tier exists and it RAN**: `tests/run-tests.sh`
-- **596 passed, 0 failed, 0 errored, 8 expected-red**, re-run here rather than quoted. No `e2e`/
`integration` tier is declared. `gaming-scan.sh`: silent, exit 0. Provenance greps for
`@owner-accepted` / `@owner-answered` / `answer-src:` over the window's ledger diff: no marker
added, removed, or modified. `orphan-scan --cross-ledger`: clean; the `id:62c9` ticks agree in
`ROADMAP.md` and `TODO.md`. `roadmap-lint`: 5 pre-existing warnings (4 DEAD-GATE, 1
NO-ACCEPTANCE-NO-TWIN `da55`), none from this window. `relay-doctor`: findings are all
pre-existing and unrelated (one install-drift file, 4 parked orphans, relay-core shadow
mismatches).
**The negative case now executes.** `verify-negative-cases.py tests/test_workflow_node_check_62c9.sh`
reports `green-now OK` / `red-there OK` and states the roadmap carve-out EXPIRED -- the id:7c82
shadow is gone now that the item is ticked. Note its scope honestly: the declared case deletes
`tests/lib-workflow-check.sh`, so it pins assertion (a) (helper absent) and nothing deeper. That is
structurally forced -- no later assertion is reachable without the helper -- not a weakened
declaration.
**The migration was checked independently, not accepted.** All 48 files that call
`workflow_node_check` also source the helper, and in every one the `source` line precedes first
use. Every tracked `*.js`/`*.mjs` in the repo was run through a bare `node --check`: exactly one
fails, `relay/scripts/relay-loop.js`. No other Workflow script exists here, so nothing was missed.
The surviving bare-check sites target `drain.mjs`, `lint-embedded-literals.mjs`,
`lint-workflow-templates.mjs`, `lint-mech-model.mjs` (all verified ordinary modules that pass) plus
one fixture heredoc in `test_source_grep_lint.sh` -- the executor's judgement holds. Assertion (d)
of the spec is a live ratchet now that the carve-out expired: a NEW bare `node --check` site added
later fails the suite.
**Line-number fidelity holds; no offset.** Braces injected after source lines 1, 2, 100, 2000, 4811
and 4930 each report at the mutated file's own line numbering (e.g. 2000 -> 2001), so the joined
prologue costs zero lines. `TMPDIR` probes (spaces, non-existent, read-only, and directories
carrying `package.json` `type: module` / `commonjs`) all behave -- loud non-zero on the two failure
cases, correct pass on the rest. No temp leakage: `/tmp/tmp.*.js` is 0 before, 0 mid-run, 0 after a
full suite.
Two defects found in the helper are mechanically actionable and were filed rather than boxed here:
`id:1b0e` (an unreadable file returns 0 -- a silent PASS) and `id:e044` (a line-initial
`export`/`import` inside a template literal or block comment falsely refuses the file). Both carry
...
- [x] id:62c9 -- **does the Workflow runtime execute `relay-loop.js` in STRICT mode?** The wrapper
  ...
  a bare `node --check` on this file resolved to CommonJS (also sloppy) before the Node upgrade, and
...
- [x] *(CLOSED 2026-09-09 `/relay human`: DISCHARGED — the file is now installed. `ls -la ~/.claude/skills/relay/scripts/lib-archive-idempotency.py` resolves to a symlink dated 2026-09-09 11:45 pointing at `relay/scripts/lib-archive-idempotency.py` (6688 B, present since 09-04), and `Makefile:98` still declares it in `relay_FILES`. The "which side is authoritative" question in the id:168c twin below is answered by the resolution actually taken: install, not drop. Re-checkable by re-running relay-doctor's install-drift check.)* **`relay/scripts/lib-archive-idempotency.py` is declared in `relay_FILES` but is not present
...
- [x] *(CLOSED 2026-09-09 `/relay human`: DISCHARGED, same evidence as the id:c47f twin above — installed 2026-09-09 11:45, declaration kept. Owner's call resolved by the action taken: install the file, do not drop the declaration.)* **`relay-doctor` install-drift: `relay/scripts/lib-archive-idempotency.py` is declared in `relay_FILES` but is not installed under `~/.claude/skills/relay`.** A declared-but-uninstalled library is the `id:1102` class -- the manifest and the install tree disagree, so a caller resolving it through the install path fails at runtime while every in-repo test passes. One file; `make install-relay` is the likely resolution, but which side is authoritative (drop the declaration, or install it) is the owner's call. <!-- id:168c -->

- [x] *(CLOSED 2026-09-09 `/relay human`: the question this box actually asks — "whether the 2026-08-26 recovered review branch still has anything unmerged worth taking" — is MOOT. `git branch -a --list '*recovered-20260826*'` returns nothing; the branch is gone and its content landed on main via `3bb72569` ("id:7354 independently verified from recovered spec"). The orphan population is now 2, not 4, and both survivors were dispositioned earlier in this same turn (leave-parked, see the 2026-09-09 boxes below). Nothing here is unresolved.)* **Four parked orphan branches now sit on this repo, and one of them is THIS run's own execute worktree.** `relay/orphan/relay-20260907-100619-27900-execute-64f9-0` (`ca6a4a04`) is the WIP residue of the execute unit whose agent-error triggered this chain-end re-ask -- expected, but it means `id:64f9` was attempted and left unfinished this round. The other three are `recovered-20260826-review-3d9ca6f3` and the two 2026-09-05 execute residues, of which `relay/scripts` reconcile already mined `id:be51` and `id:ceca`. Nothing here is lost work; the question for the owner is whether the 2026-08-26 recovered review branch still has anything unmerged worth taking, since it is the oldest and is the only one that is not an executor WIP auto-commit. <!-- id:5121 -->

- [x] *(CLOSED 2026-09-09 `/relay human`: the box names its own resolution — "a `/relay human` or a `--apply` pass closes them" — and that pass ran at the start of this turn. `scan-routed.sh --apply` drained both twinned items: `RESOLVED routed:b71e -> [cartulary]` and `RESOLVED routed:113e -> [meeting-rpg]`, each with its twin verified present in the target repo before the inbox line was removed.)* **`scan-routed` reports 2 twinned-resolvable inbox items -- 0 dead-letters.** Both have their `routed:` twin already present in the target repo, so `--apply` would simply drain them from the inbox. Report-only per the review contract; a `/relay human` or a `--apply` pass closes them. <!-- id:b555 -->
...
- [x] *(CLOSED 2026-09-09 `/relay human`: all four landed. Checked per token rather than by a bare inbox count — `routed:de13`, `routed:37ce`, `routed:f83a` and `routed:1fd4` each return 0 occurrences in `~/.claude/projects/todo-inbox.md` and exactly 1 in this repo's TODO/ROADMAP (live or archive), which is the twin-present shape `inbox-done` requires. `inbox-scan-repo.sh dotclaude-skills` now surfaces no open `[dotclaude-skills]` line at all.)* **The inbox now holds 4 dead-letters addressed to this repo, up from 0 at the 10:45
...
- [x] *(CLOSED 2026-09-09 `/relay human`: SUPERSEDED by the newest `relay-doctor` snapshot in the 2026-09-09b section, and its one actionable half is now discharged — `routed:5997` -> `[inflownistration]` was filed by this turn's `scan-routed.sh --apply` (commit `a2cabf0` in that repo). The relay-core shadow-parity gap is standing state tracked by its own box, not a per-snapshot finding; it is deliberately not re-boxed here.)* **`relay-doctor` (report-only, never blocking): one inbox dead-letter and a large relay-core shadow-parity gap, both pre-existing and neither touched this review.** `scan-routed.sh` reports `routed:5997` -> `[inflownistration]` absent from that repo's `TODO.md`+`ROADMAP.md` (a docs/instances.md INSTANCE about the executor contract riding two mutually exclusive delivery/freshness channels); routing it is a cross-repo act this worktree must not perform. `relay-core` shadow reports 34,774 mismatches over 327,490 rounds -- bash stays authoritative and the flip gate (100% parity + N=5 clean rounds) is nowhere near met. Cross-ledger drift: clean. `roadmap-lint`: clean, every open item carries a recognized lane tag + id (including the reopened `id:64f9` and the new `id:963c`). The 2 "retirable worktrees" it lists are this run's own live children, not residue.

- [x] *(CLOSED 2026-09-09 `/relay human`: SUPERSEDED by the newest snapshot in the 2026-09-09b section; `routed:5997` filed this turn, see the box above. A snapshot whose only delta from its predecessor is a counter tick is not separately actionable.)* **`relay-doctor` (report-only): unchanged from the previous review apart from drift in the shadow counter -- recorded so the silence is on the record rather than assumed.** `routed:5997` -> `[inflownistration]` is still an inbox dead-letter (routing it is a cross-repo act this worktree must not perform). `relay-core` shadow now reports 34,925 mismatches over 328,396 rounds, up from 34,774/327,490 -- bash stays authoritative and the flip gate (100% parity + N=5 clean rounds) is not close. Cross-ledger drift: clean. Trunk-vs-remote, lean-toolchain pins, quota-config, hooks-path shadowing: all clean. The 3 "retirable worktrees" are this run's own live children plus `wisenheimer`'s, not residue.

- [x] auto-reconcile parked orphan `relay/orphan/relay-20260908-231617-32609-execute-repo-0` (036524f0) — non-ledger (code) diff in 1 file(s) (e.g. tools/shrink-acceptance.py) — needs strong-turn review; --auto never auto-merges code. Subject: chore(relay): WIP UNVERIFIED residue auto-commit for worktree relay-20260908-231617-32609-execute-repo-0 (id:f272 commit-and-park; do not treat as reviewed). Surfaced 2026-09-09 09:16 by `relay-reconcile.sh --auto` (id:7809); integrate or discard manually: `relay-reconcile.sh --integrate relay/orphan/relay-20260908-231617-32609-execute-repo-0` / `--discard relay/orphan/relay-20260908-231617-32609-execute-repo-0`.

- [x] **Parked orphan `relay/orphan/relay-20260909-091623-10249-execute-c655-0` (`7425c29b`) carries 79 lines of UNREVIEWED code on `relay/scripts/todo-conformance.sh` — this run's own `id:c655` execute child died (agent-error) and its residue was commit-and-parked (`id:f272`).** Surfaced by this review, not by `--auto`: `relay-reconcile.sh --auto` never auto-merges a non-ledger diff, and it had not run again since the child died. `id:c655` (repo dimension for the head-length / shape-prose baseline keys) is correctly still OPEN and dispatchable — the parked work is NOT credited toward it, and the next executor must decide whether to salvage or restart rather than silently re-implementing over it. Disposition is a strong turn's: `relay-reconcile.sh --integrate relay/orphan/relay-20260909-091623-10249-execute-c655-0` after reviewing the diff, or `--discard` the same branch. Note the SECOND, older parked orphan (`relay-20260908-231617-32609-execute-repo-0`, `036524f0`, `tools/shrink-acceptance.py`) is still open one box above — two unreviewed code orphans now sit against the same shrink/conformance cluster, so decide them together or they will be re-implemented past each other.
...
- [x] **`id:4983`'s detail pointer points at ANOTHER item's note, and it is now a live input rather than dead decoration.** The ROADMAP line reads ``-- detail: `docs/ledger-notes/6546.md` `` followed by its own id marker; `docs/ledger-notes/4983.md` does not exist, and `6546.md` is `id:6546`'s note (strict-shape shrink wave 2 -- the 460-of-840 prose census), which says nothing about lane grammar. Introduced by the handoff commit `109d6f10`, where the two sibling items promoted in the SAME commit got correct pointers (`5f34`->`5f34.md`, `1608`->`1608.md`) -- so this is a slip on one line, not a convention. **Measured blast radius: 2 of 773 pointer-bearing item lines across all five ledgers, and both are this same item (`ROADMAP.md:1698` + its `ROADMAP.archive.md:4584` copy).** Isolated, not systemic. It did no harm HERE only because the ROADMAP block carried its Acceptance/Tests/Done-check inline; a reader or slicer following the pointer gets wave-2-shrink prose instead. Load-bearing now because `id:1608` (closed LAST review) made pointer-following live in `orphan-scan --shipped`, and `id:2ee1` exists precisely so `ledger-slice.sh` follows these pointers into executor prompts -- this is the `id:b015` shape (body relocated, address wrong). **Fix is either creating `docs/ledger-notes/4983.md` or dropping the pointer; I did NEITHER, because the item is closed+archived and rewriting an archived line is the owner's call.** <!-- id:4983 -->

  RESOLVED 2026-09-10 (/relay human --all, owner ruling): the missing note was CREATED as docs/ledger-notes/4983.md, composed from id:4983's own archived text (ROADMAP.archive.md:4584 head line + its Acceptance/Tests/Done-check bullets) and the WHY block of the test it specifies, tests/test_lane_grammar_ssot.sh. The note declares in its own header that it was hand-written rather than relocated by ledger-shrink.py, so it makes no false 'reproduced verbatim' claim. The MECHANICAL half is verified gone: classify-repo.sh charges MISSING_LEDGER_NOTE_BYTES (32,768 B) for any unreadable pointer-shaped path, and this REVIEW_ME box's own prose names docs/ledger-notes/4983.md, so every classification of this repo was inflated by 32,768 B; 'bash relay/scripts/classify-repo.sh --repo dotclaude-skills --path ~/src/dotclaude-skills 2>&1 >/dev/null | grep 4983' now returns empty (exit 1), where it previously printed the MISSING/unreadable warning. RESIDUAL, stated rather than papered over: the archived ledger line at ROADMAP.archive.md:4584 STILL spells its pointer as docs/ledger-notes/6546.md and was deliberately NOT edited -- the owner's ruling was to create the note, and rewriting an archived line remains the owner's call. So the note now exists while the item's own line still addresses another item's note; the repoint is a one-token change if wanted. Also corrected for the record: this box's line reference 'ROADMAP.md:1698' is stale -- id:4983 is no longer present in the live ROADMAP.md (grep returns no item line), so the blast radius is the single archive copy, not two live lines.

...
# Fixture rows are printf'd, not heredoc'd, so no SOURCE line here begins with a
# checkbox carrying an old-vocab lane tag -- the pre-commit lane-vocab ratchet would
# block that, and these are fixture rows rather than real ledger items.
{
  printf '# Roadmap\n\n## Items\n\n'
  printf -- '- [ ] A genuinely hands item whose re-lane criterion quotes `%s` in prose %s <!-- id:9321 -->\n' \
    '[HARD - pool]' '[HARD - hands]'
  printf -- '- [ ] A real pool item %s <!-- id:5555 -->\n' '[HARD - pool]'
} >"$tmp/src/repoP/ROADMAP.md"
...
long_title="$(python3 -c "print('x' * 115)")"
{
  printf '# Roadmap\n\n## Items\n\n'
  printf -- '- [ ] %s which relay lane the research seam gets (%s? a new [RESEARCH]) <!-- id:d84f -->\n' \
    '[HARD - meeting]' '[HARD - pool]'
  printf -- '- [ ] %s real pool item, positive control <!-- id:aaaa -->\n'    '[HARD - pool]'
  printf -- '- [ ] %s real meeting item, positive control <!-- id:bbbb -->\n' '[HARD - meeting]'
  printf -- '- [ ] %s real hands item, positive control <!-- id:cccc -->\n'   '[HARD - hands]'
  printf -- '- [ ] [INPUT - meeting] input meeting item, positive control <!-- id:dddd -->\n'
  printf -- '- [ ] [INPUT - access] input access item, positive control <!-- id:eeee -->\n'
  printf -- '- [ ] [HARD] bare hard item, positive control <!-- id:ffff -->\n'
  printf -- '- [ ] [INBOUND routed:1234 from other] %s leading prefix then genuine tag <!-- id:9999 -->\n' \
    '[HARD - hands]'
  printf -- '- [ ] %s a long non-bold title whose only tag [INPUT - access] sits well past any fixed head window <!-- id:be40 -->\n' \
    "$long_title"
  printf -- '- [ ] [HARD - strong model] a legacy-vocab bracket the lane chain does not itself recognize, but a 🚧 route:human note deep in the body still routes it <!-- id:2222 -->\n'
  printf -- '- [ ] this one truly carries no recognized lane at all, only the candidate-gate literal [HARD <!-- id:3333 -->\n'
} >"$tmp2/src/repoQ/ROADMAP.md"

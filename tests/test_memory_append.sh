...
# ── 5. Refuse to append to a GENERATED index (id:2e6d) ──────────────────────
# A derived MEMORY.md is regenerated from per-memory `hook:` frontmatter, so a
# hand-appended pointer would be clobbered. The script must refuse loudly.
gen="$tmp/generated/MEMORY.md"
mkdir -p "$(dirname "$gen")"
cat > "$gen" <<'GEN'
# Project memory — x

> Index = one terse hook per memory. GENERATED by tools/memory-index.py — do not hand-edit.

- [a](a.md) — hook a
GEN
before="$(cat "$gen")"
if "$SCRIPT" "$gen" "- [b](b.md) — hook b" 2>"$tmp/gen.err"; then
    fail "should refuse to append to a GENERATED index"
fi
grep -q "GENERATED index" "$tmp/gen.err" || fail "expected a loud 'GENERATED index' message on stderr"
[[ "$(cat "$gen")" == "$before" ]] || fail "generated index must be left untouched"
pass "refuses to append to a GENERATED index, leaves it untouched, explains why"

# ── 6. A hand-kept index (no marker) still appends normally ─────────────────
hand="$tmp/hand/MEMORY.md"
mkdir -p "$(dirname "$hand")"
printf '# Project memory\n\n- [a](a.md) — hook a\n' > "$hand"
"$SCRIPT" "$hand" "- [b](b.md) — hook b" || fail "hand-kept index must still append"
grep -q '^- \[b\](b.md)' "$hand" || fail "pointer did not land in hand-kept index"
pass "hand-kept index (no marker) still appends — the guard is per-project"

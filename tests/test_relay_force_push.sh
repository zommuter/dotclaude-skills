...
grep -qi "REFUSED" < <(printf '%s' "$out") || fail "refusal message missing when confirm gate unset"
grep -qi "ssh" < <(printf '%s' "$out") && fail "refusal output mentions ssh — gate not checked first"
...
if grep -Eq -- '(--force([^-]|$)|push +-f|-f +)' < <(grep -E 'git .*push' "$SCRIPT") ; then

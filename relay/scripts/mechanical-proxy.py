...
ALLOWED_RELAY_SCRIPTS = frozenset([
    ...
    "auto-integrate-orphan.sh",  # id:1048 -- the BOUNDED auto-integrate primitive, dispatched by autoIntegrateParkedOrphans() ONLY under the opt-in AUTO_INTEGRATE_ORPHANS knob. Missing here refuses the hop -> FAIL-OPEN to the real model, i.e. an LLM back on the merge-to-main critical path (the integrate.sh hazard above). It runs ckpt-tag.sh / sync-origin.sh / git-lock-push.sh itself as subprocesses the proxy never sees, so only this one basename is needed. Fail-open by design at the CALLER: a refused hop just leaves the orphan parked, never a blocked round.

...
- "review": commits_since_ckpt is NON-EMPTY (commits exist after the last relay checkpoint tag). Unaudited work always wins over other verdicts. EXEMPTION (id:bae5 — the zkm uv.lock cascade): if lock_only_unaudited is true, the ONLY unaudited commits are mechanical uv.lock relocks (a parent-version-pin refresh, zero code/spec change) — treat them as AUDIT-EXEMPT, i.e. as if commits_since_ckpt were empty: do NOT emit "review" for them, fall through to execute/hard/handoff/idle. (A strong review of a 1-line lockfile pin is pure waste; the pre-push guard already keeps locks honest.) Any unaudited commit touching a non-uv.lock path makes lock_only_unaudited false → normal review.
- "execute": commits_since_ckpt is empty, and roadmap has >=1 unticked "- [ ]" EXECUTOR-ACTIONABLE item tagged [ROUTINE] (see the EXECUTOR-ACTIONABLE test below).
EXECUTOR-ACTIONABLE test (id:e107 — never dispatch the executor at a [ROUTINE] item only a human can do; like the EXECUTABLE-HARD guard, re-dispatching it every round is pure waste — and worse, a no-op execute still writes a checkpoint commit, which changes the repo signature and defeats the discover cache, so it RE-FIRES next round = an executor-lane THRASH loop, observed 2026-06-22 on yinyang-puzzle for ~9 no-op rounds). A [ROUTINE] item is NON-actionable (@manual / human-only) — EXCLUDE it from the execute verdict and from openRoutine — when it is tagged "@manual" OR its text plainly requires a human (visually verify / eyeball, build/launch/fill a survey, get supplier quotes, run-on-device, physical/manual check). A repo whose ONLY open [ROUTINE] items are all @manual/human-only is NOT "execute": put it in "surfaced" with reason "ROUTINE backlog is all @manual/human-only — needs a human, not the executor (items: <ids>); not dispatched (id:e107)". This moves the human-only detection to cheap discovery instead of spawning a Sonnet executor every round to re-derive the same no-op.
- "hard": commits_since_ckpt is empty, NO open [ROUTINE] item, but roadmap has >=1 unticked "- [ ]" EXECUTABLE [HARD — pool] item (see the EXECUTABLE-HARD test below). This is the ROUTINE-drained steady state where HARD work would otherwise stall.
...
EXECUTABLE-HARD test (id:2d20 — never dispatch an un-doable HARD item; a relay child can only refuse it, and re-dispatching it every round is pure waste). An unticked "- [ ]" [HARD item counts as EXECUTABLE only if a strong child could plausibly finish it green in one turn. It is NON-executable (GATED) — EXCLUDE it from the hard verdict and from openHard — when ANY of:
  • it is tagged "[HARD — decision gate]" (vs "[HARD — pool]");
...
INTENSIVE-EMIT-GUARD (id:ad74 — SYMMETRIC PROMOTE counterpart to the id:000d DEMOTE guard): an open \`- [ ]\` item carrying an \`[INTENSIVE — <resource>]\` modifier that is OTHERWISE EXECUTOR-ACTIONABLE is executor-eligible WORK — the repo MUST emit a \`units\` entry with \`intensive\` set (parse the resource per the id:8d52 rule, or from the gather JSON's \`top_intensive\` field). Such a repo MUST NOT be classified \`idle\` or put in \`skipped\` on the strength of any repo-local "no executor-eligible work" convention prose (that prose was written for remote executors with no GPU access — it must never override the shard's emit). The auto-run-vs-defer decision stays with relay-loop.js (the existing INTENSIVE partition gates dispatch behind \`--allow-intensive\`); the shard's job is ONLY to emit the unit. HUMAN-GATED CARVE-OUT (id:a707 — [INTENSIVE] is an ORTHOGONAL resource axis, NOT an executor-eligibility override): an [INTENSIVE] item that is ALSO human-gated — it carries \`[HARD — hands]\` / \`[HARD — meeting]\` / \`[HARD — decision gate]\`, is \`@manual\`, or otherwise fails the EXECUTOR-ACTIONABLE / EXECUTABLE-HARD tests above — is HUMAN work that merely happens to load a model; it MUST NOT be force-emitted as an auto-dispatchable intensive unit even under --allow-intensive (a relay child can only refuse it — observed 2026-06-23: a zomni \`[HARD — hands] [INTENSIVE — local-llm]\` GGUF-cleanup item was dispatched and could not complete: it needs live GPU/sudo). The deterministic gather \`top_intensive\` field already EXCLUDES human-gated intensive items (it is "" when the only open [INTENSIVE] items are human-gated), so trust it: when \`top_intensive\` is "" do NOT force an intensive unit — surface the human-gated intensive item via the normal gated-HARD (id:2d20) / @manual (id:e107) path instead. Note: relay-loop.js also enforces this guard JS-side after shard results are merged, so a shard that ignores this instruction will still be self-corrected.

Order of precedence (apply the FIRST that matches): review > execute(routine) > hard > handoff > idle.

A repo with a DIRTY main working tree (dirty=true / porcelain non-empty, ignoring entries already declared acceptable in the toml_block comments) is NOT dispatched: put it in "surfaced" with the reason (cite the porcelain) instead of "units". EXEMPTION (id:bae5 — the zkm uv.lock cascade): if dirty_lock_only is true, the ONLY dirty path is a modified uv.lock (a mechanical relock). Do NOT surface/block. Instead COMMIT it in place first — \`git -C <repo path> add uv.lock && git -C <repo path> commit -m "chore: refresh uv.lock — cascade relock (id:bae5)"\` (a bounded, named git op, like the ff-merge guard below) — which makes the tree clean and turns the change into an audit-exempt committed lock; THEN classify normally from the now-clean state (it falls through to execute/hard/handoff/idle, never "review"). This avoids both a worktree-merge conflict over an uncommitted lock and a re-dispatch loop. Any other dirty path makes dirty_lock_only false → normal block. Repos with no relay checkpoint tag (latest_ckpt empty) and no handoff_date in toml_block are handoff candidates, not review.

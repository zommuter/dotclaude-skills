...
3. **Workflow launch.** The front door invokes the `relay/scripts/relay-loop.js`
   ...
   `args.EXECUTE_AGENT_TYPE` (id:c3c1 -- ONLY when set via the `EXECUTE_AGENT_TYPE` env var /
   `--execute-agent-type` flag; omit it entirely otherwise, since unset means OFF and the
   `execute` lane then dispatches on the default agent exactly as before),
...
| `EXECUTE_AGENT_TYPE` / `--execute-agent-type <name>` | agent-type name | (unset = OFF) | id:c3c1 -- dispatch pool **`execute` (Sonnet)** children under a NAMED CUSTOM AGENT TYPE (a `~/.claude/agents/<name>.md` definition, repo-managed via `AGENT_FILES` + `make install-agents`), so the child gets that definition's trimmed system prompt + narrowed tool list instead of the ~82k default delegated-subagent preamble (measured 2026-09-08: 46% of the ~176.7k Sonnet wall; 5 of 15 execute children died `Prompt is too long` in one run, all Sonnet, 0 of 12 Opus). Passed as `args.EXECUTE_AGENT_TYPE`. **Unset/empty is a strict no-op:** `opts.agentType` is never set and dispatch is identical to before. That default is deliberate, because a custom definition REPLACES the system prompt and cutting the wrong half degrades agent quality silently. **`execute` ONLY** -- `review`/`handoff`/`hard` run Opus, are not dying, and are out of scope. **FAILS LOUD:** if the name is configured but the harness rejects it (`Agent type '<name>' not found`), the unit is handed back naming the remedy (`make install-agents`, then **restart the session**, since a definition installed after a session started is invisible to it) and is **NOT** silently retried on the default agent. |

...
3. **Workflow launch.** The front door invokes the `relay/scripts/relay-loop.js`
   ...
   HARD-execute gate paragraph below. ALSO set it for `--intensive`/`--allow-intensive`, which
   implies `--afk` — id:84c5. The loop enforces that implication itself (`AFK` is
   `!!A.afk || ALLOW_INTENSIVE`), so forgetting it here cannot wedge `hard` dispatch; pass it
   anyway so the args reflect the run's real posture),
   `args.EXECUTE_AGENT_TYPE` (id:c3c1; **default flipped to `relay-implementer` 2026-09-12 by
   owner decision, id:5e5a** -- OMIT it and the `execute` lane now dispatches under
   `relay-implementer`. The superseded text read *"ONLY when set ... omit it entirely otherwise,
   since unset means OFF"*. **UNSET and EXPLICIT-BLANK are no longer the same thing**: omit =
   take the default, pass an explicit `''` = opt out. To restore the old behaviour for one run,
   pass the empty string; do not simply leave the flag off),
   `args.autoIntegrateOrphans` (id:1048 -- ONLY when set via the `AUTO_INTEGRATE_ORPHANS` env
   var / `--auto-integrate-orphans` flag; omit it entirely otherwise. Unset means OFF and the
   pool never auto-merges a parked orphan to main, exactly as before. This grants the pool an
   autonomous merge-to-main capability, so it is a deliberate per-run act, never a default),
...
| `EXECUTE_AGENT_TYPE` / `--execute-agent-type <name>` | agent-type name | **`relay-implementer`** (was OFF; flipped 2026-09-12, id:5e5a) | id:c3c1 -- dispatch pool **`execute` (Sonnet)** children under a NAMED CUSTOM AGENT TYPE (a `~/.claude/agents/<name>.md` definition, repo-managed via `AGENT_FILES` + `make install-agents`), so the child gets that definition's trimmed system prompt + narrowed tool list instead of the ~82k default delegated-subagent preamble (measured 2026-09-08: 46% of the ~176.7k Sonnet wall; 5 of 15 execute children died `Prompt is too long` in one run, all Sonnet, 0 of 12 Opus). Passed as `args.EXECUTE_AGENT_TYPE`. **An EXPLICIT BLANK is a strict no-op:** `opts.agentType` is never set and dispatch is identical to pre-id:c3c1. **Unset is NOT blank any more** -- it takes the `relay-implementer` default (owner decision 2026-09-12, on three recorded silent non-adoptions plus pooled arms of 2/9 deaths with the trim against 14/22 without; `id:3846`). The original warning STANDS and is not withdrawn: a custom definition REPLACES the system prompt and cutting the wrong half degrades agent quality silently. **New exposure:** dispatch FAILS LOUD on a missing type, so a checkout that has not run `make install` now hands back EVERY execute unit, not just opt-in ones. Never fix that with a silent fallback (id:4347). **`execute` ONLY** -- `review`/`handoff`/`hard` run Opus, are not dying, and are out of scope. **FAILS LOUD:** if the name is configured but the harness rejects it (`Agent type '<name>' not found`), the unit is handed back naming the remedy (`make install-agents`, then **restart the session**, since a definition installed after a session started is invisible to it) and is **NOT** silently retried on the default agent. |
| `AUTO_INTEGRATE_ORPHANS` / `--auto-integrate-orphans` | flag (`1`/`true`/`yes`/`on`) | **(unset = OFF)** | id:1048 -- permit the pool to AUTO-CONSUME a parked `relay/orphan/*` branch by merging it to main, via the host primitive `scripts/auto-integrate-orphan.sh` (which was built, tested and green but referenced ZERO times from `relay-loop.js`, so the "reconcile-first (integrate-if-safe)" half of meeting `2026-07-23-1735` A1/D1 was a null op). Passed as `args.autoIntegrateOrphans`. **Unset/empty/false is a STRICT no-op:** no hop is dispatched, no field is added to the discovery object, and the round is shaped exactly as before. **The default is a standing OWNER decision, not a preference** -- `a4e9-D1` ruled "NO auto-integration" and `id:1048` amends it only to a hard-bounded form; enabling it grants the pool an autonomous merge-to-main capability, so it must be an explicit per-run act. **The gates are the SCRIPT's:** per orphan it proves the bound item ticked `[x]` on the orphan (or unbindable, meeting A3), main not diverged from origin, a conflict-free 3-way merge, and the FULL suite GREEN in a throwaway scratch worktree POST-merge; ANY failure leaves the orphan parked, main unchanged, and surfaces for a human `/relay reconcile`. **FAIL-OPEN and LOUD:** a throw, a `MECH-ERROR`, an unresolvable repo path or missing success marker all log WHY and the round continues unchanged -- this can never be a reason a round cannot proceed. Runs once per round, right after discovery and before any dispatch, over that round's orphan-suppress `surfaced` lines (the only place a parked branch is named); a consumed orphan frees its item on the NEXT round's fresh discovery. |

...
- [ ] [HARD] **Dispatch must NAME the item instead of handing over the whole ROADMAP** (promoted from TODO `routed:79dc`) — **RE-LANED [ROUTINE]→[HARD] 2026-07-28**: the executor survey sized it out, and the reason is structural, not effort — it threads a new field through `classify-repo.sh` → `discover-repo.sh` → the STRICT `DISCOVER_SCHEMA` → `unitPrompt()` in `relay-loop.js`. A multi-file schema change with real blast radius against ~316 tests is apex work, not executor work. **The naming slot ALREADY EXISTS** — `unitPrompt` (~`:1755`) handles `unit.inject_item` ("Work specifically the ROADMAP.md item tagged <!-- id:XXXX -->"), so the build is mostly plumbing `own_id` (in scope at `classify-repo.sh:163`, DISCARDED at `:201`) into that existing slot rather than inventing a mechanism. <!-- children-of:93cc --> <!-- id:b09e -->
...
- [ ] [ROUTINE] **`repeatHandbacks` (id:1432) misreads QUEUE EXHAUSTION as a bug signal** (observed loderite 2026-07-28) <!-- id:3906 -->

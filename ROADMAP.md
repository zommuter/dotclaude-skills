...
- [ ] [ROUTINE] **257 ledger items have a TITLE that is itself over budget -- no relocation can shorten a title, so these need rewriting.** -- detail: `docs/ledger-notes/64f9.md` -- REOPENED 2026-09-08, see note. <!-- gated-on:ff7c,2654 --> <!-- relates:2d17 --> <!-- id:64f9 -->
...
- [x] **[ROUTINE]** `roadmap-lint.sh`'s detail-pointer extractor returns TWO paths for a line that names its note twice, so a present note is reported MISSING and its acceptance clause is never read (review run relay-20260908-174448-4421) <!-- relates:e95b --> <!-- id:78e6 -->

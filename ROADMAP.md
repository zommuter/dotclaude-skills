...
- [ ] **[ROUTINE]** **Route the integrate hop's FREE-TEXT fields (`--summary`/`--label`) through the `relay-mech-stdin` payload channel instead of inline shell args** (owner decision 2026-08-26 "sanitize now, migrate durably" — this is the DURABLE half; the sanitize half landed as `17b4e4e0`). Promoted at review 2026-08-26b (mini-handoff, review.md §5b) reusing the existing TODO token. <!-- id:76fd -->
  - **Acceptance**: the integrate mechanical hop carries its free-text prose as stdin DATA, not as a shell argument. Concretely: `integrate.sh` is a member of `STDIN_ALLOWED_SCRIPTS` in `relay/scripts/mechanical-proxy.py`, and `relay-loop.js`'s integrate hop emits the ```relay-mech-stdin fence (the `relay-status-publish.sh` pattern) instead of passing `--summary` through `mechArg()` inline. A markdown-backticked executor summary then reaches the integrator VERBATIM rather than sanitised-and-flattened. **Scope (a) is a precondition, not an option**: confirm — do not assume — that `integrate.sh` consumes stdin as INERT DATA (never `eval`/`source`/shell-interpolated); that is the standing obligation on every `STDIN_ALLOWED_SCRIPTS` member and the reason admission is a deliberate act.
  - **Tests**: `tests/test_integrate_stdin_channel_76fd.sh` (`# roadmap:76fd`) — currently RED on (1) membership and (2) the inline `--summary`. Assertions (3)/(3b)/(4) are GREEN REGRESSION-GUARDS and must STAY green: (3) a stdin payload containing `$(...)` and backticks must not execute, (4) `mechArg()` must KEEP its sanitisation as defence-in-depth so a future caller that forgets the channel degrades to cosmetic loss instead of a silent 404.
  - **Done-check**: `tests/run-tests.sh tests/test_integrate_stdin_channel_76fd.sh` green, then full `make test` after the integrator ticks.
  - **Context**: `relay/scripts/mechanical-proxy.py:435` (`STDIN_ALLOWED_SCRIPTS`, with the admission hazard documented in the comment directly above it), `relay/scripts/relay-loop.js:3458-3479` (the `integrateArgs` build + the ```relay-mech fence), `:1694` (`mechArg`), `:653` (an existing stdin-fence emission to copy), `relay/scripts/relay-status-publish.sh:17,55` (the reference member's stdin contract). **DO NOT loosen `_command_allowed()`** — that was attempted 2026-08-26 and REVERTED; it breaks `tests/test_mech_stdin_channel_33b2.sh`, and the paranoia is the contract, not the defect. TODO twin: `TODO.md` id:76fd (single-id-two-views).
...
- [x] **[ROUTINE]** **Base-ref resolution must use the repo's ACTUAL checked-out branch — never a hard-coded `origin/main` → `main` → `master` guess** (owner ruling 2026-08-26; PROPAGATION of the already-ratified id:8739 posture to the two remaining offenders, not a new design) <!-- id:758a --> — CLOSED at review 2026-08-26 (run `relay-20260826-162405-7522`): verified green against the pre-authored RED spec `tests/test_base_ref_checked_out_branch_758a.sh`, which the fix commit `478d70d2` did NOT touch (independent spec); TODO twin already `[x]`, cross-ledger drift resolved.
  - **Acceptance**: `relay/scripts/stranded-branch-scan.sh` and `relay/scripts/verify-isolation.sh`, called with NO `--base` on a repo whose checked-out branch is neither `main` nor `master` (the git-annex shape: real work on `annex-dotgit`, a STALE `master` mirror, no `origin/HEAD`), resolve the base from `git symbolic-ref --short HEAD` and run — they no longer exit 2 with "base ref 'main' does not resolve". FAIL-CLOSED is preserved per id:8739: an undeterminable base (detached HEAD) still exits 2 LOUDLY and MUST NOT silently fall back to the stale `master` that happens to resolve.
  - **Tests**: `tests/test_base_ref_checked_out_branch_758a.sh` (`# roadmap:758a`) (currently RED — case (a) reproduces the live `integrate:git-annex` failure verbatim)
  - **Done-check**: `tests/run-tests.sh tests/test_base_ref_checked_out_branch_758a.sh` green, then full `make test` after the integrator ticks
  - **Context**: offenders are `relay/scripts/stranded-branch-scan.sh:68-74` (origin/main → hard-coded `main` → hard `exit 2`; this is the one that FIRED, permanently fatal for git-annex on every future pool run) and `relay/scripts/verify-isolation.sh:78-91` (origin/main → origin/HEAD → hard-coded `main`; masked in practice only because `integrate.sh` always passes `--base` per id:8739, so any DIRECT caller still hits it). Reference implementation to copy: `relay/scripts/integrate.sh:455-465`. Blast radius verified 2026-08-26: git-annex is the ONLY one of 62 own repos with no resolvable `main`. Also in scope: the misleading handback text — `RELAY_STATUS.md` recorded the downstream symptom ("integrate.sh produced no merged= line") instead of the real cause. TODO twin: `TODO.md` id:758a (single-id-two-views).

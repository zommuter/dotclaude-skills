...
- [ ] [ROUTINE] **`unpromoted-scan.sh`'s `primary_lane` path 3 reads a lane tag from PROSE, labelling an UNLANED item `promote` — violating the script's own acceptance #3** <!-- id:6b1c -->
  ...
  - **Tests**: `tests/test_unpromoted_scan_prose_lane_6b1c.sh` (`# roadmap:6b1c`) — hermetic, `mktemp -d` fixture repos. **Do NOT assume `tests/test_unpromoted_scan_anchoring.sh` (roadmap:1312) covers this**: it guards a DIFFERENT anchoring defect (the twin-check `grep -qF "id:$token"`). Extend it or add a sibling, deliberately.

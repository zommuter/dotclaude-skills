...
- [ ] [HARD — pool] **`@wire` grammar + `classify-repo` count + `drained` render-alias** (KEYSTONE — build first; af48 child C1) <!-- children-of:af48 --><!-- id:ac7f --> — define `@wire` as a new **orthogonal marker** (like `@manual`/`@needs-auth`, NOT a lane), recorded by the *executor-verifiable-via-a-host/e2e-RED-spec* property (NOT narrowly "UI wiring"). Three deliverables, all pinned by `tests/test_wire_grammar_classify.sh`:

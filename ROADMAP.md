...
- [x] [HARD — pool] **PREREQUISITE FINDING (F2) — determine which layer blocked the two hardcoded `model:'bash'` `release:` hops** <!-- children-of:51f0 --> <!-- id:e62c --> — DONE 2026-08-11 (relay HARD child `relay-20260811-144639-28608-hard`): verdict = **branch (1)**, already written in-item (run `relay-20260729-111723-7520`, `release:dotclaude-skills:claim` hop hardcoded `model:'bash'` at `:2316`, proxy verified in path, blocked by the safety classifier anyway). Done-check completed: the branch-(1) rationale correction was threaded into the id:6b35 block (classifier-exposure struck as a driver; F2 marked RESOLVED) and the id:51f0 D3-A block (F2 resolved, D1-A vindicated). No RED spec by design (live-API/harness question, id:108e honesty). Build decisions for id:540f/c179 remain the owner's call via their `gated-on:b0b1` owner gate.

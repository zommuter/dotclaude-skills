...
- [ ] [ROUTINE] **EVERY terminal `stopReason` must ASSERT no actionable work remains — `drained` AND `blocked-pending-human`** — RED spec authored 2026-07-28 (handoff C3), NOT implemented; **WIDENED 2026-07-28 by `routed:b874`** <!-- children-of:c919 --> <!-- id:d6f0 -->
...
- [ ] [ROUTINE] **FLAKY: `tests/test_redispatch_suppression_e3b7.sh` fails nondeterministically — TRIGGER MET (n≥2), promote from observation to FIX** <!-- id:98ea -->
...
- [ ] [ROUTINE] **Diagram EDGE-coverage guard — every `relay-dispatch.mmd` edge must name its enforcer** — RED spec authored, NOT implemented <!-- id:a225 -->
...
- [ ] [INPUT — meeting] **Extend the state model to ROUND OUTCOMES (the id:4da4 matrix's missing axis)** 🚧 GATED (DEP: id:a225) <!-- gated-on:a225 --> <!-- id:5f31 -->
...
- [ ] [ROUTINE] **BUG: a context-death worktree is never PARKED, so its completed work is unreachable next run and silently reapable** (promoted from TODO `routed:3f22`, loderite 2026-07-28) <!-- children-of:61fa --> <!-- id:4df8 -->
...
- [ ] [ROUTINE] **`claim.sh`: add a `release --run <runId>` sweep verb** (D1-A) <!-- children-of:51f0 --> <!-- id:89d6 -->
...
- [ ] [ROUTINE] **Front door: EXIT-ONLY teardown trap + mode-b `--afk` prose fix** (D1-A) <!-- children-of:51f0 --> <!-- id:54be -->
  ...
  - **Tests**: `tests/test_front_door_exit_teardown_54be.sh` (`# roadmap:54be`) — hermetic. Two halves: (a) a **behavioural** half that extracts the documented trap shape and exercises it against the real `claim.sh` under a `mktemp -d` `CLAIM_BASE`, with a stub "Workflow" that returns and a second that dies; (b) a **prose/lint** half over `relay/SKILL.md` asserting the no-swallow and two-action properties and the mode-b rewrite. Currently RED (no teardown trap is documented at all today).

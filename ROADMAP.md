...
- [ ] [ROUTINE] 🔴 **`provisionWorktree()` FAILS OPEN: it discards the mechanical hop's return value and only catches a `throw`, so a refused/errored provision dispatches a child into a repo with no worktree** <!-- children-of:f91a --> <!-- routed:8d64 --> <!-- id:66d9 -->
...
- [ ] [ROUTINE] **Dispatch bookkeeping fires BEFORE provisioning, so a provisioning handback leaves a spurious `dispatch` event and a stale `inFlight` row** <!-- routed:8d64 --> <!-- id:ec8a -->
...
- [ ] [ROUTINE] **`id:34b7` part (3) is INCOMPLETE — review children are still handed the target's MAIN checkout path** <!-- children-of:f91a --> <!-- id:ba7e -->
...
- [ ] [ROUTINE] **`RELAY_STATUS.md` has NO representation for a failed agent/hop, so an `--afk` operator cannot see per-agent failures at all** <!-- routed:d9a5 --> <!-- id:06a1 -->

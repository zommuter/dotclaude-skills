...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract — RESCOPED 2026-07-28 after an adversarial review refuted this item's own fix direction and its RED spec** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  ...
  - **⚠️ UNRESOLVED DISCREPANCY — do not silently pick a number.** Two measurements from the SAME source (loderite) disagree: the original `routed:d2a1` report gave the Skill load as **+26,674 / +26,425 tok**, while the 2026-07-28 direct transcript read of entry [10] gives an injected payload of **63,220 chars ≈ 15,805 tok** (+ ~1.7k skill_listing ≈ **17.5k**). Gap ≈ **9k, unexplained**. Plausible causes to check, NOT to assume: context-growth accounting vs raw payload size (cache-creation tokens counted separately), a second attachment beyond skill_listing, or a different measurement point in the turn. **The item must resolve this before quoting a saving** — the direct payload read is the better-grounded number and should be preferred if the two cannot be reconciled, but say which was used and why.

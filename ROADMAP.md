...
- [ ] [ROUTINE] **A regrowth BELOW the baselined value is forgiven -- the `id:718c` shape the snapshot ratchet structurally cannot catch.** -- detail: `docs/ledger-notes/cf64.md` <!-- children-of:2d17 --> <!-- gated-on:2654 --> <!-- relates:2d17 --> <!-- id:cf64 -->
...
- [ ] [ROUTINE] **`grammar_item_class` counts the `-- detail:` pointer as title while `shape-prose` excludes it, so planting a REQUIRED pointer pushes an item over budget.** -- detail: `docs/ledger-notes/60eb.md` <!-- relates:2d17 --> <!-- id:60eb -->
...
- [ ] [ROUTINE] **The length ratchet cannot detect its own STALE baseline, so a shrunk item silently keeps its old floor and a regrowth reads as grandfathered.** -- detail: `docs/ledger-notes/2654.md` <!-- relates:2d17 --> <!-- id:2654 -->

...
- [x] [INPUT — decision] **Pre-register the burn-ranking decision rule — THE GATE that orders id:a955 and id:3ca7** <!-- children-of:1f4f --> <!-- id:87f5 --> — **PRE-REGISTRATION MADE 2026-08-18 (`/relay human .`, OWNER-DECIDED) — this item's human half is DELIVERED, so it is TICKED.** The rule, fixed BEFORE any number is looked at: **rank levers by their target phase's share of PARALLELITY-WEIGHTED WALL-CLOCK** (`relay-econ.py`'s third lens — per-category UNION of `[start,end]`), **not by cost**; **n = all retained runs**; **promote a lever iff its phase clears ≥25% of that share**; **order = descending share**. Per-phase attribution stays REQUIRED (a whole-run total-burn ranking provably cannot order levers targeting different phases). **Why wall-clock over cost** (the owner's reasoning, recorded so `id:4438` cannot quietly substitute the easier axis): the banked `relay-econ.py` finding is that status+scaffold are a bigger share of WALL-CLOCK than of COST — i.e. they run at low concurrency and sit on the critical path. Ranking by cost would systematically under-weight exactly the phases worth mechanizing. The accepted trade-off is explicit: a lever may be promoted that saves time but little money. **Reconciliation against the banked 47.6% discover baseline is still REQUIRED of `id:4438`** — in numbers, agreeing or disagreeing, never ignored. **GATE TOPOLOGY CORRECTED in the same pass:** `id:a955` and `id:3ca7` were `gated-on:87f5`, which under the 2026-08-14 split would have released BOTH into dispatch the moment this pre-registration landed — before any ranking exists, which is the precise outcome the gate was filed to prevent. Both are re-targeted to `gated-on:4438` (the item that actually PUBLISHES the ranking). This is a mechanical consequence of the owner's already-ratified split, not a new decision. **The `Contract (unchanged)` bullet above is STALE** — "a published per-phase ranking that ORDERS a955 and 3ca7" is now `id:4438`'s contract, not this item's; this item's contract was the pre-registration alone.
  ...
  - **Lane**: `[INPUT — decision]` as of the 2026-08-14 owner re-lane. ~~`[HARD — hands]`~~ — **this bullet previously asserted the hands lane was "the tag of record", corrected here rather than left to contradict the head tag.** The 2026-07-31 note reasoned from the whole item needing a human; the 2026-08-14 ruling separated the two halves, and only the pre-registration is human. A `REVIEW_ME` box had earlier called it "`[INPUT — access]`-class", which was also wrong — it is a decision, not an access problem.
...
- [x] [ROUTINE] **Make `@owner-gated` a FIRST-CLASS exclusion in `lib-roadmap-sections.sh` — the PREREQUISITE for `id:6446`** — DONE 2026-08-14 — covered by tests/test_owner_gated_first_class_f391.sh (4 fail -> 10 pass). @owner-gated is now a separately-defined, standalone-anchored marker composed into the parked-heading vocab, so the id:6446 anchoring can land WITHOUT un-protecting owner-gated work; the test includes a faithful id:6446 stand-in mutation proving exactly that. id:6446's gate is now RESOLVABLE. The parked-vocab ANCHORING item stays gated and untouched. <!-- id:f391 -->
  - Promoted 2026-08-14 (`/relay human .`) together with the `id:6446` gate, so that gate is mechanically resolvable — a `gated-on:` pointing at a TODO-only item can NEVER clear from the execution queue (the DEAD-GATE class `roadmap-lint` flags, and exactly why `id:d4ca`/`id:e405` warn today). Filing the gate without promoting its target would have reproduced that defect while I was closing a box about it.
  - **The defect and the protection are the same mechanism.** `relay/scripts/lib-roadmap-sections.sh:42` sets `ROADMAP_PARKED_HEADING_VOCAB='(gated|deferred|done|icebox|archive|parked)'` and applies it unanchored (`:65`). A heading containing the literal `@owner-gated` is parked ONLY because that string contains `gated` — verified 2026-08-14: `@owner-gated` appears nowhere in that file. Loderite reports three open items (`f303`, `d385`, `6e7a`) relying on the coincidence. Anchoring the vocab — which IS `id:6446` — removes owner-gate protection in the same edit.
  - **Acceptance**: `@owner-gated` is a named, explicit exclusion (not a substring coincidence), with a fixture proving a section headed `@owner-gated` stays parked **both before and after** the `id:6446` anchoring change. The before-and-after property is the point — a fixture that passes only after is not evidence the gap was closed.
  - **Also answer in the same pass**: `routed:34a2`'s open question (folded into `id:7517`) — should `@owner-gated` join `gather-repo-state.sh`'s `open_hard_pool` exclusion list? One token, two consumers; a first-class definition is what lets them agree.
  - **Out of scope**: the anchoring change itself (`id:6446`, gated on this), and loderite's twin `id:920b`.
...
- [ ] [ROUTINE] **Parallelise `tests/run-tests.sh` — MEASURE the speedup first; do not assume it** <!-- id:f69b -->
...
- [ ] [ROUTINE] **Make the fast subset first-class: `make test FILES=…` / `make test-changed`** <!-- id:d3f8 -->
  - `tests/run-tests.sh` ALREADY accepts explicit file arguments (`tests/run-tests.sh tests/test_foo.sh`), and `CLAUDE.md` documents it — but there is no `make` front door, so the reflex is `make test` and the reflex costs 10-12 minutes even for a change that touches no code. Observed today: a ledger-only commit (three `.md` files) triggered three full-suite runs, where the correct check was a handful of files.
  - **Deliberately independent of `id:f69b`** — this one needs no measurement and no concurrency reasoning, so it should not wait behind the parallelism question. It is the cheaper half of the same problem and is worth landing on its own.
  - **Acceptance**: `make test FILES="tests/test_a.sh tests/test_b.sh"` runs exactly those files with unchanged PASS/FAIL/EXPECTED-RED semantics; `make test` with no `FILES` is byte-identical in behaviour to today. A `test-changed` target deriving the set from `git diff --name-only` is desirable but SECONDARY — get `FILES=` in first, since the derivation rule (which tests cover which sources) is a judgement call and mis-deriving it silently narrows coverage, which is worse than being slow.
  - **Do NOT let this become a default.** The full suite stays the definition-of-done gate (`CLAUDE.md`: tick the ROADMAP box, then `make test` must be fully green). This target is for the inner loop only — say so in the Makefile help text so it is not mistaken for the release check.

## Session-derived fix 2026-08-14 (/relay human, owner-directed)

- [x] [ROUTINE] **`relay-doctor` check 9: distinguish ABANDONED main-checkout residue from IN-FLIGHT residue via live-claim state** <!-- id:aee5 -->
  - **Why**: a loderite `/meeting` wrote `meeting/personas.md` in this repo and ended WITHOUT committing. It sat for hours. Detection was NOT missing — check 9 already reported `RESIDUE — N uncommitted non-lock entry(ies)`. What was missing is the discriminator that makes the check safe to run ROUTINELY: residue under a live run is normal mid-run state, residue with no live holder is abandoned and will never be committed. Reporting both identically is why the check was only ever run on demand, which is why this residue went unseen — the `id:4347` shape (a correct detector nobody can afford to route).
  - **Cost it caused, measured**: the `id:aa93` dirty guard handed back `review-repo-1` of `relay-20260814-101319-22034` naming exactly that file, and `git-lock-push.sh` refused to rebase, forcing a manual fast-forward push.
  - **Mechanism**: `claim.sh peek` is the oracle and needed no new machinery — it emits ONLY live claims (dead skipped, `id:7570`), so a claim keyed to the repo means a live run holds it. Overridable via `RELAY_DOCTOR_CLAIM_SH` for hermetic tests.
  - **Issue-counting contract (deliberate)**: only ABANDONED residue increments `repo_issues`; IN-FLIGHT is reported but does NOT, so `--strict` stops false-alarming on healthy concurrent work. This is the change that makes routine invocation viable.
  - **Tests**: `tests/test_doctor_residue_liveness_aee5.sh` (`# roadmap:aee5`) — 5 assertions incl. a negative control that identical dirt yields different labels purely from claim state. RED-verified before the fix. Also re-derived against the ORIGINAL incident shape (a dirty `meeting/personas.md`) using the REAL `claim.sh`, not only the stub.
  - **NOT done here (routing, the other half)**: nothing yet RUNS this on a cadence. That is the same gap as `routed:057f` / `routed:982e` (wire detector output into a channel a human reads) and is deliberately left to that cluster rather than bolted on.
## Inbox drain safety 2026-08-14 (self-inflicted data loss, `/relay human` follow-up)

- [x] [HARD] **`scan-routed.sh --apply` silently DELETED inbox items whose token was merely quoted in a sibling item's prose — the twin check now requires the token to be a line's OWN marker** <!-- id:c97c -->
  - **Predicate chosen**: a token counts as present in a target ledger only as (1) an `<!-- routed:XXXX -->` / `<!-- id:XXXX -->` HTML-comment marker, or (2) the leading `[INBOUND routed:XXXX …]` bracket tag of a checkbox line (the ingest stub `--apply` itself writes; its own HTML comment carries the freshly minted TARGET id, not the routed token, and 176 already-filed items in `TODO.md` carry no other marker). A bare `routed:`/`id:` token in running prose never counts. The old predicate anchored only the `routed:`/`id:` PREFIX plus a non-hex boundary, so a backticked citation satisfied it.
  - **Where**: the fix is in the SHARED primitive `relay/scripts/lib-anchored-id.sh` (`token_marker_in_files` / `token_marker_in_text`), and `scan-routed.sh` was migrated onto it (it hand-rolled its own copy). This deliberately fixes `append.sh inbox-done`'s `id:9fdb` refusal guard in the same stroke — both must ask the same question, or a successful stub write is followed by a refused drain.
  - **Per-iteration fresh read KEPT**: `grep` re-opens the target on every loop iteration, which is how the intra-run stub write became visible and destructive. Case 4 asserts a second `--apply` is still a no-op; with an ownership-anchored predicate the freshness is a feature (it sees concurrent writes) rather than the hazard.
  - **Tests**: `tests/test_scan_routed_own_marker_twin_c97c.sh` (`# roadmap:c97c`) — 4 cases. RED-verified against the unmodified tree: case 1 reproduced the live loss verbatim (`RESOLVED routed:b2b2 → [depot] (twin present …; removed from inbox)` after the sibling's stub was written), and a soft-fail copy confirmed cases 1/2a/2b all FAIL pre-fix while the no-regression cases 3/4 pass in both directions.
  - **Done-check**: `tests/run-tests.sh` fully green (425 passed, 0 failed, 2 expected-red).

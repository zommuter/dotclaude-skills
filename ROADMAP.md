...
- [ ] [ROUTINE] **`write-relay-status`: haiku → `model:'bash'`** 🚧 GATED (DEP: id:a05c — needs the stdin channel first) <!-- children-of:6b35 --> <!-- gated-on:a05c --> <!-- id:d4ca -->
  ...
  - **Unblocks when** id:a05c lands; then the change is: fence carries the bare one-liner, payload moves to the stdin channel, `model: 'bash'`.
...
- [ ] [INPUT — decision] **Proxy stdin channel — separate the data plane from the command plane** (owner's call; deliberately NOT pool work — this widens a security-sensitive trust boundary) <!-- children-of:176f --> <!-- id:a05c -->

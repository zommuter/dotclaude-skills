...
- [ ] **[ROUTINE]** Rewrite the first ~10 over-budget TODO.md titles (of 42) with full original preserved verbatim in docs/ledger-notes/<id>.md, verified against the new shrink-acceptance.py gate (after id:(the gate seam above)) — seam of id:64f9 (auto, id:3801) — UNBLOCKED 2026-09-09 (relay review): the gate seam it waits on is id:521b, now `- [x]` in ROADMAP.archive.md with all 8 spec cases green, so `tools/shrink-acceptance.py` is available to verify a rewrite batch. Read id:227d first -- Check 4 does not detect a title that GREW, so a batch must be checked for that by hand until 227d lands. <!-- id:b437 -->
...
- [x] [ROUTINE] **`shrink-acceptance.py` Check 4 cannot see a title that GREW: an item edited from under-budget to over-budget is ACCEPTED and misreported as `LEFT unmodified`** <!-- relates:521b --> <!-- id:227d -->
  - **Context**: `tools/shrink-acceptance.py`, `check_title_rewrites()` -- the `left` loop iterates `before_ids & after_ids` UNSCOPED, while `common`/`touched` are scoped to `before_title_long`. An item whose BEFORE title was under budget is therefore never `touched`, so it falls through to the `left` list and is emitted as `LEFT unmodified, still over the title budget` even though its line WAS modified, in the one direction a shrink gate exists to prevent.
  - **Measured 2026-09-09** (relay review, constructed fixture pair): a batch that rewrites `id:b2b2` from a 66-char title to a 280-char title exits **rc=0 (ACCEPTED)** and prints `items checked=0   touched=0   left-still-over-budget=1`. The only other signal is Check 2's count-only `WARN [verdict] todo-conformance: ... grammar-item-title-long moved None -> 1`, which its own text records as unattributable to an item.
  - **NOT a defect in id:521b's acceptance.** The `before_title_long` scoping was a correct fix for a real false positive (an unrelated prose edit read as an undocumented title rewrite). This item is the coverage hole that fix left behind, and nothing in the id:64f9 spec exercises it -- cases 0-7 all use BEFORE titles that are already over budget.
  - **Acceptance**: the reporting half is unambiguous and is what this item OWNS -- an item whose head line changed is never described as `LEFT unmodified`. A new spec case drives a before/after pair whose title grows past the budget and asserts the output does not claim the item was left unmodified.
  - **Done-check**: a new case in `tests/test_title_rewrite_batch_acceptance_64f9.sh` (or a sibling spec) that reddens against today's `tools/shrink-acceptance.py` and passes after the fix; `make test` fully green.
  - **OPEN DESIGN RESIDUE, deliberately not decided here**: whether a grown title should additionally be a FATAL **refusal** or only a WARN is a judgment call with a real blast radius (a refusal could newly reject batches that are otherwise honest). The executor must implement the reporting fix only and surface the refusal question rather than settle it.
- [ ] [ROUTINE] **A spec keyed `# roadmap:<container id>` gets a PERMANENT expected-red umbrella: when the implementing seam lands, the container stays open and the file's failures are still swallowed** <!-- relates:521b --> <!-- relates:64f9 --> <!-- id:11a4 -->

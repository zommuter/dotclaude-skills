...
- [ ] [ROUTINE] **257 ledger items have a TITLE that is itself over budget -- no relocation can shorten a title, so these need rewriting.** -- detail: `docs/ledger-notes/64f9.md` <!-- gated-on:ff7c,2654 --> <!-- relates:2d17 --> -- REOPENED 2026-09-08 by review relay-20260908-231617-32609: ticked in error after 4 of 46 titles; spec test assertion (2) still RED. <!-- id:64f9 -->
...
- [ ] [ROUTINE] **`roadmap-tick.sh` ticks every `worked_ids` id unconditionally, so an executor that honestly reports PARTIAL progress has its item closed AND archived.** Observed this run: `id:64f9` (4 of 46 titles rewritten; the executor's own RELAY_LOG paragraph says "Leaving the item open, unticked, for a follow-up batch") was ticked by the integrator and swept into `ROADMAP.archive.md`, and the repo's own definition-of-done check caught it -- `make test` went from 5 expected-red to 1 REAL failure, because `tests/test_title_rewrite_batch_acceptance_64f9.sh` stops being expected-red the moment its checkbox is ticked. Reopened by hand this review. The channel has no way to say "advanced": `relay/scripts/relay-loop.js:3202` defines `worked_ids` as the ids "you closed/advanced", and `relay/scripts/integrate.sh:803-825` feeds every one of them straight to `roadmap-tick.sh`, which flips the box. NOTE the deeper question is NOT settled here and is the owner's: whether the child return contract should grow a separate closed-vs-advanced signal. This item asks only for the cheap mechanical guard the repo already documents in `CLAUDE.md` §Testing. <!-- relates:5b12 --> <!-- relates:64f9 --> <!-- id:963c -->

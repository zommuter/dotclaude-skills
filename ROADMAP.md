...
- [ ] [ROUTINE] **`classify.sh`'s GATE detector matches the bare substring `gate`, so *investigate* / *mitigate* / *aggregate* / *delegate* / *navigate* all render `[GATED]`** — the pattern `grep -qiE 'gated?|gate:|…'` has no word boundary on `gated?`, so ordinary TODO prose containing "gate" is flagged gated in every `/meeting` bucket summary. <!-- id:259f -->

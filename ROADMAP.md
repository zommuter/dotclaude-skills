...
- [x] [ROUTINE] **The `# fails-against` exemption is FILE-scoped but the risk is CASE-scoped: a defect-fix case grafted into an already-landed `# roadmap:`-keyed spec inherits a blanket exemption and is verifiable by nothing** <!-- relates:292b --> <!-- relates:a73c --> <!-- relates:11a4 --> -- RESIDUE ALREADY ON MAIN, UNVERIFIED (breadcrumbed 2026-09-09 by review `relay-20260909-205831-5121`): a died-mid-work attempt was auto-committed as `3929d248` (id:f272 commit-and-park, "do NOT treat as reviewed") and then MERGED to main by the 20:42 reconcile via `71f51e78`, so unlike id:11a4/id:b437 this item has NO parked orphan left to read -- the partial implementation is already in your working tree. It is substantial: `tests/lint-vacuous-fixtures.py` +141/-10, carrying the whole `graft_since_close()` machinery and the "THE POST-CLOSE GRAFT CHECK" docstring. Its own spec `tests/test_lint_post_close_graft_799f.sh` (also from that commit) is cases 1 and 2 GREEN, case 3 RED: a still-OPEN roadmap item's spec is flagged when `analyse()`'s open-item exemption should have returned early, which points at fixture root-resolution in `_repo_root()`/`roadmap_item_open()` rather than at the graft logic. START BY READING THE DIFF OF `3929d248`, not from scratch. <!-- id:799f -->

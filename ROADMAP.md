...
- [ ] [ROUTINE] Relay ROADMAP archiver — `relay/scripts/roadmap-archive.sh` moves done `[x]` items out of the live ROADMAP <!-- id:6b67 -->
...
- [ ] [HARD — meeting] Auto-reconcile-on-restart for the relay loop — `.relayactive`/heartbeat marker + safe-vs-judgment orphan classifier so a restarting loop auto-handles a dead run's SAFE leftovers (clean/green/ledger-only → auto-integrate) and SURFACES the judgment ones (BLOCKED/partial/red → strong turn / `/relay human`), instead of needing a manual `/relay reconcile`. Design + rationale: **TODO id:7809** (single-id-two-views). → /meeting before building. <!-- id:7809 -->
- [ ] [HARD — meeting] Outage-resilient LOCAL relay loop without `--dangerously-skip-permissions` — the three-way bind: in-session mechanisms (`/loop`→cron, `ScheduleWakeup`, `CronCreate durable:true` is a no-op) all die with the session on a network/API outage; cloud `/schedule` survives but can't reach local `~/src`/worktrees/fievel; the only fit (OS systemd timer running `claude -p "/relay --afk"`) hits the headless permission wall the user won't bypass. Meeting to choose: broad-enough allowlist vs dedicated-OS-user scoped allowlist (id:2d01) vs sandboxed auto-approve vs accept best-effort in-session cron. Design + rationale: **TODO id:98f0**. <!-- id:98f0 -->

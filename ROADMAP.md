...
- [x] **[HARD]** Invert ROADMAP tick ownership: executors return worked_ids, driver ticks; bump executor-contract v11->v12 + CLAUDE.md pointer — seam of id:ae08 (auto, id:3801) <!-- id:5b12 --> — **DONE 2026-08-13 (hard[opus]):** executor-contract v11→v12 (rule 2 driver-ticks note, rule 5, ROADMAP-format tick note, v12 Maintenance entry); CLAUDE.md pointer + table → v12; new `relay/scripts/roadmap-tick.sh` (idempotent, flock'd, hermetic `tests/test_roadmap_tick.sh` 7/7) ticks `- [ ]`→`- [x]` by worked id; integrate path in relay-loop.js gained a driver-tick step gated to execute/hard (review/handoff self-tick in their merged worktree); hard child prompt no longer self-ticks. Asserted by `tests/test_relay_driver_ticks.sh` over BOTH contract text and integrate path. Transition-safe (idempotent double-flip). node --check + lint-workflow-templates clean; full suite green.

...
- [ ] [ROUTINE] **A spec keyed `# roadmap:<container id>` gets a PERMANENT expected-red umbrella: when the implementing seam lands, the container stays open and the file's failures are still swallowed** <!-- relates:521b --> <!-- relates:64f9 --> <!-- relates:799f --> <!-- id:11a4 -->
...
## 2026-09-09 user-injected promotion (C2 of id:5295)

- [x] [ROUTINE] **Rule 2c's budget guard is INERT in Workflow-nested children: the dispatch prompt spells the worktree with a literal `~`, the child passes `$(pwd)`, and the literal marker match can never succeed** -- detail: `docs/ledger-notes/5295.md` <!-- relates:c219 --> <!-- relates:ff30 --> <!-- relates:5eeb --> <!-- id:5295 -->
  - **LANE RE-TRIAGE (the open question in the note is now ANSWERED -- do not re-derive it)**: the note asked whether the discriminating marker must be a nonce injected by `relay-loop.js`, in which case this would not be a pure resolver change. It must NOT. The failure is a SPELLING mismatch between two strings that both already exist, so the fix lives entirely in `relay/scripts/self-transcript.sh` (plus, optionally, the wording of `relay/references/executor-contract.md`). No dispatch-prompt change, no nonce, no `relay-loop.js` edit -- which also keeps this clear of the loop-crash class. Lane therefore stays `[ROUTINE]`.
  - **Root cause, read off the code**: `worktreePathFor()` at `relay/scripts/relay-loop.js:2957` returns the template literal `` `~/.cache/relay/worktrees/${unit.repo}/${state.runId}-${unitKey(...)}` ``. Nothing expands the tilde, so the prompt rendered at `relay-loop.js:3262` ("Your worktree `${wt}` on branch ...") puts `~/.cache/relay/worktrees/...` into line 1 of the child's transcript. Rule 2c (`relay/references/executor-contract.md:81`) asks the child for "your worktree path"; a child that answers `$(pwd)` answers `/home/<user>/.cache/relay/worktrees/...`. `self-transcript.sh`'s filter is a literal `[[ "$head_bytes" == *"$marker"* ]]`, so the match is structurally impossible, the resolver exits 4, and `context-budget.sh --self` fails open to `unknown`.
  - **Reproduced live during this handoff, not inferred**: session `88e0cae7-804a-48e4-9975-ef2568e05095`, worktree `relay-20260909-185356-12943-handoff-5295-0`. `self-transcript.sh --marker "$(pwd)"` -> `matched none of the 707 transcript(s)`, exit 4. The same command with the tilde spelling -> exit 0, and it resolved the correct transcript. `grep -bo` on that transcript puts the tilde form at byte 457 (inside the 128 KiB marker scan) and the absolute form only at byte 464811, written by this session's own later tool output -- i.e. at dispatch time the absolute form was not in the file at all.
  - **Why the existing c219 fixture did not catch it**: `tests/test_self_transcript_workflow_nesting_c219.sh`'s `mk_child` writes the dispatch prompt as `Your worktree /home/x/.cache/relay/worktrees/<marker>` -- an ABSOLUTE path -- and passes the worktree BASENAME as the marker. Both halves diverge from what the dispatcher really emits, so the fixture models a shape that cannot fail this way. Fixing the fixture's fidelity is part of this item.
  - **SECOND defect on the same line, latent behind the first**: even with the spelling reconciled, a per-unit worktree path is NOT unique. The tilde-form probe above matched **two** transcripts -- the unit child and the `id:34b7` `provision-worktree.sh` mechanical child, whose own dispatch prompt names the same worktree as an argument. `self-transcript.sh`'s documented AMBIGUITY POLICY then picks the newest mtime. That happened to be right here, but a wrong pick measures somebody else's context and returns a confidently wrong verdict, which is worse than `unknown`. This item must make the multi-match case LOUD and assert it; whether it should additionally become a hard refusal is an owner call filed in `REVIEW_ME.md` -- **do not change the tie-break semantics on your own.**
  - **Acceptance**: a marker naming the worktree in EITHER spelling (`~/...` or `/home/<user>/...`) resolves the same transcript, in both the flat and the `subagents/workflows/wf_*/` nested shape; a marker matching several transcripts still names EVERY candidate on stderr (never silent, id:4347); `context-budget.sh --self` returns a real verdict (`ok`/`handback`), never `unknown`, on a fixture whose dispatch prompt carries the tilde spelling.
  - **Done-check**: `bash tests/test_self_transcript_tilde_marker_5295.sh` passes, and `make test` is fully green. The spec is hermetic -- `mktemp -d` projects tree driven through `--projects-root`/`--session-id`, Workflow-nested, with MORE THAN ONE sibling carrying the marker. **Do NOT close this on a live run that happens not to die**: `unknown` is invisible unless asserted, which is how it survived this long.
  - **The spec is verified REACHABLE, not just red**: during this handoff the ten cases of `tests/test_self_transcript_tilde_marker_5295.sh` were run against a throwaway prototype (build a `marker_variants` array from `$marker` plus its `~/`↔`$HOME/` counterpart, match if ANY variant is a substring) and all ten passed; the prototype was then reverted, and the file is red again at case 1, the assertion it names. So no case is an unreachable fixture. That shape is a hint, not a mandate -- any implementation satisfying the ten cases is fine.
  - **Context**: `relay/scripts/self-transcript.sh` (the marker filter, ~line 195), `relay/scripts/context-budget.sh` (`--self`, the fail-open branch), `relay/scripts/relay-loop.js:2957` + `:3262` (READ-ONLY here -- the tilde spelling is the input this fix must tolerate, not something to change), `relay/references/executor-contract.md:81`, `tests/test_self_transcript_workflow_nesting_c219.sh` (the fixture whose fidelity gap hid this).

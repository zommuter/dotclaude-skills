...
- [ ] [HARD — decision gate] Relay consumer of the id:2840 derived ledger index (cross-ledger / count / promotion) — **2026-07-14 BUILD-AUTHORIZATION RETRACTED 2026-07-28 (relay human, owner call):** the premise it rested on is FALSE — the shipped `edges.json` is a co-citation graph carrying no checkbox-state and no counts, so it CANNOT retire `orphan-scan --cross-ledger` or source the count line. Item stays OPEN and un-authorized. **Re-decide at a /meeting:** extend the project_manager index to emit checkbox-state + counts, vs. keep `orphan-scan --cross-ledger` as the guard and drop the count prose only (id:1de1). Producer facts as of 2026-07-14: `proj refresh` → 1288 nodes / 2791 edges, `proj graph` consumes it. Meeting `docs/meeting-notes/2026-06-23-0803-ledger-drift-derived-index.md`. <!-- id:659c --> — 🚧 GATED (auto, id:3801; route:decision-gate): premise false (co-citation graph, no checkbox-state/count) — needs a /meeting
...
- [ ] [ROUTINE] **Record the ACTUAL child death cause — relay child deaths are currently untracked** (promoted from TODO `routed:9c91`) <!-- children-of:93cc --> <!-- id:61fa -->
  ...
  - **Why [ROUTINE]**: read-side parsing of a file that already exists, plus two existing sinks and one cache stamp; no contract or model change.

- [ ] [ROUTINE] **Dispatch must NAME the item instead of handing over the whole ROADMAP** (promoted from TODO `routed:79dc`) <!-- children-of:93cc --> <!-- id:b09e -->
...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract — RESCOPED 2026-07-28 after an adversarial review refuted this item's own fix direction and its RED spec** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  ...
  - **DISCREPANCY RECONCILED 2026-07-28** (evidence: `~/.claude/logs/relay-executor-skill-load-evidence-2026-07-28.md` — self-contained, outside any repo; cites both raw transcripts). Two loderite measurements disagreed: original `routed:d2a1` = **+26,674 / +26,425 tok** for the Skill load; direct transcript read of entry [10] = **63,220 chars ≈ 15,805 tok** + 6,974-char `skill_listing` ≈ 1,743 tok = **~17,548 tok**. Gap ≈ **9,126**.
    - **Leading explanation — the original numbers DOUBLE-COUNT.** `routed:d2a1` reported the Reads separately at **+9,505 / +9,473 tok**. `17,548 + 9,505 = 27,053`, within **1.4%** of the reported 26,674. So the original "Skill load" measurement window almost certainly extended past the Skill call to include the child's subsequent `Read`s of `executor-contract.md` + `conventions.md`. Not proven — the two figures come from DIFFERENT runs (07-26 pair vs 07-28 pair) — but the payload sha256 is identical across both 07-28 children and `relay/SKILL.md` is byte-identical at 64,376 B on both dates (`git show` verified), so the Skill load should be constant at ~15.8k on 07-26 too.
    - **DOWNSTREAM CORRECTION**: if the double-count holds, this item's headline "~36k combined fixed contract overhead ≈ 20% of budget" is ALSO wrong — the real combined figure is **~27k**. Do not quote ~36k.
    - **A second apparent mismatch is NOT one**: the evidence lists `SKILL.md` as 63,789 while `wc -c` here gives 64,376. That is **bytes vs characters** — the file is UTF-8 with many multibyte glyphs (`—`, `⇒`, `🚧`); 64,376 B = 63,789 chars. No contradiction, and the 63,220-char payload is that file minus frontmatter plus the prepended `Base directory for this skill:` header.
  - **Use ~17.5k (Skill) and ~27k (combined) as this item's numbers.** Both are transcript-grounded; the ~26.7k and ~36k figures are superseded.
  ...
  - **Achievable lever to evaluate FIRST** (since the Skill behaviour may be immovable): change the fleet `## Relay contract` pointer in each managed repo's `CLAUDE.md` from "Load `/relay executor` before working on any item" to a **direct Read of `relay/references/executor-contract.md`**, so children never invoke the Skill at all. That converts ~17.5k of orchestrator prose into ~3.1k of contract. Confirm the child still receives everything the contract assumes.
  - **Relative priority ROSE even as the number fell** (loderite, post-archive): with the 672 KB ledger term removed by archiving, this skill overhead is now the **larger** remaining share of a child's fixed budget.
...
- [ ] [ROUTINE] **Teach the executor contract symbol-level exploration — it never mentions LSP / Grep / Glob** (promoted from TODO `routed:59b2`) <!-- children-of:93cc --> <!-- id:6f1c -->
...
- [ ] [ROUTINE] **BUG: discovery agent surfaced a PHANTOM parked orphan and idled the pool with 0 dispatched** (promoted from TODO `routed:30fd`) <!-- id:1af1 -->
...
- [ ] [ROUTINE] **`roadmap-lint.sh`: add an acceptance/done-check clause so a structurally un-workable item is caught** (promoted from TODO `routed:14f4`) <!-- id:213a -->
...
- [ ] [ROUTINE] **id:3801's seam emitter CANNOT express an acceptance block — extend the `proposed_split` schema** (promoted from TODO `routed:dbd6`) <!-- id:44a1 -->
...
- [ ] [ROUTINE] **Retire the Fable-availability PROBE; replace it with explicit local config** (owner 2026-07-28: "there shouldn't _be_ a Fable probe anymore since Fable has become a fixed part of the Max subscription") <!-- id:aa26 -->

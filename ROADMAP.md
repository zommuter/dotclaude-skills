...
- [ ] **[ROUTINE]** **Mid-run context-budget check that triggers a clean CHECKPOINT-AND-HANDBACK before an executor dies of `Prompt is too long`** — two loderite executors in run `relay-20260826-162405-7522` died mid-unit and orphaned partial work onto `relay/orphan/…-execute-5adb-0` / `…-execute-repo-0`, SUPPRESSING re-dispatch of those items until a human ran `/relay reconcile`. Measured cause is pure ACCUMULATION, not an oversized dispatch: initial prompts were **8,275 B / 6,359 B**, final transcripts **428,151 B / 477,574 B** over **166 / 193** turns, growth LINEAR (~100 KB by line 24/30, ~400 KB by line 152/150) with no single line above ~5% of its transcript; `tool_result` payloads are ~40-42% of bytes; 46-56 Bash calls per unit (`npm run build`, `npx vitest run`, `npx playwright test`, `npx tsc --noEmit`) streamed multi-KB output that was never trimmed or offloaded. **This is NOT `id:35b7` / `id:4f9b` and that gate MUST NOT be touched** — it sized correctly here and mid-run growth is structurally invisible to any dispatch-time gate. Owner's recorded judgement on the TODO item: of candidates (a) redirect-and-`tail` build output, (b) cap `tool_result` payloads, (c) mid-run budget check → checkpoint-and-handback, **(c) is the highest-value half** — dying costs a human `/relay reconcile` per orphan AND blocks re-dispatch, whereas an early handback costs nothing and keeps the work in the normal flow; the failure is also SILENT to the operator until they notice a suppressed re-dispatch. Promoted from TODO with the same id (single-id-two-views). <!-- children-of:93cc --> <!-- id:5eeb -->
  - **Acceptance**: (1) `relay/scripts/context-budget.sh` exists, is executable, and is a PURE read-only decision function over a byte count — `--bytes N` **or** `--transcript PATH` (the latter just `wc -c`s the file), with `--warn-bytes` / `--handback-bytes` overrides. (2) It prints exactly ONE machine-readable line to stdout: `context-budget: <verdict> bytes=<N> est_tokens=<T> warn_bytes=<W> handback_bytes=<H>`, verdict ∈ `ok|warn|handback|unknown`. (3) Exit status is **0** for `ok`/`warn`/`unknown` and **3** for `handback` (the same "you cannot proceed here" code `host-gate.sh` already uses). (4) **Fail-open, LOUDLY**: a missing/unreadable transcript yields verdict `unknown`, exit 0, and a non-empty stderr line — a measurement failure must never block work, and must never be silent (id:4347). (5) `relay/references/executor-contract.md` gains a rule instructing the executor to run the check at natural checkpoints and, on `handback`, to COMMIT the work already done, append a `HANDBACK:` line to `RELAY_LOG.md`, and return `contract_met=false` with `route="none"` so the item stays plainly re-dispatchable rather than being gated or re-tagged.
  - **Threshold calibration (do NOT re-derive from the 200k window)**: the defaults are **`warn_bytes=200000`, `handback_bytes=300000`**, calibrated on the two OBSERVED deaths (428,151 B / 477,574 B), i.e. handback at ~70% of the smaller one. Transcript bytes are a **PROXY that UNDER-counts true context** — the system prompt, tool definitions, skill payloads and CLAUDE.md are all in the context window but absent from the transcript — so the honest direction here is the OPPOSITE of `prompt-size-gate.mjs`'s window-fraction derivation, and the script's own comments must say so. `est_tokens` is `bytes/4` (`CHARS_PER_TOKEN`, cross-referenced to `prompt-size-gate.mjs`) and is REPORTING ONLY; it must not be what the thresholds compare against. Note `id:9eb7`'s measured 2.66 chars/tok for dense relay markdown — another reason the token estimate is advisory, not the trigger.
  - **VERSIONED-CONTRACT DISCIPLINE (mandatory, silent-breakage class)**: editing `relay/references/executor-contract.md` is a behaviour change an in-flight executor must know about, so the SAME change MUST bump its in-file marker `<!-- relay-executor contract v12 -->` → **`v13`**, add a `**v12 → v13 (id:5eeb)**` paragraph to the contract's `## Maintenance` section, AND update the `## Relay contract <!-- relay-executor contract vN -->` pointer in this repo's `CLAUDE.md` to match. A contract edit without both is the failure the marker exists to prevent, and the RED spec asserts marker ≥ 13 and pointer == marker.
  ...
  - **Context**: `relay/scripts/context-budget.sh` (new), `relay/references/executor-contract.md` (rule + v13 bump + Maintenance entry), `CLAUDE.md` (`## Relay contract` pointer). Precedents to follow: `relay/scripts/host-gate.sh` (exit-3 convention), `relay/scripts/prompt-size-gate.mjs` (`CHARS_PER_TOKEN`, the dispatch-time sibling), `tests/test_contract_mentions_symbol_tools_6f1c.sh` (contract-marker assertion shape).
  - **SCOPE GUARDS**: do NOT change `relay/scripts/prompt-size-gate.mjs` or its byte-identical inline copy in `relay-loop.js`; do NOT edit `relay-loop.js` at all; do NOT implement candidates (a) or (b) here (they remain open on the TODO item); do NOT add a new `route` enum value — `"none"` is deliberately reused so `handback-followup.py` needs no change and the item is not gated.
  - **HONEST LIMIT** (stated on the item so the reviewer does not read more into a green suite than is there): the contract half is a STATIC grep over contract TEXT — it proves the contract carries the rule and that the version markers agree; it CANNOT prove a Sonnet child's behaviour changes. Same limit as `id:6f1c` / `id:9eb7`. The script half IS behaviourally tested.

...
- [ ] [ROUTINE] **`tracker/ledger-map.py` loses parents on a multi-`children-of:` line -- silent DATA LOSS.** <!-- id:8302 -->
  ...
  - **Done-check**: `bash tests/test_ledger_map_multi_parent_8302.sh` exits 0 (RED today: `parent_aa01=0`), then tick this box and `make test` is fully green -- `tests/test_tracker_golden_fixture.sh` and `tests/test_tracker_schema_drift_roundtrip.sh` must stay green, since widening `parent` may need a `schema/ledger-intermediate.schema.json` change plus a schema-version bump.
  - **Representation is the implementer's call** -- widen `parent` to a list, fold `children-of:` into the parent-side `children` list, or add a reconciliation pass. The spec asserts the OBSERVABLE (both edges survive) and deliberately excludes `body`/`title`/`sources`, because `body` retains the raw `<!-- children-of:aa01,aa02 -->` marker verbatim and makes a whole-item substring match vacuous.
- [x] [ROUTINE] **Executor-contract rule 2c's ZERO-COMMIT branch leaks its own escalation counter, so `route="hard-split"` is unreachable.** <!-- id:d8b3 -->
  - **Context.** The branch (`relay/references/executor-contract.md`, "ZERO-COMMIT branch -- the livelock guard") tells the executor to count prior occurrences with `grep -c "HANDBACK: <item-id> .*ZERO-COMMIT" RELAY_LOG.md` and escalate at `>= 1`. That line is committed on the HANDBACK branch, and the integrator never merges a handback -- so the next attempt branches from main, reads 0, and the escalation never fires. Observed on run `relay-20260831-220243-21277`: `git-annex` handed back `hard` TWICE through this exact branch (2c fired at 273,449 B against the 300,000 B threshold, zero-commit both times) and the repeat-handback ALERT flagged it as a bug signal.
  - **Acceptance**: two successive zero-commit 2c handbacks for one repo/item leave a count of **2** readable by the third attempt, which starts from a fresh worktree off `main`. The contract's counting instruction names a concrete accumulator path OUTSIDE the repo working tree, and records at the point of use WHY `RELAY_LOG.md` cannot be the store (the integrator never merges a handback), so a later contract edit cannot reintroduce it.
  - **Done-check**: `bash tests/test_zero_commit_accumulator_durable_d8b3.sh` exits 0 (RED today, at assertion (a)), then tick this box and `make test` is fully green.
  - **Store -- use the existing primitive, do not invent one.** `relay/scripts/relay-state-write.sh event-append <abs-path>` is already a flock'd, append-only, branch-independent JSONL substrate and is the intended store. **Do NOT write the counter into `relay.toml`**: that is owner-facing configuration and `toml-set` is the owner's durable-judgement surface, not a runtime accumulator. The TODO line named both candidates as open; this narrowing is the REVIEWER'S RECOMMENDATION and is flagged in `REVIEW_ME.md` for owner confirmation -- if the owner prefers `relay.toml`, assertion (a) still passes and only this bullet changes.

...
- [ ] [ROUTINE] **A unit whose worktree is LEFT on disk (isolation-gate defer, dirty exit) can never be retried in the same run — the branch name is deterministic, so re-provisioning fails "branch already exists"** <!-- children-of:f91a --> <!-- id:9834 -->
  ...
  - **Fix direction, NOT settled — decide before building**: either (a) make the branch/worktree name attempt-scoped (append the existing `unit.attempt` so a retry gets a fresh name — note `unitKey()` already carries `attempt`, so the concept exists), or (b) detect the deferred-worktree case at provision time and REUSE the existing worktree rather than re-creating it, or (c) mark the unit un-retryable for the run and skip straight to the circuit breaker without burning three dispatches. (a) is cheapest; (b) risks handing a child a tree with another attempt's uncommitted state — weigh that carefully. If `id:76d2` lands first, this becomes much rarer but not impossible (a genuinely dirty exit still parks a worktree).

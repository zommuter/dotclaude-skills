...
- [ ] [INPUT — decision] **Wire the built-but-unreferenced fan-out machinery (`disjoint-greenlight.sh` + `drain-integrate.sh`) into `relay-loop.js`** <!-- children-of:1f4f --> <!-- id:ae08 --> — 🚧 GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:02b2, id:99e5, id:5b12 — pick those, not this. ae08 bundles 3 scopes incl. a tick-ownership contract inversion (executor-contract v11→v12); too large for one turn — decompose into greenlight-wire / drain-integrate-route / tick-ownership seams (deps b099/923b already closed; design ratified meeting 2026-07-26-1922).
...
- [ ] **[HARD]** Route same-repo drain-mode integration through drain-integrate.sh (serialized one-writer safety net) (after id:<seam-1 id>) — seam of id:ae08 (auto, id:3801) <!-- id:99e5 -->
...
- [ ] **[HARD]** Invert ROADMAP tick ownership: executors return worked_ids, driver ticks; bump executor-contract v11->v12 + CLAUDE.md pointer — seam of id:ae08 (auto, id:3801) <!-- id:5b12 -->
  - **Acceptance**: executor-contract.md rule text states executors no longer tick ROADMAP.md checkboxes (they report worked_ids only) and the driver/integrator performs the tick from the returned worked_ids; the integrate path in relay-loop.js does that tick; the contract marker is bumped v11->v12 and CLAUDE.md's '## Relay contract' pointer matches; asserted by a test over BOTH the contract text and the integrate path.
  - **Done-check**: tests/run-tests.sh tests/test_relay_worked_ids.sh green, the new/updated assertion test green, full tests/run-tests.sh green, and lint-workflow-templates.mjs clean
  - **Context**: relay/references/executor-contract.md + CLAUDE.md (## Relay contract pointer) + relay/scripts/relay-loop.js (integrate/tick path near worked_ids handling ~line 821-826)

### Provisioning-vs-isolation collision (found in run relay-20260812-001727-5554, `children-of:f91a`)

- [x] [ROUTINE] 🔴 **`provision-worktree.sh`'s artifact SYMLINKS dirty the worktree, so `verify-isolation.sh` refuses to merge the child's work — `id:34b7` part (2) actively defeats `id:f682`** <!-- children-of:f91a --> <!-- id:76d2 -->
  - **Root cause, verified by command 2026-08-12**: `provision-worktree.sh:29-30` creates `.venv`/`node_modules` as **symlinks**. The idiomatic gitignore form is the trailing-slash **directory** pattern (`.venv/`), and git does **not** match a symlink against it. Proof in the live worktree: `ls -ld .venv` → `lrwxrwxrwx … -> /home/tobias/src/linguistic-universals/.venv`; `.gitignore:7` → `.venv/`; `git check-ignore -v .venv` → **NOT IGNORED**; `git status --porcelain` → `?? .venv`.
  - **Consequence**: `verify-isolation.sh` (id:f682) sees a dirty tree and correctly refuses the merge — *"isolation failure: worktree has a DIRTY tree (uncommitted changes) — not safe to merge / ?? .venv"*. **The child's work is then parked unmerged.** Live loss this run: `linguistic-universals` branch `relay/relay-20260812-001727-5554-execute-4d35-0` holds 2 real commits (`c1059cd` feat: construct_class + ValidityAnchor schema for id:4d35, `cade0cd` the tick + session log) that never reached main. The executor did nothing wrong — contract rule 5b told it to gitignore throwaway, and the repo already had.
  - **Blast radius**: EVERY repo whose `.gitignore` uses the trailing-slash form for a provisioned artifact class, which is the common form. This is silent — the merge is refused, the work parks, and only the handback text names it.
  - **Fix direction (preferred)**: have `provision-worktree.sh` write the names it symlinks into the worktree's **`.git/info/exclude`** (per-worktree, local, never committed) right after creating them. That fixes it for every repo without touching any repo's `.gitignore` and without weakening the isolation gate. Do NOT relax `verify-isolation.sh` to ignore these paths by name — the gate's value is that it does not special-case, and a name-based carve-out would also hide a genuine breach.
  - **Also recover the parked work**: the `linguistic-universals` branch above is intact on disk; integrate it once the fix lands (`relay-reconcile.sh --integrate`), do not discard it.
  - **Acceptance (BDD)**: GIVEN a repo whose `.gitignore` contains `.venv/` and which has a real `.venv` directory WHEN `provision-worktree.sh` provisions a worktree THEN `git -C <wt> status --porcelain` is EMPTY and `verify-isolation.sh <wt>` exits 0. GIVEN a genuinely dirty worktree (a real uncommitted source edit) THEN `verify-isolation.sh` still exits non-zero — the gate is not weakened.

- [x] [ROUTINE] **A unit whose worktree is LEFT on disk (isolation-gate defer, dirty exit) can never be retried in the same run — the branch name is deterministic, so re-provisioning fails "branch already exists"** <!-- children-of:f91a --> <!-- id:9834 -->
  - **Observed same run**: after `id:76d2` parked round 1's `linguistic-universals` worktree, round 2's provision returned `MECH-ERROR exit=255 … fatal: a branch named 'relay/relay-20260812-001727-5554-execute-4d35-0' already exists`, so the unit was refused; round 3 likewise; the id:365b circuit breaker then skipped the repo as "dispatched >3× with no substantive change". Three dispatch attempts were spent on a unit that could not possibly succeed.
  - **This is NOT a regression from `id:66d9`** — worth stating plainly, because the failure only became *visible* with fail-closed. Before it, round 2 would have dispatched a child into a **non-existent** worktree (the fail-open this cluster fixed). The corrected gate turned a silent corruption into an honest refusal; what it exposed is that retry-after-defer was never viable.
  - **PREMISE CORRECTED at spec-authoring 2026-08-12 — the original text below was WRONG and would have sent an executor to rewrite working code.** It proposed "(a) make the branch/worktree name attempt-scoped". **The names are ALREADY attempt-scoped**: `unitKey` is `${verdict}-${itemId||'repo'}-${attempt||0}` (`relay-loop.js:~2098`) and both `worktreePathFor`/`branchFor` (`:2110`/`:2111`) thread `attempt: unit.attempt || 0` into it. That machinery is correct and must NOT be touched. **The actual gap**: nothing anywhere INCREMENTS `attempt` — a re-dispatched unit is a fresh discovery object with no `attempt` field, so it renames to `…-0` every round and collides with its own parked branch. (Verified: `grep -n attempt relay-loop.js` shows reads in `unitKey`/the name builders and nowhere an assignment or increment.)
  - **Fix shape (reviewer's decision, recorded so it is re-checkable)**: on a provision failure whose body reports the branch/worktree already exists, bump the unit's `attempt` **once** and re-provision under the fresh name. Exactly one bump — an unbounded retry would spin against a genuinely broken repo. The two rejected alternatives, and why: **reuse the parked worktree** would hand a child another attempt's uncommitted state, which is exactly what the isolation gate exists to prevent; **skip straight to the circuit breaker** is strictly weaker, since it abandons work that a fresh name completes. If `id:76d2` lands first this becomes much rarer but not impossible — a genuinely dirty exit still parks a worktree.
  - **Acceptance (BDD)**: GIVEN a unit whose prior attempt left a worktree and branch on disk WHEN the same unit is dispatched again in the same run THEN provisioning either succeeds under a distinct name or the unit is skipped with a reason naming the prior attempt — and in NO case are three dispatch attempts spent on a structurally impossible provision.

- [x] [ROUTINE] **Count a BLOCKED/failed `agent()` dispatch into `state.agentFailures` — the classifier-block channel and the failure-accumulator channel are still disjoint** <!-- children-of:83c2 --> <!-- id:3222 -->
  - **Slice (c) of `id:83c2` ONLY** — the cheap, strictly-improving half. It does NOT attempt (a) self-attesting hops or (b) moving hops off the agent dispatcher; both need an owner decision and stay open on `83c2`.
  - **Evidence**: run `relay-20260812-001727-5554` had **39 agents blocked by the harness safety classifier** (release ×17, write-relay-status ×6, inject-take ×6, provision ×1, gaming-log ×1) while `RELAY_STATUS.md` recorded `agent-failures=8`. The two numbers come from different channels: `id:a104` wired the three *parse* sites, but a dispatch that resolves null/throws at the `agent()` boundary is still counted nowhere the operator can see. 39 invisible failures next to 8 visible ones is the `id:4347` shape.
  - **FIRST, establish the observable** (do not assume): determine what a classifier-blocked `agent()` call actually does in this harness — resolves `null`, throws, or returns a string. `provision:linguistic-universals` round 3 recorded `no PROVISION-OK token: ""`, which suggests an empty-string resolution rather than a throw, but ONE observation is not the contract. Check the journal at `~/.claude/projects/-home-tobias-src-dotclaude-skills/*/subagents/workflows/wf_3da78c35-8a0/journal.jsonl` for the blocked agents' recorded returns. If the two cases are distinguishable, record which one happened; if not, record the failure without inventing a cause.
  - **Then**: wrap the fire-and-forget / best-effort `agent()` dispatch sites (the `release:*`, `write-relay-status`, `gaming-log`, `heartbeat-*` hops) so a null/empty/thrown result records into `state.agentFailures` with that hop's label. **Keep the fail-soft semantics unchanged** — this is visibility only; a hop that currently continues on failure must still continue.
...
- [ ] [ROUTINE] **`lint-mech-model.mjs` no longer lints the `release:` fence — routing it through `dispatchGuarded` moved it out of a bare `agent(` call, and the linter matches only the identifier `agent`** <!-- children-of:3222 --> <!-- id:ed3f -->

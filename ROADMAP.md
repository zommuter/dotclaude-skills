...
- [ ] [ROUTINE] **Orchestrator writes the ledger SLICE to a tmp file and hands the child a PATH — the child can no longer over-read, because the bytes are not in its prompt** (owner proposal 2026-08-21) — today the prompt embeds/points at the whole `ROADMAP.md` + `TODO.md` and a prose instruction TELLS the child to slice; a prose instruction the child can get wrong is not a guard. <!-- id:e68f -->

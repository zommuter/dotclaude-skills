...
- [ ] [ROUTINE] **The pool silently works `actionable_routine_ids[0]`, so the item a human cares about is never the one picked — surface the choice** <!-- id:8af2 -->
...
- [ ] [ROUTINE] 🔴 **GATE-CLEARING — the DOCUMENTED `ckpt-tag.sh` label format never advances `last_strong_ckpt`, so supervised strong checkpoints silently leave the watermark stale** <!-- id:1a34 -->

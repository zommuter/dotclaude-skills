...
- [ ] [ROUTINE] **Extract `isDryRound`/`workCreated` into ONE shared definition — CROSS-FILE, via a generation step; record the drain-path gap MOOT-BY-RETIREMENT** <!-- id:6217 -->
...
- [ ] [ROUTINE] **DISSOLUTION — the parent creates + provisions the worktree BEFORE dispatch, and the child is never told the main-checkout path** <!-- children-of:f91a --> <!-- id:34b7 -->
  - **Why**: removes the child's *reason* and *means* to reach into the main checkout, rather than guarding the write after the fact. Needs no API support (neither the Workflow `agent()` surface nor the interactive Agent tool exposes `cwd`; `grep -rn "cwd:" relay/scripts/` finds no precedent) — it is prompt + dispatch plumbing only. **Explicitly NOT gated on `id:d464`**: no hook, no permission surface, so the afk-prompt hazard that parked `d464` does not touch this.
  - **Three parts, and the ORDER is load-bearing (owner's objection, 2026-07-30)**:
    1. Move `git -C <repo> worktree add <wt> -b <branch> HEAD` out of the child's first action (`relay-loop.js:1962`, the `Create your worktree first:` line) into a **pre-dispatch step** in `relay-loop.js`, so the child receives a ready worktree. Reuse the existing `worktreePathFor(unit)` / `branchFor(unit)` helpers (`:1877-1878`) — the deterministic names the API-error recovery path already depends on must not change.
    2. Provision the gitignored build artifacts the child would otherwise go to main for — symlink/reuse `node_modules`, `.venv`, caches, downloaded browser binaries. Children already do this by hand (loderite `RELAY_LOG.md:2681`, with `:3022`/`:3422`/`:3486` showing them removing the symlink before commit), which is exactly the manual step this part absorbs.
    3. **Only after (1) AND (2)**: drop `main checkout: ${unit.path}` from the child prompt (`:1958`) and from the resume prompt (`:1988`). Removing the path first would break exactly the repos that legitimately reuse main's artifacts.
  - **DOGFOOD EVIDENCE (new, 2026-08-01)**: this very handoff ran with part (1) already applied by hand — the orchestrator created `~/.cache/relay/worktrees/dotclaude-skills/relay-20260801-205940-1772-handoff` and its branch BEFORE dispatching the child, and the child's prompt named only the worktree path as its work root. The child completed C1–C3 with no need to reference the main checkout, so part (1) is demonstrated viable for at least the handoff verdict, by hand, once. That is an existence proof for the shape, **not** coverage of the execute/hard/review verdicts or of part (2).
  - **Acceptance**:
    - a dispatched child's prompt string contains NO main-checkout path — for EVERY verdict, including the resume prompt;
    - the prompt no longer instructs the child to run `git worktree add` itself;
    - the parent performs the `worktree add` before the `agent()` call, using the same `worktreePathFor`/`branchFor` names;
    - `git worktree list` in the target shows the worktree exists at dispatch time;
    - artifact provisioning (part 2) is implemented and stated in the commit message for whichever artifact classes are covered — a part-2 that covers nothing must say so and MUST NOT unlock part (3).
...
- [ ] [ROUTINE] **INTEGRATE and a real REVIEW share the label `reviewer (<model>)`, so integrating your own executors' work silently marks it AUDITED** <!-- id:ecce -->
...
- [ ] [ROUTINE] **`validate-flags.sh` does not parse `--flag=value`, so an explicit quota cap is SILENTLY LOOSENED to the default** <!-- id:2047 -->
...
- [ ] [ROUTINE] **`open_hard_pool` counts `@container` epics, so a fully-decomposed repo draws a PHANTOM `hard` verdict forever** (ADOPTS `routed:d866` — loderite filed the identical finding from the same run) <!-- routed:d866 --> <!-- id:d808 -->

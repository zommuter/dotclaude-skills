...
- [ ] [HARD] **`classify-verdict.sh`'s strict rank order starves `hard` and `handoff` INDEFINITELY -- three victims now (hard dispatch, review cadence, promotion itself). The CASCADE IS ALREADY FINE (--exclude walks it); the defect is loop-side at relay-loop.js:4615, which discards every chain-end verdict that is not `review` and passes no --exclude.** -- detail: `docs/ledger-notes/4e84.md` <!-- routed:71c6 --> OWNER RULING 2026-09-10: the apex-gate restoration (review D1) STANDS -- a chain-end `hard` obeys enforceApexGate like a discovered one. <!-- id:4e84 -->
...
- [x] **[ROUTINE]** Install the head-length and shape-prose baselines via the Makefile, gated on the repo-keyed baseline format landing first (after id:<the repo-dimension seam's id — seam 2 above>) — seam of id:4839 (auto, id:3801) -- NO aa5e WORK EXISTS ANYWHERE, despite the aa5e-keyed park (breadcrumbed 2026-09-09 by review `relay-20260909-205831-5121`): the residue commit `18359d7a` is labelled *"for worktree relay-20260909-143257-21736-execute-aa5e-0"* but its whole content is 62 lines added to `tests/test_workflow_check_hardening.sh` -- the `(b3)`/`(b4)` mutation-pinning cases belonging to **id:0165**, not to this seam. It was merged to main by the 20:42 reconcile via `f3ba0127` and those cases pass. So do NOT go looking for a partial baseline-install implementation to resume: this seam is untouched and starts clean. Its stated gate on the repo-dimension seam (id:c655) is unaffected and still stands. <!-- id:aa5e -->

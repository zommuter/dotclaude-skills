...
- [ ] [ROUTINE] full-unit assembler — `classify-repo.sh --emit unit` (flip step b sub-component, id:a0b6; discovered 2026-07-01) <!-- id:3d61 -->
...
- [ ] [HARD — pool] a0b6 remainder — the confined `relay-loop.js` verdict-source swap (flip step b; meeting 2026-07-01-1904) <!-- id:a0b6 -->
  - **GATED on id:5987 (done ✓) AND id:3d61 green** (the runner needs both `reconcile-repo.sh` for side effects and `classify-repo.sh --emit unit` for full units). Careful supervised engine work — the engine crashed the pool 3× on the template-literal-lint hazard; NOT a tail-of-session edit.
  ...
  - **Acceptance**:
    1. Replace the `shardPrompt` builder + the `agent(shardPrompt(chunk), …)` call (`relay-loop.js:820-900`) with a runner-agent prompt: per repo run `reconcile-repo.sh` (fold its `actions`/`surfaced`) then `classify-repo.sh --emit unit` (the full unit), return the SAME `SHARD_SCHEMA` `{units,surfaced,skipped}`. LLM judgment ONLY on an `AMBIGUOUS` verdict. Reuse the id:612f NO-FILESYSTEM-HUNTING guard verbatim.

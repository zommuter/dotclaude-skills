...
- [x] [ROUTINE] **`children-of:` and `children:` still resolve to TWO different relations -- the UNMET half of id:8302's acceptance.** <!-- id:59c5 -->
  - **Context.** id:8302 shipped clause (a) (a multi-parent `children-of:` line no longer drops a parent) but NOT clause (b) of the same ratified acceptance: *"the two spellings resolve to ONE relation, so a parent-side `children:` edge and a child-side `children-of:` edge naming the same pair produce the same fact"*. Its RED spec `tests/test_ledger_map_multi_parent_8302.sh` built a fixture for (b) (`aa03 children:bb02`) and then asserted nothing about it, so the clause shipped unimplemented with a green suite. MEASURED this review on the post-8302 tree: `repo/bb01 parents=[aa01,aa02] children=[]`, `repo/aa01 parents=[] children=[]`, `repo/aa03 parents=[] children=[bb02]`, `repo/bb02 parents=[] children=[]` -- the relation is resolvable in exactly ONE direction and WHICH direction depends on which end the author wrote it from. Every consumer that walks the graph (closure by child edges, gate fan-out, the tracker projection) therefore sees a different graph for two ledgers stating the same fact.

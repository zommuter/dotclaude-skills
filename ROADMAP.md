...
- [ ] [INPUT - decision] **Build the `/inflownistration` skill -- three modes plus a parameter-less ROUTER, scope RATIFIED 2026-08-21 (promoted from TODO.md id:32c3, single-id-two-views)** -- detail: `docs/ledger-notes/32c3.md` OWNER RULING 2026-09-10: NEITHER mode reads the marks -- id:93d6's constraint on (b) stands; the CLAUDE.md (c) amendment's "belongs to (b)" clause is SUPERSEDED (cross-repo amendment pending). <!-- id:32c3 --> — 🚧 GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:68c1, id:d846, id:a187, id:ccf7 — pick those, not this. Three independent modes plus a router, spec in a private sibling repo, no per-item RED spec -- split into 4 seams (skeleton+router, drift-check, filing, public-source guard).
  - **Not in the detail note, so recorded here**: the consumption boundary is FORMAT-PARSING for row insertion ONLY, and originating item `id:93d6` adds that (b) may NOT read the per-component marks to cross-check a claim. **Unresolved restatement conflict, reviewer to settle before building**: inflownistration's `CLAUDE.md` amendment (c) instead says that mark-reading "belongs to filing-mode (b), not (c)" -- the two ratified-day restatements disagree about whether (b) may read marks at all. **Ratified spec**: `~/src/inflownistration/docs/meeting-notes/2026-08-21-0925-inflownistration-three-input-items.md` D3+A3, plus that repo's `CLAUDE.md` carve-drift tripwire amendment (c) -- a cold reader BUILDS (c), it does not refuse it as unratified accretion. **Origin**: inflownistration `id:93d6` (`routed:37bd`); the TODO.md twin stays OPEN. **Done-check**: installs as per-file symlinks via a `make install-inflownistration` target, `make status` reports it, no `/infln` alias anywhere, the public source carries no instance name and no populated table schema, `make test` green. REVIEWER-worked `[HARD]` -- no RED spec, no executor session.
...
- [x] [ROUTINE] **Teach `lint-pipefail-sigpipe.py` the SCRIPT consumer -- `producer | bash <script>` whose script exits before draining stdin SIGPIPEs the producer, and the id:81d5 sweep never counted it.** -- detail: `docs/ledger-notes/6294.md` <!-- relates:7518 --> <!-- relates:81d5 --> <!-- relates:ebd0 --> <!-- id:6294 -->
  - **Why it is not speculative**: reproduced live in this review's own `make test`. `tests/test_privacy_gate_prepush.sh` printed 8 `ok:` lines, zero `bad:`, no `---- N ok, M bad ----` summary, and the runner still marked it FAIL; standalone it passed. Traced to `tests/test_privacy_gate_prepush.sh:114` piping `printf` into `bash "$HOOK"`, where `hooks/pre-push-privacy-gate.sh` takes its D4 `exit 0` at `:48` -- before its `while IFS= read -r line` at `:54`.
  - **Acceptance**: (a) the lint flags a pipeline whose consumer invokes a script under this tree, unless that script provably drains stdin -- the commit states which branch was taken, since flag-outright and prove-it-drains differ in false-positive cost; (b) the seven `printf ... | bash "$HOOK"` sites in `tests/test_privacy_gate_prepush.sh` move to a non-pipe producer form; (c) the `out="$(...)"; rc=$?` sites there are rewritten so a non-zero hook status is actually reportable -- today `set -e` exits before `rc=$?` is read, so the `[[ $rc -eq 0 ]] || bad ...` guard can never fire.
  - **Red spec**: `tests/test_pipefail_script_consumer_6294.sh` -- RED today at exactly 2 of 5 assertions (the `bash` and `sh` consumer cases), with BOTH negative controls (drain-to-EOF `wc -l`, and the recommended `< <(producer)` rewrite) already GREEN, so it is not vacuously red. Its case 5 runs the lint over the real privacy-gate test and will turn red the moment (a) lands, which is what forces (b).
  - **Done-check**: `bash tests/run-tests.sh tests/test_pipefail_script_consumer_6294.sh tests/test_pipefail_sigpipe_lint.sh tests/test_privacy_gate_prepush.sh` green, then the full `make test` green.
  - **Files**: `tests/lint-pipefail-sigpipe.py` (`early_exit_reason()`, which returns None for any command outside grep/head/read/sed/awk), `tests/test_privacy_gate_prepush.sh`, `hooks/pre-push-privacy-gate.sh` (read-only reference for the `:48` early exit).
...
- [ ] **[HARD]** Mode (b): the report-only ratified-source DRIFT CHECK (after id:seam 1 (skeleton + router)) — seam of id:32c3 (auto, id:3801) <!-- id:d846 -->
  - **Acceptance**: Mode (b) locates a claim's ratified source by `id:XXXX` grep and diffs the restatement verbatim, hunting the three named mutations: disjunctive rule silently becoming conjunctive (or vice versa), criterion swapped under the same name, and criterion lifted out of a recorded-not-adopted / rejected-alternative section. It is REPORT-ONLY (writes nothing), FAILS LOUD when no ratified source is found instead of guessing one, does NOT read the per-component marks (owner ruling 2026-09-10; id:93d6's constraint on (b) stands and supersedes the inflownistration CLAUDE.md (c) amendment clause), and STATES ITS COVERAGE LIMIT in every report -- it sees only id-anchored claims, so a clean pass is never 'no drift'.
  - **Done-check**: tests/test_inflownistration_drift_check_32c3.sh green: fixture pair (ratified source + drifted restatement) proves a report for each of the three mutation shapes, a LOUD non-zero failure when the id has no ratified source, the coverage-limit line present in a clean pass, and zero writes (fixture bytes identical after the run). `tests/run-tests.sh` fully green.
  - **Context**: inflownistration/drift-check.sh (new) and the mode (b) section of inflownistration/SKILL.md; tests/test_inflownistration_drift_check_32c3.sh (new)
- [ ] **[HARD]** Mode (c): file a new instance or worked example, propose->confirm with an --afk refusal (after id:seam 1 (skeleton + router)) — seam of id:32c3 (auto, id:3801) <!-- id:a187 -->
  - **Acceptance**: Mode (c) drafts the row plus its evidence prose, proposes each per-component mark with a one-line reason, shows a diff, and writes ONLY on explicit owner confirmation -- the marks are the owner's, so the skill never assigns them. Under `--afk` / pool dispatch it REFUSES AND SURFACES rather than default-writing. Its consumption boundary is FORMAT-PARSING for row insertion only, never semantic consumption of a mark's value.
  - **Done-check**: tests/test_inflownistration_file_mode_32c3.sh green: unconfirmed and `--afk` invocations both exit non-zero with the surfaced reason and leave the fixture table byte-identical; a confirmed invocation inserts exactly one row; a grep-based assertion proves no code path branches on a mark's value. `tests/run-tests.sh` fully green.
  - **Context**: inflownistration/file-instance.sh (new) and the mode (c) section of inflownistration/SKILL.md; tests/test_inflownistration_file_mode_32c3.sh (new)
- [ ] **[ROUTINE]** Public-source cleanliness guard for the new skill (after id:seam 3 (mode (c) filing)) — seam of id:32c3 (auto, id:3801) <!-- id:ccf7 -->
  - **Acceptance**: A test enforces, as a standing invariant, that this PUBLIC skill's own source embeds no instance names and no populated table schema (only an empty/structural one), and that no `/infln` alias exists anywhere in the tree. The test must report pattern INDICES, never the private pattern text, following the tools/privacy-audit.sh precedent -- printing the pattern in a tracked file of this public repo would itself publish it.
  - **Done-check**: tests/test_inflownistration_public_source_clean_32c3.sh green, and demonstrably red against a fixture that plants an instance name plus a populated row (declare it with `# fails-against-mutation:` and a `# fails-against-assertion:` substring matching exactly one FAIL line, per CLAUDE.md Testing). `tests/run-tests.sh` fully green.
  - **Context**: tests/test_inflownistration_public_source_clean_32c3.sh (new), guarding inflownistration/**; pattern-index convention borrowed from tools/privacy-audit.sh

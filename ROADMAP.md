...
- [x] [INPUT — decision] **A relay round that dispatches NOTHING leaves no trace of its verdict or reason — emit a `verdict`-kind event per repo per round** <!-- id:c7dc --> — @container SUPERSEDED-BY-SEAMS (closed 2026-08-11 review): all three seams landed — id:258d (classify-repo.sh priority_rank passthrough), id:37f2 (no-unit verdict/priority_rank carry), id:e87d (relay-loop.js pushEvent('verdict',…) once per repo per round incl. cache-reused) — jointly delivering the full acceptance below; no separate owner-decision remained (the `[INPUT — decision]` lane was the hard-split auto-tag, not a pending ruling). Was: 🚧 @container GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:258d, id:37f2, id:e87d. id:c7dc spans 4 files (classify-repo.sh/discover-repo.sh/discover-chunk.sh/relay-loop.js) + a new dual-mode test; no sub-piece is independently acceptance-complete — split into 3 dependent seams.

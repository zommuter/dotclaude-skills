...
- [x] **[ROUTINE]** mechanical-proxy: a `relay-mech-stdin` fence on a MULTI-STAGE pipeline misdelivers the payload to the FIRST stage -- refuse it (reverse-handoff promotion, review run relay-20260908-174448-4421) <!-- id:09e4 -->
  - **Acceptance**: When a ```relay-mech-stdin fence is present, `_mechanical_dispatch` requires the command to be a SINGLE stage and REFUSES a multi-stage pipeline (returns None, fail open) instead of admitting it on the strength of its last stage. The refusal is LOUD: a `mechanical_stdin_refused` log entry whose `reason` names the PIPELINE shape, distinguishable from the existing id:a05c not-admitted refusal. Nothing else loosens or tightens: a BARE admitted invocation carrying a fence still dispatches with its payload, and a multi-stage command with NO fence behaves byte-identically to today.
  - **Done-check**: `tests/run-tests.sh tests/test_mech_stdin_pipeline_misdirect_09e4.sh` green (RED at promotion: assertions (B) flagship and (C) loudness fire; negative controls (A) and (D) already pass and MUST stay passing -- they are what stops the fix being "refuse everything"). Then full `make test` green.
  - **Context**: `relay/scripts/mechanical-proxy.py` -- the mismatch is between line 689 (`pinned = _last_stage_relay_script(command)`, admission keyed on the LAST stage) and line 723 (`subprocess.run([MECH_SHELL,'-c',command], input=stdin)`, delivery to the shell = the FIRST stage). Both verified by reading the file at HEAD this review. Detail + the original diagnosis: `docs/ledger-notes/09e4.md`. Rated LOW/latent, not an active vuln: the command is constructed by `relay-loop.js` (trusted) and no admitted member is piped today. Unblocks the ROADMAP items gated on it (`id:d4ca`, `id:e405`), and clears 2 of this repo's 4 stale `roadmap-lint` DEAD-GATE warnings.
- [ ] **[ROUTINE]** `roadmap-lint.sh`'s detail-pointer extractor returns TWO paths for a line that names its note twice, so a present note is reported MISSING and its acceptance clause is never read (review run relay-20260908-174448-4421) <!-- relates:e95b --> <!-- id:78e6 -->
  - **Acceptance**: `item_detail_path()` returns exactly ONE path for a ledger line containing two or more `<dir>/<id>.md` mentions. Today `grep -oP -m1 ... <<<"$line"` bounds matching LINES, not matches per line, and `-o` prints every match on the single here-string line -- so `rel` becomes a two-line string, `[[ -f "$_rl_notes_root/$rel" ]]` in `item_has_body_clause()` fails, and the function returns 2. The visible damage is TWO-fold and the second half is the one that matters: a WARN claiming a note "does not exist" when it does (measured this review on `id:8372`, whose gate annotation repeats `docs/ledger-notes/8372.md` in its reason text while the file is present at 8609 bytes), and -- because the early return skips the note read -- the item's real Acceptance/Tests/Done-check clause inside that note is never consulted. A guard that reports the wrong thing AND stops looking. Take the FIRST match; keep the `-m1`-not-`head -1` SIGPIPE reasoning in the comment above it intact and correct it rather than deleting it.
  - **Done-check**: a new `tests/test_roadmap_lint_duplicate_detail_pointer_78e6.sh` green -- hermetic fixture with an open item whose line names `docs/ledger-notes/<id>.md` twice and whose note EXISTS and carries an `**Acceptance**:` clause; assert `roadmap-lint.sh` emits NO `DETAIL-POINTER-MISSING` and NO `NO-ACCEPTANCE` for that id (RED today: it emits DETAIL-POINTER-MISSING). Declare a `# fails-against-mutation:` case per CLAUDE.md §Testing. Then `relay/scripts/roadmap-lint.sh <repo>` no longer WARNs on `id:8372`, and full `make test` reports 0 failed.
  - **Context**: `relay/scripts/roadmap-lint.sh` -- `item_detail_path()` at line 388 (the `grep -oP -m1` at line 394) and its caller `item_has_body_clause()` at line 409 (the `return 2` at line 418). The trigger in the live ledger was written this window by the id:3801 auto-gate annotation on `id:8372`, so the shape recurs whenever a gate reason cites the item's own note path; fix the extractor, not the one annotation.
- [ ] **[ROUTINE]** `count-indented-ids.py` does not satisfy id:8679's own RED spec: no positional-path invocation, no enumeration in DEFAULT output, no stated counting rule (review run relay-20260908-174448-4421) <!-- relates:8679 --> <!-- id:0f0a -->
  - **Acceptance**: `tests/test_indented_id_population_8679.sh` passes against `tools/count-indented-ids.py`. Three gaps, each verified this review by running that spec against the delivered tool: (1) the tool REJECTS a positional ledger path (`error: unrecognized arguments: .../TODO.md`, exit 2) -- accept one as a synonym for `--root <dir> --file <name>`; (2) DEFAULT output does not enumerate the ids (only `--show-lines` does), so spec assertion (1) cannot see the population -- enumerate by default, or have the positional form imply it; (3) the header names the population but never the RULE, so assertion (3)'s `rule|counts?:|criteri` matches nothing -- print the counting rule. The PREDICATE is already correct and must NOT change: run against the spec's own fixture this review, the tool returns exactly `{a1a1,a2a2,a3a3}`, count 3, with cases B/C/D/F/G excluded exactly as the spec requires. This is an invocation-and-presentation contract, not a recount.
  - **Done-check**: `tests/run-tests.sh tests/test_indented_id_population_8679.sh` green (RED at promotion -- it dies at assertion (0) today), `tests/run-tests.sh tests/test_count_indented_ids_8679.sh` STAYS green, and full `make test` reports 0 failed.
  - **Context**: `tools/count-indented-ids.py` (its `argparse` setup and the report writer) and `tests/test_indented_id_population_8679.sh` (the handoff-authored RED spec; its `INDENTED_ID_COUNTER` env hook and assertions (0)-(6) are the contract). If an assertion is judged genuinely superseded rather than unmet, say so IN THE FILE with a written reason -- do not leave it silently red, and never weaken an assertion to make it pass.

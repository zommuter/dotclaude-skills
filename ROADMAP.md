...
- [ ] [ROUTINE] **Correct the stale `id:6b35` scope table — and add the lint that keeps it honest** <!-- children-of:6b35 --> <!-- id:c480 -->
  - **The defect, exactly**: `ROADMAP.md`'s id:6b35 block has an **"OUT of scope — the 7 NON-eligible hops MUST STAY `model:'haiku'`"** bullet that lists ``` `release:` (~1993, `claim.sh release && heartbeat.sh beat` — two commands) ```. **That is no longer true**: id:f7d3 SPLIT that hop and converted it — `relay-loop.js:2316` now dispatches `release:` as `model: 'bash'`, and `tests/test_release_hop_mechanical_f7d3.sh` asserts it. An implementer working the id:6b35 table faithfully would "restore" `model:'haiku'` and thereby **re-introduce the exact invariant violation f7d3 removed**.

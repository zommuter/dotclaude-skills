...
- [ ] [HARD — pool] Gate-detection (id:3801) must commit its main-checkout ROADMAP edits atomically — uncommitted residue is a self-blocking dirty backlog <!-- id:2147 -->
  ...
  - **Done-check**: tick this checkbox, then `make test` fully green.
- [ ] [HARD — pool] Standalone GPU/intensive background jobs must acquire a relay `resource:<name>` claim so `--intensive` can serialize against them <!-- id:a643 -->

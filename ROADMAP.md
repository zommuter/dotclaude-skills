...
- [ ] [ROUTINE] Margin-aware quota-stop staleness — stop false-stopping a healthy pool when the cache is stale + unrefreshable <!-- id:1d64 -->
...
- [ ] [ROUTINE] Sync relay-loop.js classifier to the canonical `[HARD — pool]` lane token (close the id:78ff drift) <!-- id:3c0f -->
  ...
  - **Done-check**: tick this checkbox, then `make test` fully green.
- [ ] [ROUTINE] Add `references/hard-lanes.md` to the Makefile install manifest + guard every reference doc <!-- id:69ef -->
...
- [ ] [HARD — pool] `roadmap-lint.sh` — a GRAMMAR validator that LOUD-rejects any open ROADMAP item not matching the proper syntax; wire into `review` + `human` <!-- id:09a3 -->

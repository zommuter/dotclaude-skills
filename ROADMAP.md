...
- [ ] [ROUTINE] **`changelog-append.sh` puts a lock file in the target repo's WORKING TREE, so running the changelog deriver permanently dirties the repo and the dirty-guard then silently drops it from every pool round** <!-- id:798b -->
...
- [ ] [ROUTINE] **`RELAY_STATUS.md` drops four non-dispatch classes, so the pool reports a false clean and the owner cannot see why nothing ran** <!-- id:8c85 -->

...
- [ ] [INPUT — decision] **`md-merge.py update-ids` (and `lib-typed-edges.sh`) must resolve an item's OWN id as the LAST `<!-- id:XXXX -->` on the line, never the first** 🚧 RE-LANED [ROUTINE]→[INPUT — decision] by review relay-ckpt-20260819-1449 (executor BLOCKED it 2026-08-19): the spec CONFLICTS with the already-shipped id:6059 design in `meeting/md-merge.py`, which REFUSES any line carrying multiple anchored `id` markers rather than resolving own-id positionally (first-vs-last) at all — so test case (A) (an `update-ids` write aimed at the line's trailing OWN id must APPLY) cannot pass against the intended, implemented behaviour. NOT executor-fixable without gaming (rewriting the RED test, or reverting the code to last-match which id:6059's own docstring argues is unsound). OWNER CALL PENDING: either close cc7e in favour of id:6059, or REDEFINE it to assert the id:6059 refusal and author a fresh RED spec under owner sign-off. Blocked pending that decision. See RELAY_LOG 2026-08-19. <!-- id:cc7e -->

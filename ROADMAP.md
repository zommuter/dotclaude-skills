...
- [ ] [ROUTINE] **PRIMARY — amend `id:c919`'s `workCreated` predicate to VERDICT-CLASS CHANGE, so the drain does not quit on the exact round its verdict would have flipped** <!-- id:907e -->
...
- [ ] [ROUTINE] **Amend `id:cc90`: a chain-end classifier RE-ASK replaces the `chainDepth === K` forced-review trigger, and correct two false-premise sites** <!-- gated-on:1a34 --> <!-- id:8123 -->
...
- [ ] [ROUTINE] **Extract `isDryRound`/`workCreated` into ONE shared definition inside `relay-loop.js`; record the drain-path gap MOOT-BY-RETIREMENT** <!-- gated-on:907e --> <!-- id:6217 -->
...
- [ ] [ROUTINE] **HIGH PRIORITY — `relay-reconcile.sh --integrate` hardcodes a checkpoint label with no model id, so every reconcile-integrate silently leaves `last_strong_ckpt` stale** <!-- id:c500 -->
...
- [ ] [ROUTINE] **`meeting/personas.md` has accreted redundant entries because `merge=union` can never reconcile a re-registration** <!-- id:069b -->
  ...
  - **VERIFIED at promotion** (the TODO quotes 79/52; the reproducible commands give): `grep -oE '\*\*[A-Za-z]+\*\*' meeting/personas.md | wc -l` → **81**, `| sort -u | wc -l` → **54**, so **27 redundant**; `| sort | uniq -d | wc -l` → **18 duplicated names**. **Sage** and **Otto** appear 3× each; **Cal** is an exact byte-duplicate; **Quinn** is at `meeting/personas.md:32` and `:74`. Also verified: `meeting/append.sh:327` routes `-t personas` to `$SKILL_DIR/personas.md` and `:28` documents that path as *"free prose, no validation, no echo"* — it is the sole sanctioned writer and it validates nothing today.
  ...
  - **What to build (three parts)**:
    ...
    3. A **conformance check that FAILS when a name appears twice**, wired into the test suite so the file cannot re-accrete.
  - **Acceptance**:
    - `grep -oE '\*\*[A-Za-z]+\*\*' meeting/personas.md | sort | uniq -d` is EMPTY;
    - every name surviving the dedup carries the union of its variants' provenance tails — assert on at least one 3× case (**Sage** or **Otto**) that all three dates survive;

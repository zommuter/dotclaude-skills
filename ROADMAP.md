...
- [ ] [HARD — pool] **`@wire` grammar + `classify-repo` count + `drained` render-alias** (KEYSTONE — build first; af48 child C1) — define `@wire` as a new **orthogonal marker** (like `@manual`/`@needs-auth`, NOT a lane), recorded by the *executor-verifiable-via-a-host/e2e-RED-spec* property (NOT narrowly "UI wiring"). Three deliverables, all pinned by `tests/test_wire_grammar_classify.sh`:
...
- [ ] [HARD — pool] **Spec-completeness handoff consumer-enumeration aid** (af48 child C4) — a handoff-time listing aid `relay/scripts/consumer-enum.sh <artifact> [root]` that lists every file referencing/reading `<artifact>` (the readers of the artifact a RED spec governs), so a handoff author cannot silently miss a consumer. **A listing aid, not a gate** — it surfaces readers (exit 0, one path per line), it does NOT mechanically prove coverage. Pairs with the discipline (documented in `references/handoff.md`) that a RED spec **names the consumers it covers**.
  ...
  - **Context**: `references/handoff.md` (C3 spec discipline), chidiai `red-spec-verified-named-consumers`. TODO twin id:78df. <!-- children-of:af48 --><!-- id:78df -->

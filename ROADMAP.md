...
- [x] **[HARD] [INTENSIVE — disk-io] Standalone warm-vs-cold copy timing on a build-dep repo (H3)** <!-- id:3c9d -->
  ...
  - **DONE 2026-08-20 (relay HARD child, under an exclusive `resource:disk-io` lease) — results + verdict published in `docs/reflink-warm-vs-cold-timing-2026-08-20.md`.** Measured `zkm-ner` (heavy: spaCy+models, 341 MB `.venv`) and `zkm-stt` (light: pure-Python, 106 MB) on btrfs `/home`. **Verdict: the premise is FALSE at the copy step alone** — `git clone --local` is *faster to copy* (moves only ~2 MB of git objects vs reflink's CoW metadata over the whole tree) — **but CORRECT in substance once tree warmth is priced in**: the reflink win equals the cold-clone rebuild cost avoided, and that cost is dominated by dep weight + uv-cache warmth. **ACCEPT for heavy/network-bound repos** (`zkm-ner`: reflink 0.14 s / 0 exclusive disk vs a 16 s online `uv sync` that is *offline-unsatisfiable*), **REJECT as a blanket justification for light cache-hit repos** (`zkm-stt`: clone+`uv sync --offline` ≈ 0.28 s, same order as reflink 0.05 s). Recommendation to the owner: adopt reflink for the d03d fleet migration **weighted by per-repo rebuild cost, not uniformly** — and note fievel (Raspbian, non-btrfs) can only take the expensive cold path. Owner ratifies the fleet GO/NO-GO (`id:d03d`/`id:ca9e`).

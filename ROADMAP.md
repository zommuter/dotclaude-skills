...
- [x] [ROUTINE] **Cap the mechanical recipe runner: cgroup MemoryMax + no-swap + wall-clock ceiling (`capped-run.sh`)** -- `mechanical-daemon.sh` ran each recipe as a bare `bash -c`, uncapped: no cgroup, no memory limit, no timeout, with only `Nice=10` on the service unit (CPU priority, which does nothing about memory). A local-LLM benchmark recipe (est_wall 4500-5400s, ~20 GB resident) could thrash the machine into unusability with nothing to stop it. `capped-run.sh` is adapted with attribution from toesnail's `docs/dreamed/capped.sh` and carries over its reasoning (why `nice` and `ulimit -v` both fail); it runs in its OWN `relay-mech.slice`, not toesnail's `dreamed.slice`, so a dreaming session and a benchmark cannot share one limit. FAILS CLOSED: no `systemd-run` means exit 3, never an uncapped fallback. The kill deadline is `max(est_wall * 2, 600s)` and deliberately NOT `est_wall` itself, which stays an ADMISSION estimate for `relay-intensity.sh permits`. Failures are classified (137 cap / 124 ceiling / 3 refusal) and the tick summary gained a `failed=` count it lacked -- a failed recipe used to increment nothing, so `ran=0 deferred=0 rejected=0` read identically to an idle tick. Verified empirically: a deliberate 200M-cap breach exits 137 with the machine responsive. Spec `tests/test_capped_run_c057.sh` (12 assertions). — **SCOPE CORRECTION 2026-09-08, same day, from a live failure: this does NOT cover `resource: local-llm` recipes, which is the class that motivated it.** The canary filled RAM and swap and brought the machine to a crawl WHILE the capped scope reported `MemoryCurrent=396439552` (378 MB). Cause: `ai-codebench judge` is an HTTP CLIENT; `llama-swap` (a root service in `system.slice`, `MemoryMax=infinity`, `MemorySwapMax=infinity`) loads the model, and its `llama-server` children sit in ITS cgroup — verified via `/proc/<pid>/cgroup` → `0::/system.slice/llama-swap.service`. So the cap bound a thin client while ~17 GiB loaded outside it. The 378 MB reading was the tell and was misread as 'climbing'. Aggravators: the Intel Arc 140V is an iGPU with SHARED system memory, so `-ngl 99` allocates from the same 30 GB rather than offloading; and the 30B entry uses `--ctx-size 32768` against 8192 elsewhere. `llama-swap` v255 has NO memory limits (`groups`/`exclusive`/`swap` govern co-residency, not usage), so the lever must live at the server — a `systemd-run --scope` wrapper inside the model's `cmd:`, or a `MemoryMax` drop-in on `llama-swap.service` — and both need root. What this item DID deliver stands and is unaffected: in-process recipes are genuinely capped, failures are classified, `failed=` is reported, and the retry timer is installed. Successor item to be filed. <!-- id:c057 -->
- [x] [ROUTINE] **relay-doctor check: trunk vs the remote default branch (informational, not a fault)** -- a repo whose trunk differs from `origin/HEAD` is a SUPPORTED configuration: `trunk-branch.sh:12-13` exists precisely because ai-codebench works on `claude/opusplan` while its `main` is frozen at a June checkpoint, and hardcoding `main` parked a correctly-integrated worktree every round. So this check does not say wrong-branch; it says writes here land off `origin/HEAD`, once, in the health report rather than every turn. Two deliberate silences: NO `origin/HEAD` reports NOTHING (absence is not divergence, and guessing `main` is the id:758a anti-pattern `verify-isolation.sh` refuses), and a `trunk_intentional` declaration in `relay.toml` suppresses it -- but ONLY while it still matches the live trunk; a STALE declaration is reported louder than an undeclared divergence, because an exemption that no longer describes reality hides a real divergence forever. ai-codebench is declared. Spec `tests/test_relay_doctor_trunk_default_f9dc.sh` (9 assertions, hermetic fixtures). <!-- id:f9dc -->
...
- [x] [ROUTINE] **`md-merge.py update-ids` cannot reach a WRAPPED item's continuation lines at all, so the flock'd helper has no compliant path for the commonest ledger edit there is.** -- detail: `docs/ledger-notes/4f0f.md` <!-- relates:3bd4 --> <!-- id:4f0f -->
- [x] **[ROUTINE]** ratify-queue.sh: self-verifying list — report a remote-confirmed pending entry as landed, not pending (Acceptance D) — seam of id:7408 (auto, id:3801) <!-- id:4d65 -->
  - **Acceptance**: ratify-queue.sh's `list` (human/--tsv/--json as applicable) checks each PENDING entry's recorded merge sha against its repo's remote(s) via the same read-only verification path resolve already uses (_verify_remote/_verify_pending), and reports an entry the remote demonstrably carries as landed rather than pending -- WITHOUT mutating the queue file (no auto-resolve; the owner's explicit `resolve` remains the only writer). An entry the remote does not carry still reports pending exactly as today.
  - **Done-check**: new tests/test_ratify_queue_self_verify.sh: plant a queue entry whose merge sha IS present on a test bare remote -> `list` reports it landed, excluded from the pending count; plant a second entry whose merge sha is NOT on the remote -> still reports pending. tests/run-tests.sh tests/test_ratify_queue_self_verify.sh green, then full make test.
  - **Context**: relay/scripts/ratify-queue.sh (list command ~line 400+, reusing _verify_remote/_verify_pending ~line 267-360)
...
- [ ] [ROUTINE] **LOCALIZED (one missing `idx += 1`) -- but SUPERSEDED: a later GREEN iteration of the same work is parked under the misleading name `...execute-8372-0`, 4/4 tests rc=0, only 44 commits behind.** -- detail: `docs/ledger-notes/be51.md` <!-- relates:3bd4 --> <!-- relates:4f0f --> <!-- id:be51 -->
- [x] [ROUTINE] **REFUTED and CLOSED 2026-09-08 (owner) -- `list --tsv` never dropped the pending entry; the test grepped a 40-char sha against a field `main` renders as 12 chars.** -- detail: `docs/ledger-notes/ceca.md` <!-- relates:4d65 --> <!-- relates:95a3 --> <!-- id:ceca -->
- [ ] [ROUTINE] **The `id:2964` fix has TWO halves and only one is pinned: reverting the `_MARKER_RE` shape anchor alone leaves the whole suite green, because every over-match fixture is backticked and the quoted-example mask suppresses it first.** -- detail: `docs/ledger-notes/32ba.md` <!-- relates:2964 --> <!-- relates:8372 --> <!-- id:32ba -->
- [x] **[ROUTINE]** Pin todo-conformance.sh's length measurements to CHARACTERS regardless of locale — seam of id:4839 (auto, id:3801) <!-- id:eccb -->
  - **Acceptance**: Every length/residue measurement site in relay/scripts/todo-conformance.sh (head-line length, shape residue, title length, and the --regen-length-baseline/--regen-shape-baseline writers) returns the identical character count under LC_ALL=C and under LC_ALL=de_CH.UTF-8, matching Python len() on the same string.
  - **Done-check**: tests/test_conformance_length_metric_locale_4839.sh green when run under both LC_ALL=C and a UTF-8 locale; make test green.
  - **Context**: relay/scripts/todo-conformance.sh (the ${#var} measurement sites — head length, shape_residue, title length, and both regen writers)
...
- [ ] **[ROUTINE]** Surface todo-conformance.sh's INERT baseline warnings from relay-doctor.sh, and widen install-drift to cover all of relay_FILES — seam of id:4839 (auto, id:3801) <!-- id:cb9a -->
  - **Acceptance**: relay-doctor.sh's todo-conformance invocation (currently `2>>"$LOG"`) no longer swallows its stderr from the reviewer's own view — the INERT warnings (and any future stderr from that tool) reach relay-doctor's own output, not only the log file. install-drift walks every path declared in relay_FILES (not just scripts/* and references/*), so a missing declared non-script file (e.g. a baseline) is reported.
...
- [x] [ROUTINE] **The parked `id:4d65` attempt is red two ways, BOTH localized and BOTH FIXED: a stale fixture in a neighbouring test, and a one-token typo in its own declaration.** -- detail: `docs/ledger-notes/95a3.md` <!-- relates:4d65 --> <!-- id:95a3 -->
...
- [x] [ROUTINE] 🔴 **`workflow_node_check` returns 0 -- a silent PASS -- on a file it could not read: awk and sed both fail and their status is discarded.** -- detail: `docs/ledger-notes/1b0e.md` <!-- relates:62c9 --> <!-- id:1b0e -->
- [x] [ROUTINE] **`workflow_node_check` refuses a whole file for a line-initial `export`/`import` inside a template literal or a block comment.** -- detail: `docs/ledger-notes/e044.md` <!-- relates:62c9 --> <!-- id:e044 -->
- [x] [ROUTINE] 🔴 **The Workflow runtime is STRICT (probed, 2026-09-08), so the sloppy wrapper in `workflow_node_check` passes strict-only syntax errors that crash the pool.** -- detail: `docs/ledger-notes/ad67.md` <!-- relates:62c9 --> <!-- id:ad67 -->
...
- [x] [ROUTINE] **NARROWED then FIXED: `--all` now reports RETIRABLE RESIDUE (merged worktree / unregistered cache dir) in its own group; id:2b4b's `list_stranded` already covered unmerged ones.** -- detail: `docs/ledger-notes/ba95.md` <!-- relates:2b4b --> <!-- id:ba95 -->

...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract — RESCOPED 2026-07-28 after an adversarial review refuted this item's own fix direction and its RED spec** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  ...
  - **FIX — do (1) NOW in one file; (1) is not blocked on anything.**
    1. **Immediate countermand in the dispatch prompt** (`relay-loop.js` `unitPrompt`): "Do NOT invoke the Skill tool for `relay` — the contract you must follow is the file at `relay/references/executor-contract.md`." The Skill call is triggered by the TARGET repo's `CLAUDE.md` `## Relay contract` section (verified present, loderite `CLAUDE.md:260`); the dispatch prompt never mentions the Skill. A fleet-wide pointer rewrite rides the review cycle and takes as long as the slowest repo — but the countermand lands in ONE file today and covers the dying population (pool children) regardless of fleet state. **Saving: ~26.4k per dispatch** (31.9k → ~5.5k, the contract Read alone).

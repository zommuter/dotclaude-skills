...
- [ ] [HARD — strong model] Integrator can DESTROY uncommitted edits in a repo's main checkout (data loss) <!-- id:aa93 -->
  ...
  - **Fix directions:** (1) make the clean-tree gate DETERMINISTIC + FAIL-SAFE in a non-agent wrapper: `git status --porcelain` before any merge/push; if the tree carries changes the run did not create → DEFER the repo and surface it, never force-clean. (2) The integrator must NEVER run `git stash` / `git checkout --` / `git reset --hard` / `git clean` on a main checkout to make room. (3) `git-lock-push.sh`: refuse (or skip) a foreign-dirty tree on the autostash path instead of stashing it. (4) Structural fix remains id:3558 (worktree-per-session merge), but (1)+(2) are the cheap interim guard.

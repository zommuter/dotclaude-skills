...
- [ ] [ROUTINE] **drain-driver run-heartbeat wiring (id:e149 parity)** (BLOCKED on id:cd7a — driver skeleton first) <!-- children-of:93fe --><!-- gated-on:cd7a --><!-- id:f9d2 --> — the driver mints a runId matching the watchdog namespace glob (`relay-drain-<ts>-<pid>` — MUST match `relay-*`, the `--prefix` the watchdog/reap consumers scope by), calls `heartbeat.sh beat` before round 1 and once per round, `heartbeat.sh stop` on every clean exit; crash detection itself stays heartbeat.sh's already-tested TTL contract.

...
- [x] **[HARD]** Build relay/scripts/integrate.sh — standalone mechanical integrator (all 11 deterministic steps, fail-closed LOUD), tested in isolation — seam of id:a955 (auto, id:3801) <!-- id:9e50 -->
  - **Acceptance**: integrate.sh runs the full sequence (lease-release -> clean-tree gate -> verify-isolation -> sync-origin -> merge --no-ff -> version-bump -> changelog-append -> ckpt-tag -> git-lock-push -> worktree-retire -> state-write) in a hermetic mktemp fixture repo; a forced nonzero injected at >=3 distinct steps each yields a DISTINCT nonzero exit + a loud handback message and leaves main unmoved; the id:aa93 defer-don't-clean rule (no stash/checkout--/reset--hard/clean on foreign-dirty main) and the id:6e02 cleanup-scope rule (remove ONLY this unit's own worktree+branch) are enforced IN-SCRIPT, fail-closed, not restated in a comment; the semver user-observable judgement is left as an explicit input parameter/hook, not embedded. Does NOT touch relay-loop.js.
  - **Done-check**: new tests/test_integrate_sh_mechanized.sh passes (asserts forced-failure at >=3 distinct steps + aa93/6e02 enforcement); existing tests/test_relay_integrate_contain.sh and tests/test_integrate_ckpt_merged_tip.sh remain green unmodified
  - **Context**: relay/scripts/integrate.sh (new); tests/test_integrate_sh_mechanized.sh (new); reuses relay/scripts/{verify-isolation.sh,sync-origin.sh,version-bump.sh,changelog-append.sh,ckpt-tag.sh,worktree-retire.sh,relay-state-write.sh} and git-diary-workflow/git-lock-push.sh
- [ ] **[HARD]** Rewire relay-loop.js integrate() to dispatch integrate.sh via the mechanical hop (id:6176 relay-mech/MECH_MODEL), leaving only the semver micro-hop as an LLM agent (after id:seam-1) — seam of id:a955 (auto, id:3801) <!-- id:087b -->

...
- [ ] [ROUTINE] Em-dash delimiter migration S4 — convert the em-dash lane-tag match patterns in the 12 remaining `relay/scripts/*.sh` readers (incl. `lane-convert.sh`'s own `[INTENSIVE — ]` regexes and the `todo-conformance.sh` `'[HARD-'` defect) to the two-delimiter alternation; do it in two batches of 6 files. <!-- gated-on:2ee5 --> — **BATCH 1 of 2 LANDED 2026-08-31** (`f035a5bc`, merged `ad0377c8`): `classify-repo.sh`, `lane-convert.sh`, `todo-conformance.sh`, `unpromoted-scan.sh` are converted; the `unpromoted-scan.sh` promote-disposition defect in the executor residue was fixed before merge and is covered by case (p) of `tests/test_unpromoted_scan_newvocab.sh`. **REMAINING: the other 8 readers**, tracked as `id:6bf5`. Two executor children died on `Prompt is too long` attempting all 12 files in one session -- this item MUST be worked in the two batches of six its acceptance already prescribes, never as one unit. <!-- children:6bf5 --> <!-- id:e8d4 -->
- [ ] [ROUTINE] Em-dash delimiter migration S4 BATCH 2 — convert the REMAINING 8 `relay/scripts/*.sh` readers to the two-delimiter alternation (batch 1 landed the first 4 as part of `id:e8d4`). Work these EIGHT ONLY: attempting all 12 in one session killed two executor children with `Prompt is too long` (2026-08-31, run relay-20260831-190008-13032). Pattern to follow: enumerate BOTH spellings, and change every downstream comparison in the same commit — the batch-1 residue updated `unpromoted-scan.sh`'s tag list but not its promote-disposition regex, which silently re-created the lodelore verdict-invisibility bug (id:4b64). <!-- gated-on:2ee5 --> <!-- id:6bf5 -->

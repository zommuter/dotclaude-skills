...
- [x] [ROUTINE] **LOCALIZED (one missing `idx += 1`) -- but SUPERSEDED: a later GREEN iteration of the same work is parked under the misleading name `...execute-8372-0`, 4/4 tests rc=0, only 44 commits behind.** -- detail: `docs/ledger-notes/be51.md` <!-- relates:3bd4 --> <!-- relates:4f0f --> **CLOSED 2026-09-08 (review, run relay-20260908-174448-4421):** the superseding green iteration LANDED -- `8b40b1fc` is an ancestor of HEAD via merge `e834375a`, the `...execute-8372-0` branch no longer exists in `git branch -a`, and the four md-merge tests run rc=0 in seconds at HEAD (`test_md_merge_item_scope_4f0f.sh`, `test_md_merge_silent_noop_3bd4.sh` both rc=0; full suite 603/0/0/7-expected-red). The hang had no residue to carry forward. <!-- id:be51 -->

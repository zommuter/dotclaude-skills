...
- [ ] [ROUTINE] **Record the ACTUAL child death cause — relay child deaths are currently untracked** (promoted from TODO `routed:9c91`) <!-- children-of:93cc --> <!-- id:61fa -->
  ...
  - **Why [ROUTINE]**: read-side parsing of a file that already exists, plus two existing sinks; no contract or model change.
...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract (~9.5k tok wasted every dispatch)** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  - **Defect, REPRODUCIBLE at n=2** (independent dispatches, not an anomaly): `Skill relay executor` already loads the contract (+26,674 / +26,425 tok), then the child Reads `executor-contract.md` + `conventions.md` **again** (+9,505 / +9,473 tok). ~36k combined fixed contract overhead ≈ 20% of budget, leaving under 90k working room.
  - **Fix**: load the contract ONCE — either Skill-only (drop the child's Reads) or drop the Skill in favour of the Reads. Pick one and say which in the close note; do not leave both paths live.
  - **Also audit while there**: the pre-first-tool-call baseline measured 57,559 / 55,818 tok (system + dispatch prompt). Report the number; a reduction is welcome but not required by this item.
  - **RED spec**: `tests/test_contract_single_load_9eb7.sh` (`# roadmap:9eb7`) — assert the dispatch path references the contract exactly once (no Skill-load AND Read-load of the same file in one child prompt).
...
- [ ] [ROUTINE] **Teach the executor contract symbol-level exploration — it never mentions LSP / Grep / Glob** (promoted from TODO `routed:59b2`) <!-- children-of:93cc --> <!-- id:6f1c -->
  ...
  - **BOTH halves are required** (the item is explicit): teach the cheaper tools AND a **bounded survey**. The unfocused survey is the other half of the defect — a child that ranges over 8 ids cheaply still dies, just later. Pairs with id:b09e, which removes the need to survey at all.
...
- [ ] [ROUTINE] **ONE shared ledger-only-diff predicate — three consumers now want it, including the isolation gate that just false-handed-back** <!-- id:88f0 -->
...
- [ ] [ROUTINE] **Build the opt-in proxy stdin channel** (`` ```relay-mech-stdin ``) — carries the id:a05c option-B ruling <!-- children-of:a05c --> <!-- id:33b2 -->
  ...
  - **Why [ROUTINE] despite touching security code**: the design decision is already ratified (id:a05c option B); this is a bounded implementation with a pinned acceptance set and an explicit non-loosening constraint.
  - **Unblocks**: id:d4ca (`write-relay-status`) and id:e405 (the other two payload-trapped hops).

- [ ] [ROUTINE] **Convert the two remaining payload-trapped haiku hops once the stdin channel exists** 🚧 GATED (DEP: id:33b2) <!-- children-of:6b35 --> <!-- gated-on:33b2 --> <!-- id:e405 -->

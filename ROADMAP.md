...
- [ ] [ROUTINE] Review→execute chaining within a pool — **emit a `review→execute re-enqueue` EVENT into `relay-events.jsonl`** (RE-LANED from `[INPUT — access]` 2026-08-14, `/relay human .`, OWNER-DECIDED: "Mechanize it — emit an event") — see TODO.md <!-- id:b8ae -->
  - **Why the lane changed**: the code shipped 2026-06-13 and the only remaining work was a HUMAN OBSERVATION — watch for the re-enqueue log line on the next qualifying pool run. As of 2026-07-02 no such evidence existed in `RELAY_LOG`/diary/events, and it still did not on 2026-08-14 — **six weeks of pools with nobody catching it**. That is the standard failure of observe-by-human, and the mechanize-first rule applies: emit the event so the next occurrence is RECORDED rather than needing someone to be looking.
  - **Acceptance**: when a review child reports `routine_open > 0` and `runUnit` re-enqueues an `execute` unit into the live queue, a `fallthrough`-style event is appended to `relay-events.jsonl` naming the repo, the re-enqueued verdict, and that it was a re-chain — so the item can close on evidence. The existing `!unit.rechained` single-hop guard must remain untouched (an execute never re-chains; this preserves the D3 review-after-execute invariant).
  - **Note the open possibility this closes either way**: six weeks of silence is weak evidence the chaining may not fire at all. If the event never appears on a run that clearly qualifies, that is a DEFECT finding, not a pending observation — say so rather than waiting longer.
...
- [ ] [ROUTINE] **Run the burn measurement and publish the per-phase ranking (measurement half of `id:87f5`)** <!-- children-of:87f5 --> <!-- gated-on:87f5 --> <!-- id:4438 -->
  - Filed 2026-08-14 (`/relay human .`) as the mechanical half of the `id:87f5` re-lane. **Gated on `id:87f5` deliberately and non-negotiably**: this item may not begin until the pre-registration fixes `n`, the per-phase attribution requirement, and the promote-lever threshold X%. A threshold chosen after seeing the ranking is not a pre-registration, and the whole point of `87f5` is that it orders two levers (`id:a955`, `id:3ca7`) that target DIFFERENT phases — which a whole-run total-burn ranking provably cannot do.
  - **Acceptance**: a published **per-phase** ranking over the pre-registered `n` runs, evaluated against the pre-registered threshold, that states an ORDER for `id:a955` vs `id:3ca7`; plus an explicit reconciliation against the banked **47.6% discover baseline** (agreeing or disagreeing with it in numbers, not ignoring it).
  - **Context**: the data side largely exists — `relay/scripts/relay-burn.sh report` (per-bucket %/h, $/h, segmented at resets) and `relay/scripts/relay-econ.py` (per-agent cost, cache-accurate, plus parallelity-weighted time by category over all retained runs). Verify what per-phase attribution they already emit before writing anything new; a prior finding was that status+scaffold are a larger share of wall-clock than of cost, which is exactly the kind of phase split this ranking must preserve.
- [ ] **[HARD] [INTENSIVE — disk-io] Standalone warm-vs-cold copy timing on a build-dep repo (H3)** <!-- id:3c9d -->
  - **PROMOTED 2026-08-14 (`/relay human .`) to make the owner's "pool runs it as [INTENSIVE]" ruling actually true.** Re-laned from `[HARD — hands]` in the same pass. The TODO twin alone could never be dispatched: `gather-repo-state.sh`'s `top_intensive` extraction scans **`ROADMAP.md` only**, so a TODO-only `[INTENSIVE]` item is invisible to the dispatcher no matter how it is tagged. Promotion is the step that was missing, not the tag.
  - **Measure-only**, no code change: `cp -a --reflink=always` vs `git clone --local --no-hardlinks` on a repo with **real build deps** (a zkm plugin, or llm-from-scratch). dotclaude-skills has no deps and structurally cannot test this — that is the whole reason the item exists separately from the pilot.
  - **Acceptance**: published warm and cold timings for both methods on a real build-dep repo, enough to accept or reject the reflink economic premise, plus a stated verdict on it. Gates the d03d FLEET migration and now also `id:ca9e` (the single-repo pilot was deferred onto this in the same pass), so it is the head of the d03d chain.
  - **`[INTENSIVE — disk-io]` is operative, not decorative**: it must run serially-alone under an explicit `--intensive` opt-in, holding an exclusive `resource:disk-io` claim — copy timings taken while a relay pool is churning the same disk are noise.
...
- [ ] [ROUTINE] **`md-merge.py update-ids` (and `lib-typed-edges.sh`) must resolve an item's OWN id as the LAST `<!-- id:XXXX -->` on the line, never the first** <!-- id:cc7e -->
  - **Acceptance**: for a ledger line whose BODY quotes `<!-- id:AAAA -->` before its own trailing `<!-- id:BBBB -->` — (A) an `update-ids` write aimed at `BBBB` MUST apply (the line is addressable); (B) an `update-ids` write aimed at `AAAA` (only a quoted body id, no line's own id) MUST NOT match that line — it fails LOUD (exit non-zero, names `AAAA`) and writes NOTHING, rather than silently rewriting the wrong line. The silent mis-route (B) is the load-bearing negative case: `AAAA` is owned by a closed archived item, so a legitimate write to it must never land here.
  - **Tests**: `tests/test_md_merge_own_id_last.sh` (`# roadmap:cc7e`) — RED as of this handoff. Verified RED against the unmodified tree (`update-ids` reports `BBBB` unmatched because `re.search` resolves the own id as the quoted `AAAA`), and non-vacuous: goes GREEN once the own-id resolution AND `_validate_replacement`'s marker check both use the LAST marker.
  - **Done-check**: `tests/run-tests.sh tests/test_md_merge_own_id_last.sh`, then full `make test` after ticking.
  - **Context**: the bug is TWO first-vs-last sites in `meeting/md-merge.py` — `md-merge.py:263` (`re.search` → own-id) AND `_validate_replacement` (`_ID_MARKER_RE.search(new_line)`, `md-merge.py:184`), which is why fixing only line 263 leaves case (A) red on the marker-guard. `re.findall(...)[-1]` at both. The convention is already settled and shipped elsewhere: `gather-human-backlog.sh` was fixed to last-marker at `d7727be`; `relay/scripts/lib-typed-edges.sh:22` `typed_edges_own_id_of_line()` still uses `| head -1` (change to `tail -1`) — apply the same fix there for consistency (the third site named in the TODO item). Found 2026-08-14 by `/relay human` when an annotation for `id:7756` failed with "unmatched id(s) not found" despite the item being present (its line quotes an id-comment for 2f81 before its own trailing id-comment for cc7e (both written WITHOUT the literal marker syntax here on purpose — spelling them would reproduce the very trap this item describes, since md-merge scans every line, not just item lines)). Relates `id:7756` (produced the colliding token), `id:2f81`. Reverse-handoff of the TODO `id:cc7e` item filed this window.
...
- [x] [ROUTINE] **LANDED 2026-08-19 — measured, not assumed.** **Parallelise `tests/run-tests.sh`** <!-- id:f69b -->
  - **✅ RESULT: 374s → 69s (5.4x), triple identical at 442 passed / 0 failed / 3 expected-red.** Delivered in two steps, and the FIRST one was not parallelism: (1) `relay-doctor.sh --only <check>` scoped mode took `test_relay_install_drift_check.sh` from 106s to 0.56s, dropping the serial suite 374s → **294s (-21%) with no concurrency at all**; (2) the parallel harness then took 294s → **69s and 95s** across two runs. Measured by the reviewer on the integrated tree, contention recorded: idle 75-77%, run queue 2-3, loadavg ~4.5-4.9 (which again read 'busy' on a 3/4-idle box — see the instrument note above). **A third change was REQUIRED and is the interesting one**: parallelism exposed `test_lean_toolchain_drift.sh` as non-hermetic — it never overrode `HOME`, so the doctor read the real `~/src`/`~/.claude`/`~/.config/relay`, and it asserted a DELTA in the doctor's GLOBAL issue counter across four runs assuming the environment was constant. It was not: other sessions move that counter continuously, so the test could flake SERIALLY too; `-j 8` merely made it reliable (`base=473, got 474`, green on re-run). Fixed by scoping the doctor to the one check (so the counter IS that check's contribution — an absolute 0/1, not a delta) and sandboxing every root. **Acceptance, all met**: same triple as serial (verified across 2 parallel + 1 serial run); `-j 1` byte-identical to the previous harness (diff, empty); output emitted in FILE order so failure blocks stay contiguous and diffable; EXPECTED-RED semantics, hooksPath neutralisation, subset-args, summary/failed lines and exit 0/1 all preserved; nested runs forced serial via `RUN_TESTS_NESTED=1` (the 10 recursive tests cannot spawn nested pools); scheduling is longest-first from a LEARNED duration cache outside the repo, so there is no hardcoded slow-test list to rot; the runner exits 2 if the scheduler ever leaves a test unexecuted, because a silently-skipping harness would otherwise report a clean pass.
  - **Structural finding (from reading the code, and this part IS verified)**: `tests/run-tests.sh:48` is a strictly serial `for` loop over 429 test files, one `bash "$f"` per file, on an 8-core box. Full-suite wall clock is ~10-12 min, which makes it too slow to run per-change; today that cost produced two abandoned verification runs before one completed.
  - **Parallelism is safe by construction here, not a gamble**: 324 of 429 tests already `mktemp -d`, and hermeticity is a MANDATED contract in `CLAUDE.md` (own `HOME`/`DEST_DIR`, never touch `~/.claude`, no network). And the harness ALREADY buffers each test's output into `$out` before printing it as a block — the normally-fiddly part of parallel reporting is done.
  - **MEASURE FIRST — this item's premise is unproven and must not be assumed.** An attempted benchmark on 2026-08-14 was INVALID: it ran concurrently with a serial suite the author wrongly believed had died (`pgrep -c run-tests` matches the process NAME `bash`, not the script path — `-f` was needed), so both competed for the same 8 cores. No trustworthy speedup number exists. Establish a clean baseline (serial wall clock, machine otherwise idle) and a clean parallel wall clock before committing to the change; if the win is small, say so and close this rather than shipping complexity for it. Bound the expectation honestly: the longest single test is the floor, so the ceiling is well under 8×.
  - **Acceptance**: the parallel harness reports the SAME pass / fail / expected-red tallies as the serial one on an unchanged tree (assert the triple, not just the exit code — a harness that silently skips tests also "passes"); per-test output stays unmangled on failure; `EXPECTED-RED` semantics are preserved exactly (a failing file whose `# roadmap:XXXX` item is still unticked does not fail the suite); and a measured before/after wall clock is recorded in the commit message.
  - **Watch out**: 9 tests invoke `run-tests.sh` RECURSIVELY (`test_roadmap_tick.sh`, `test_scan_routed_apply.sh`, `test_mechanical_proxy.sh`, and 6 more) — a nested harness under an outer parallel pool can oversubscribe badly. Decide deliberately whether nested runs stay serial. Job count should default to `nproc`, be overridable, and 1 must remain a valid setting that reproduces today's behaviour exactly.
  - **🔴🔴 ESCALATED TO HIGHEST PRIORITY 2026-08-18 (owner directive, verbatim: "file fixing that timeout and suite duration as highest priority to be fixed").** This supersedes the item's previous ordering, not its content — the MEASURE-FIRST discipline below still binds, and the escalation is emphatically NOT permission to skip it.
  - **NEW DATA POINT 2026-08-18, recorded WITH its load context because without that it is worthless:** a full `tests/run-tests.sh` was killed by a **10-minute** timeout. At that moment `uptime` reported **load average 26.39** (1-min), with a `lake build` (pid 3813724) and 7 concurrent `claude` processes on an 8-core box. **What this establishes and what it does NOT:** it establishes the suite exceeds 10 min *under heavy contention*, which is itself the operational problem — the suite is unusable exactly when a relay pool is running, which is most of the time. It does **NOT** establish the clean-load duration, and must not be quoted as if it did. Two earlier runs the SAME day completed comfortably inside the same 10-min budget (438 passed / 0 failed / 3 expected-red) at load ~2.5.
  - **DATA POINT 2026-08-18 evening (3 full runs, recorded WITH load context per the rule above — and explicitly NOT offered as the clean baseline this item demands).** Three complete `tests/run-tests.sh` runs finished in **~6.5 min, ~7 min and ~5 min** wall clock (440 passed / 0 failed / 3 expected-red on the last two). Context at the time: `load average 3.5–4.7` with **9 concurrent `claude` processes; NO `lake build`, and no Lean work of any kind — this repo contains no `.lean`/`lakefile`/`lean-toolchain`.** ⚠️ **CORRECTION, and it is the third instance of this item's own signature error**: an earlier revision of this bullet reported "a `lake build` live throughout (pid 1221281)". That was FALSE — `pgrep -f "lake build"` **SELF-MATCHES**, because the shell command line running the probe itself contains the literal string, so it returns its own pid and looks like a hit. Verified by `ps -eo pid,args | grep -E "[l]ake |[l]ean --"` (bracket-class, self-match-proof), which shows no such process. This is the same family as the `pgrep -c run-tests` matching the process NAME confound recorded above: **a process probe that can match the probe itself is not evidence.** Use a bracket class (`pgrep -f "[l]ake build"`) or `pgrep -x`, and prefer `ps | grep '[x]'` when it matters. **What this establishes**: under MODERATE contention the suite is ~5–7 min, i.e. the 10-min-timeout observation above came from the load-26 spike specifically, not from the suite's ordinary cost — the two data points are consistent with each other and with the ~5.2 min figure in session memory. **What it does NOT establish**: the quiet-box serial baseline, because a `lake build` and 9 claude processes were running the whole time. So this NARROWS the target (the honest estimate for the serial baseline is now ~5 min, not ~10-12) without satisfying the measure-first gate. **Consequence for this item's framing**: the headline "~10-12 min" in the structural finding above is likely an over-estimate drawn from contended runs; re-derive it on a quiet box before quoting it as the thing parallelism must beat, or the speedup will be measured against a number the serial harness never actually costs.
  - **✅ MEASURED 2026-08-18 on a genuinely quiet box — and the result REDIRECTS this item.** Serial baseline: **374s (6m14s)**, 441 passed / 0 failed / 3 expected-red. Contention context, recorded per the rule below: 81% CPU idle before, 77% during, run queue 1-2, 5 idle interactive `claude` processes, **no** lake/lean. Per-test wall clock for all 444 files sums to 371.1s, i.e. the harness overhead is negligible and the loop is ~100% serial. **Instrument warning, third instance of this item's signature error: `uptime` ALONE IS THE WRONG INSTRUMENT.** Load average read 3.30 while the CPU was 81-85% IDLE with a run queue of 1 — load decays over minutes and counts uninterruptible sleep, so it says "contended" on an idle box and would have wrongly blocked this benchmark. Use instantaneous idle (`vmstat`) + the runnable count from `/proc/loadavg`; keep `uptime` only as a coarse cross-check. (The other two instances: `pgrep -c run-tests` matching the process NAME, 2026-08-14; and `pgrep -f "lake build"` SELF-MATCHING on the probe's own command line, same day as this measurement.)
  - **🔺 THE PREMISE IS NOW MEASURED AND IT IS LARGELY WRONG — parallelism is NOT the cheapest lever here, and the "~10-12 min" figure above is an over-estimate from contended runs.** The distribution is extremely skewed: **355 of 444 tests finish in under 0.5s**, while **ONE test is 27.8% of the whole suite**:
    | test | wall | share |
    |---|---|---|
    | `test_relay_install_drift_check.sh` | **103.2s** | **27.8%** |
    | `test_chain_end_reask_mechanical.sh` | 37.1s | 10.0% |
    | `test_make_test_files.sh` | 26.2s | 7.1% |
    | `test_classify_repo_large.sh` | 16.1s | 4.3% |
    | *(top 4 combined)* | *182.6s* | *49%* |
    **Consequence for parallelism: the Amdahl floor is 103.2s**, so no job count can beat ~3.6x, not the ~8x an 8-core box suggests. **Root cause of the 103s, isolated by component timing**: the test stages a synthetic install root (`make install-relay` — only **272ms**, irrelevant) and then invokes **`relay-doctor.sh` TWICE at 51s each**. The doctor's 51s is five full-ledger scanners it runs unconditionally — `classify-repo.sh --emit unit` 18.8s, `unpromoted-scan.sh` 12.0s, `todo-conformance.sh` 10.4s, `roadmap-lint.sh` 5.0s, `orphan-scan.sh --cross-ledger` 3.0s — **none of which this test asserts anything about**. It only needs the id:1102 install-drift check.
  - **RECOMMENDED ORDER (owner decides).** (1) Give `relay-doctor.sh` a scoped mode (e.g. `--only install-drift` / `--check 1102`) and have `test_relay_install_drift_check.sh` use it: a small, low-risk change that should cut ~100s, taking the suite **374s → ~271s (-27%) with no concurrency machinery at all**, and it also makes the doctor usable interactively. (2) THEN parallelise — because step 1 moves the Amdahl floor from 103s to 37s, which is what makes parallelism actually worth its complexity (~271s → plausibly ~50-70s). Doing (2) first buys much less and locks the design around a floor that step (1) removes. (3) Consider whether `relay-doctor.sh` should be fast by default at all — 51s for a routine health check is itself a usability defect, and it is invoked outside the suite too.
  - **DO NOT benchmark under contention — that error is already in this item's own history.** The 2026-08-14 attempt was invalidated because it ran against a serial suite the author wrongly believed dead (`pgrep -c run-tests` matches the process NAME `bash`, not the script path; `-f` was needed). Today's timeout is the same confound wearing different clothes. Any speedup measurement must record `uptime` + `pgrep -f 'lake build'` + `pgrep -fc claude` BEFORE and AFTER, and be taken on a quiet box, or it measures the box rather than the harness.
  - **🔓 INTERLOCK CLEARED 2026-08-18 — `id:2799` is FIXED, so this item is now dispatchable; the history below is kept because it explains why the escalation initially could not start.** (Was: this item could not be DISPATCHED, so raising its priority alone did not start it.) `dotclaude-skills` classifies `verdict:execute` with `actionable_routine_open:6` = `b8ae`, `4438`, `cc7e`, `f69b`, `5bef`, `dd7d` — **`f69b` is in that set** — but the unit is also stamped `intensive:"disk-io"` (inherited repo-wide from the unrelated `[HARD] [INTENSIVE — disk-io]` `id:3c9d`), so a bare `--afk` pool defers the whole unit and works none of the six. Verified live in run `relay-20260818-152657-28729`. **Three ways to actually start this item:** (a) fix `id:2799` first so the routine unit dispatches normally — the clean fix, and it unblocks five siblings too; (b) run `/relay --intensive` — dispatches today, at the cost of running this unit serially-alone under an exclusive `resource:disk-io` claim it does not need; (c) work it in an attended session. Priority ordering (`--priority`) does NOT help: it reorders within a verdict class and cannot rescue a unit from the intensive partition.
  - **Dispatch UNBLOCKED 2026-08-18 (`id:2799` fixed + verified).** `classify-repo.sh --repo dotclaude-skills` now returns `verdict:"execute"`, `intensive:""`, `priority_rank:1` over the same six ids — it returned `intensive:"disk-io"` before, which is what deferred the whole unit. So a plain `/relay --afk` pool will now dispatch this repo's routine unit, and `f69b` is reachable without `--intensive`. Option (b) from the line above (running `--intensive` purely to escape the partition) is no longer needed, and option (a) is done.
...
## Human-triage promotion 2026-08-18 (`/relay human .`, owner-decided)

- [ ] [ROUTINE] **Author the systemd units + hardening for the two relay service users (authoring half of `id:8e7a`)** — see TODO.md for the full spec <!-- children-of:8e7a --> <!-- id:5bef -->
  - **PROMOTED 2026-08-18 by owner decision (`/relay human .`), reusing its existing TODO id** (single-id-two-views; no duplicate minted). **This promotion is the entire fix** — the OS-users cluster was dead-stopped by a ROUTING gap, not a technical one: the owner split `id:8e7a` on 2026-08-14 into this authoring half plus a run residue, and `8e7a` is `gated-on:5bef` — but `5bef` existed in `TODO.md` **only** and had never reached the execution queue, so the pool could not see it and `8e7a` was gated on work nobody had scheduled.
  - **Verified 2026-08-18 before promoting** — the prerequisites really are discharged, and the older "nothing is provisioned" framing is stale: `relay-ro` (uid 1002) and `relay-svc` (uid 1003) exist with their own primary groups, `0700` homes and linger enabled; the ACLs are applied and `getfacl`-verified including the negative constraints; `id:13ae`, `id:02c7` and `id:5937` are all `[x]` in `TODO.archive.md`. What is genuinely absent is the unit migration: zero relay units under `/etc/systemd/user/`, and the daemons still run as the login user from unit files carrying no `User=`, no `EnvironmentFile=` and none of the hardening directives.
  - **Scope is AUTHORING ONLY — never install, never invoke sudo, never touch the real `/etc/systemd/user/`.** Write the unit files with `User=` and the hardening directives (`ReadWritePaths` / `ReadOnlyPaths` / `ProtectHome` / `NoNewPrivileges` / `PrivateTmp`), using LITERAL absolute paths rather than `%h` (the recorded F3 trap — `%h` resolves to the *service user's* home once `User=` is set); the shared `EnvironmentFile` enumerating every `$HOME`-relative override the scripts read (F1); a loud `EUID`-is-a-relay-service-user guard in the producer and daemon entrypoints; and the `make install-*` target the human will run.
  - **Acceptance** (already written on the TODO twin, unchanged): a hermetic test asserts the emitted unit **content** against a fixture — literal paths present, carve-outs present, uid assertion present — while never installing to the real unit directory and never invoking sudo. Full `tests/run-tests.sh` green, then tick.
  - **Out of scope**: installing/enabling the units and the two runtime verifications — those are `id:8e7a`, the human's half. Also out of scope: `id:e8a3`, whose author-then-run split the owner **explicitly declined** on 2026-08-14 ("I'll build and run it end to end") — do not re-propose it. Run `id:e8a3` BEFORE the unit migration: it is the clean negative control, since afterwards an ACL failure and a `ProtectHome` mount-namespace failure are indistinguishable.
  - **Drift found in passing, not covered by any item**: `heartbeats.done/` and `inject.d/` under the relay config dir carry no ACL at all and `apply-relay-acls.sh` does not cover them — a service-user write there will fail once the daemons switch uid. Fold into `id:e8a3`'s run or raise a follow-up before the switch.

## Re-dispatch stranded-branch guard (owner-decided 2026-08-18, inbox `routed:d99c` + `routed:678c`)

- [ ] [ROUTINE] **Re-dispatch of an item that already has a prior attempt's branch carrying commits must REFUSE and surface, and integrate must compare against any sibling branch for the same item.** Reuses the existing TODO id (single-id-two-views; no duplicate minted) — `TODO.md` stays the prose SSOT for the incident trail, this item is the executor spec. <!-- routed:d99c --> <!-- routed:678c --> <!-- id:dd7d -->
  - **Owner ratified the disposition 2026-08-18: build (b) + (c). Option (a) — hand the child the stranded branch — is explicitly NOT adopted**: it re-opens the uncommitted-state hazard `id:9834` closed (`relay-loop.js:3113-3115` states that rationale in-file). Do not implement (a), and do not "helpfully" reuse a parked worktree anywhere in this unit.
  - **The incident** (lodelore run `relay-20260818-110858-18948`, `id:15d2`): round-4 dispatch handed back on a dirty-tree defer (`id:aa93`), round 5 re-dispatched the same still-open item, and the second child redid the work from scratch — reaching inner/middle/outer = 3/6/12 in-game days against the stranded branch's 6.337/12.675/25.350, a factor 2.11 apart, i.e. two different planet masses at identical semi-major axes. The two children were **SEQUENTIAL, never concurrent** — so an "at-most-one live child per id" guard would not have prevented it and is NOT the fix. Nothing detected the divergence: the per-item review verified the merged one "genuinely green, spec unweakened" and had no way to see the sibling; it surfaced only as an add/add git conflict during a manual integrate.
  - **Build a new script `relay/scripts/stranded-branch-scan.sh`** — observe-only, mechanized (never an LLM judgement), usage `stranded-branch-scan.sh <repo-path> --verdict <v> --item <itemId> [--base <ref>]`. It prints one `<branch>\t<commit-count>` line per branch for that item carrying **>0** commits vs base, and prints nothing when there are none. Exit 0 whether or not it finds any (observe-only, like `verify-isolation.sh`); reserve non-zero for a usage/repo error. It NEVER deletes, renames, checks out or merges anything.
  - **Naming contract, VERIFIED 2026-08-18 against the code — do NOT re-derive it:** `relay-loop.js:2219` `unitKey = ${verdict}-${itemId || 'repo'}-${attempt}`; `:2228` `branchFor = relay/${runId}-${unitKey}`; `worktree-retire.sh:80-81` parks as `relay/orphan/$(basename <worktree-dir>)`, which is that same `<runId>-<verdict>-<itemId>-<attempt>` stem. So scan BOTH namespaces with a **runId-AGNOSTIC** glob: `relay/*-<verdict>-<itemId>-*` and `relay/orphan/*-<verdict>-<itemId>-*`.
  - **Run-id-agnostic is required, not a nicety** — `branchFor` is runId-PREFIXED, so a same-run-only glob would catch the lodelore case and miss every branch stranded by an EARLIER run, which is the longer-lived half of the hazard.
  - **Three traps, each already paid for once:** (i) `itemId` falls back to the LITERAL string `'repo'`, so a repo-scoped unit is `…-review-repo-0` and CANNOT be item-matched — the guard fires only for item-scoped dispatches, and that limit must be STATED in the surfaced message rather than silently narrowing it (the `review-repo-0` branch in the `routed:678c` report is exactly this shape). (ii) Do NOT build the matcher against `relay/orphan/15d2-kepler-derived-periods` — that name was a HUMAN hand-rename during manual integration, not the programmatic form; a matcher fitted to it would never fire on a real one. (iii) Branch presence is NOT sufficient — gate on `git rev-list --count <base>..<branch>` > 0, because a zero-commit leftover is also exactly what a LIVE parallel child's fresh worktree looks like (`id:6e02`).
  - **Wire it at BOTH points — a built-but-unreferenced script is the `id:5367`/`id:2062` failure mode and does not close this item.** (b) pre-dispatch in `provisionWorktree`/`runUnit`: on a non-empty scan, do NOT dispatch — record a handback naming every branch found and its commit count, so a human dispositions it. (c) at integrate: on a non-empty scan for the item being merged, surface the sibling branch(es) LOUDLY in the integrate report rather than relying on git conflict luck — a divergence touching DIFFERENT lines merges silently and this is the only thing that would catch it.
  - **Acceptance (BDD)**:
    - GIVEN a repo with branch `relay/relay-OLDRUN-execute-dd7d-0` holding 1 commit beyond main WHEN `stranded-branch-scan.sh <repo> --verdict execute --item dd7d` runs THEN it prints that branch with count 1, and exits 0.
    - GIVEN the same item's branch parked as `relay/orphan/relay-OLDRUN-execute-dd7d-0` THEN the scan finds it too (both namespaces).
    - GIVEN a branch for the item with ZERO commits beyond base THEN the scan does NOT print it (trap iii).
    - GIVEN only a repo-scoped branch `relay/relay-OLDRUN-review-repo-0` WHEN scanning `--item dd7d` THEN the scan prints nothing (trap i — no false item-match).
    - GIVEN a stranded branch from a DIFFERENT runId than the current one THEN the scan still finds it (run-id-agnostic).
    - GIVEN `relay/scripts/relay-loop.js` THEN it references `stranded-branch-scan.sh` at BOTH the pre-dispatch and the integrate site (wiring, not just existence).
  - **Done-check**: tick this checkbox, then `tests/run-tests.sh` fully green (`tests/test_redispatch_stranded_branch_dd7d.sh` is the RED spec and must go green).
...
## Parallel test-suite load-flakes exposed after id:f69b (2026-08-19 review)

- [ ] [ROUTINE] **`tests/test_run_tests_parallel.sh` is itself load-flaky — its `-j 1`/`NESTED` seriality checks intermittently observe 2 when they require 1** <!-- id:f875 -->
  - **Found by the 2026-08-19 relay review (id:f69b verification).** Running the FULL suite in a loop under concurrent load (a relay pool is the normal case), the parallel harness is NOT deterministically green: across ~20 full-suite runs I saw distinct intermittent failures — `test_run_tests_parallel.sh` failing `-j 1 overrides JOBS=4: expected '1', got '2'` (and the sibling `-j 1 is serial` / `NESTED forces serial` checks share the mechanism), plus `test_statusline_tokens.sh` and one `test_no_silent_swallow.sh`. All pass standalone. This is the operational cost id:f69b's own history named ("the suite is unusable exactly when a relay pool is running"): a flaky green makes every item's `make test` done-check unfalsifiable.
  - **This test is NEW with id:f69b** (it specs the parallel harness), so its own flakiness is a f69b-lineage regression, tracked NOWHERE until now. **Root shape**: the test measures concurrency by having fixture tests `touch "$LIVE/$$"; sleep N; ls "$LIVE" | wc -l >> OBS; rm "$LIVE/$$"` and asserting the max observed count equals 1 for serial modes. That observation is timing-dependent: under heavy load a just-finished fixture's marker removal can lag past the next fixture's `ls`, so a genuinely-serial runner is observed as concurrency 2. The runner's seriality is (believed) correct; the TEST's way of observing it is not load-robust.
  - **Acceptance**: `for i in $(seq 20); do tests/run-tests.sh >/dev/null 2>&1 || echo FAIL; done` under a loaded box (or an artificial `stress`/parallel background load) prints no `FAIL` attributable to `test_run_tests_parallel.sh`'s seriality checks. The seriality guarantee must be asserted by a mechanism that does not depend on wall-clock overlap of marker files — e.g. have each fixture record a globally-ordered start/stop sequence and assert no interval overlap from the ORDERING, not from a sampled live-count; or gate the timing-sampled assertion behind a low-load precondition and skip-with-reason under load rather than fail.
  - **Tests**: harden `tests/test_run_tests_parallel.sh` in place (do NOT weaken its real coverage of `-j 4` concurrency, file-order output, EXPECTED-RED, nested-serial). No new RED spec needed — the existing test IS the spec; the item is to make its seriality observation deterministic.
  - **Done-check**: the loop above is clean across ≥20 loaded runs; then `tests/run-tests.sh` fully green. Tick only after the loaded loop passes.
  - **Also recorded here (n=1, OBSERVE-don't-fix per the repo's observe-before-preventing discipline)**: `tests/test_no_silent_swallow.sh` failed ONCE in the same loop and passed on every other run and standalone. It is a source-scanner, not obviously HOME/`/tmp`-racy, so its cause is NOT yet established — do NOT fix on n=1. One more full-suite failure naming it promotes it to a fix. `tests/test_relay_loop_intensive_emit.sh` (also seen flaking here) is ALREADY tracked as `id:7a3d` — no duplicate filed.
  - **Context**: `tests/run-tests.sh` (the parallel scheduler), `tests/test_run_tests_parallel.sh` (subject). Lineage: the id:ec3c parallel-suite flake CLASS (class-container closed 2026-08-18); members id:7a3d (intensive_emit), TODO id:24fa/id:04d9/id:456a (git-lock-push load-flake), and instance (1) statusline `/tmp` paths — whose ORIGINAL test `tests/test_statusline_tokens.sh` was still racy and was made hermetic inline in THIS review (usage-state paths now pointed into its mktemp).


## Human triage 2026-08-19 (`/relay human .`, owner-decided)

- [ ] [ROUTINE] **Mechanical `core.hooksPath` shadowing detector across the relay own-set** — PROMOTED from TODO 2026-08-19 (owner-decided), reusing the existing id per single-id-two-views. It was `promote`-classified by `unpromoted-scan` but lived ONLY in `TODO.md`, so the pool could never author or execute it — while `id:293f`'s sweep names it as its gate, making that gate structurally unopenable rather than merely pending. That dead stop is why this promotion happened. A repo that sets a LOCAL `core.hooksPath` silently loses **every** global hook, because `core.hooksPath` REPLACES rather than overlays; on 2026-08-18 that held for **7 of 51 own repos**, each unguarded by both the pre-push privacy gate and the pre-commit lane-vocab ratchet, one pointing at a *different repo's* hook directory (a rename residue). The condition recurs on every clone/move/rename, so a one-off manual sweep rots. **Consume the canonical own-set primitive** (`relay/scripts/lib-own-repos.sh`'s `own_repos`, honoring `# path:` overrides) — **never a `~/src` glob, never a re-derived repo list** (the `id:7877` defect class). Report each repo's local `core.hooksPath` (empty = correct) and classify **EMPTY-SHADOW** (points at a dir with no non-sample hooks → gate silently hollowed, actionable) vs **DELIBERATE** (real repo-local hooks → an owner call, since fixing means merging them into the global dir, not unsetting). Read-only `git config` + a directory listing; **no writes, no unsetting** — the detector reports, the human acts. <!-- id:2bc6 -->
  - **Acceptance**:
    - GIVEN a fixture with one empty-shadow repo, one deliberate-hooks repo and one clean repo THEN the detector yields exactly one EMPTY-SHADOW row, one DELIBERATE row, and NO row for the clean repo — assert all three, since a positive-only test passes against a detector that flags everything.
    - GIVEN a repo whose `# path:` override relocates it THEN it is still enumerated (via `own_repos`, not a glob).
    - GIVEN `relay/scripts/relay-doctor.sh` THEN it references the detector, so the check recurs rather than being a one-shot.
  - **Done-check**: tick this checkbox, then `tests/run-tests.sh` fully green. Unblocks `id:293f` (the sweep) and, through it, `id:ebd0`'s activation done-check.

- [ ] [ROUTINE] **`md-merge.py`: add insert-relative-to-id and an IN-LOCK line transform — the mandated flock path cannot express what agents actually need** (INBOUND `routed:f88b` from loderite, NARROWED by its own author's correction `routed:9aaf`; filed 2026-08-19 `/relay human .`) — **read the correction first, the original claim is overstated.** `f88b` said md-merge has "NO line-prefix/lane rewrite"; that is wrong — `update-ids` accepts a full replacement `{"id":X,"line":…}` which CAN rewrite a lane, and with `--commit` does the write and the scoped commit under the same flock (verified 2026-08-19 re-laning a loderite item). What is genuinely missing: **(a) insert-relative-to-id** — place a new item before/after a given id. Absent, and it is the operation that forced every unflocked hand-rolled read-modify-write observed: ids-not-found are appended at EOF, which is the wrong placement for a seam that must sit beside its siblings. **(b) a TOCTOU-free line EDIT** — the caller must read the line OUTSIDE the lock to compute the replacement, so a concurrent append to that SAME id between read and lock is silently overwritten (last-under-lock wins). The existing `append` mode has no such window and should be preferred wherever it suffices; add an in-lock transform (replace-prefix-by-id, or a regex-sub applied under the lock) so whole-line edits stop needing an out-of-lock read. **Evidence this is not theoretical:** 5 unflocked RMW writes to a loderite `ROADMAP.md` while an autonomous pool was concurrently integrating and running an archive sweep on the same file — no corruption that time, verified after the fact, i.e. luck not design. It recurred in THIS repo the same day: the session that filed this item appended a ROADMAP section and edited TODO.md by hand, outside the flock, for exactly the reason (a) names. **CLAUDE.md states ROADMAP.md/TODO.md/REVIEW_ME.md are written by BOTH the relay pool and `/meeting` and are NOT `merge=union`.** <!-- routed:f88b --> <!-- routed:9aaf --> <!-- id:f26d -->
  - **Second half, NOT this item's code** (`f88b` part 2, surfaced for the owner): the auto-mode system instruction "make file changes with sed, heredocs, or short scripts, rather than using the dedicated Read, Edit, or Write tools" actively steers toward hand-rolled RMW. Its escape hatch ("only when Bash genuinely cannot do the job") does cover flock-mandated files but is unstated and easy to miss. That is a global-`CLAUDE.md` wording change — the owner's text, not an executor edit.
  - **Acceptance**:
    - GIVEN a ROADMAP with items A, B and an insert-after request for A THEN the new item lands between A and B, not at EOF — assert position, not mere presence.
    - GIVEN two concurrent in-lock transforms against the SAME id THEN both survive per the file's stated contract, with neither silently overwritten by an out-of-lock read.
    - GIVEN an insert targeting an id that does not exist THEN it fails LOUDLY (non-zero) rather than silently appending at EOF — silent EOF placement is the defect, not the fallback.
    - GIVEN a line carrying more than one id marker THEN the existing multi-marker refusal still fires (do not regress the `routed:3ad9` guard).
  - **Done-check**: tick this checkbox, then `tests/run-tests.sh` fully green.

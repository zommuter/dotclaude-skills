...
- [x] **[INPUT — decision]** Rewire relay-loop.js integrate() to dispatch integrate.sh via the mechanical hop (id:6176 relay-mech/MECH_MODEL), leaving only the semver micro-hop as an LLM agent (after id:seam-1) — seam of id:a955 (auto, id:3801) <!-- id:087b --> — 🚧 GATED (auto, id:3801; route:decision-gate): Done-check requires 3 tests unmodified, but noop_guard + ckpt_merged_tip structurally assert the very Sonnet integrator prompt this item removes (Acceptance vs Done-check contradict); also seam 9e50/integrate.sh omits tick/archive/6b/push-seed/siblingBranches — owner must rule before build. — needs a /meeting — **SHIPPED + VERIFIED + MERGED 2026-08-21** (`relay-ckpt-20260821-0855`, merge `c31a401`): all five previously-LLM-only behaviours ported into `integrate.sh`, **no LLM agent remains in `integrate()`** (the only `agent()` call is the `MECH_MODEL` fence, and `test_relay_integrator_noop_guard.sh` now actively REFUSES `sonnet|haiku|opus|fable` on that dispatch). Suite green at 453 at merge time. The Acceptance-vs-Done-check contradiction was resolved by the owner-ratified D1 amendment (2026-08-21): the two structurally-blocking tests were rewritten to assert the mechanical dispatch, preserving their real invariants. Bump trigger ships fail-closed pre-merge (`HANDBACK[bump]` exit 30) per the owner's ship-as-is ruling. **TICK WAS LATE — the box stayed `[ ]` for hours after merge because the executor was told to leave it for owner verification and the ticking was then missed; while unticked, `tests/test_integrate_mechanized_ports_087b.sh` counted as EXPECTED-RED, so a genuine regression on the merge-to-main path would not have failed the suite.**
- [x] [ROUTINE] **A POST-PUSH integrate failure is no longer misread as a retryable defer** — PROMOTED from TODO 2026-08-21 (blast-radius review; the item was TODO-only, promoted with the SAME id per the single-id-two-views rule). `integrate.sh` now emits a STDERR handback block that names the class: PRE-push exits print `handback=<step>` + `landed=false` and NEVER a `merged=` line, while the POST-push exits (retire 28, state-write 29, strong-state 33) print `handback=<step>`, `landed=true`, `merged=<sha>`, `ckpt=<tag>`, `push=pushed`, `remaining=<steps that did NOT run>` and `ckptRecorded=<bool>`. STDERR because `mechanical-proxy.py` DISCARDS a non-zero-exit child's stdout. `parseIntegrateResult` returns a THIRD outcome, `landedUnfinished`, distinct from `deferred`; the integrate call site gives it its own branch that SURFACES the unit as a handback naming the merged sha, ckpt, failing step and unrun steps — and never re-merges it (a retry would fail forever at merge/isolation). `push(27)` stays DEFERRED (the push failed, the remote is untouched) and every pre-push exit behaves exactly as before. The stale-`last_ckpt` symptom is closed by a best-effort in-handback reconcile of `relay.toml last_ckpt` to the already-pushed tag, reported as `ckptRecorded`. Tests: `tests/test_integrate_post_push_handback_5fe2.sh` (real git fixtures, one PRE-push and one POST-push failure plus push(27) and the happy path, asserting the caller's parsed state differs). Suite green at 461. Relates id:087b, id:9e50. <!-- id:5fe2 -->
  - **Acceptance**: the Sonnet integrate agent at relay-loop.js:2909 (model:'sonnet') is replaced by a mechanical relay-mech hop invoking integrate.sh; the ONLY remaining LLM agent on the integration path is the semver 'user-observable?' micro-hop; enqueueIntegration (:1104) per-repo serialization is unchanged; the id:aa93 clean-tree gate and id:6e02 cleanup-scope stay enforced mechanically; the id:c563 no-agent-before-dispatch invariant still holds (prefer keeping test_relay_integrator_noop_guard.sh unmodified; only if genuinely superseded, update — never delete — its test)
  - **Done-check**: full tests/run-tests.sh green with tests/test_relay_integrate_contain.sh, tests/test_integrate_ckpt_merged_tip.sh and tests/test_relay_integrator_noop_guard.sh unmodified
  - **Context**: relay/scripts/relay-loop.js (integrate() ~2696-2909, enqueueIntegration ~1104, MECH_MODEL/relay-mech pattern per id:6176 at ~2185)

## Dispatch-path defects 2026-08-21 (C2 promotions from TODO, `/relay handoff`)

> Promoted from `TODO.md` 2026-08-21 reusing the existing ids (single-id-two-views — no
> duplicates minted). All four are `[ROUTINE]`. `TODO.md` stays the prose SSOT for the
> measured evidence; these are the executor specs. RED tests landed in the same commit.

- [x] [ROUTINE] **The `id:4f9b` prompt-size gate UNDER-COUNTS by ~50% — it never counts `TODO.md`** — see `TODO.md` for the measured incident. `relay-loop.js:2466-2473` (inline copy of `relay/scripts/prompt-size-gate.mjs`) estimates `round((promptChars + roadmapBytes)/4) + 12000`; the child prompt also embeds `TODO.md`, which is absent from the estimate. loderite: ROADMAP 350,698 B → est. 99,674 tok → ALLOW (under a 100,000 budget by 326) → child died `Prompt is too long`, recorded as the generic failure string `4f9b` exists to eliminate. ROADMAP+TODO = 197,685 tok, ~2× budget. <!-- id:b018 -->
  - **Acceptance**: `classify-repo.sh` emits `todo_bytes` on the unit alongside `roadmap_bytes`; `estimateDispatchTokens` counts BOTH ledgers; `oversizeDispatchReason` REFUSES the loderite unit (`roadmap_bytes:350698, todo_bytes:392043`) with a reason naming `TODO.md` and its byte count, while the roadmap-only 4f9b calibration points (523,926 B refuse / 254,087 B dispatch) are unchanged. FAIL-OPEN posture preserved: a unit with no `todo_bytes` behaves exactly as today, never blocked on absent data. Audit the assembled child prompt for any OTHER embedded ledger and count it too.
  - **Done-check**: `tests/test_prompt_size_gate_todo_b018.sh` green AND `tests/test_prompt_size_gate_4f9b.sh` still green; then full `tests/run-tests.sh`.
  - **Sequencing**: touches `relay/scripts/relay-loop.js` (the inline copy must stay byte-equivalent to the module) — **sequence AFTER `id:087b` lands.** NOT gated on `id:e68f`: `b018` is the cheap immediate fix, `e68f` the structural one, and they are independent — do not defer this behind it. <!-- relates:e68f --> <!-- relates:4f9b -->

- [x] [ROUTINE] **Orchestrator writes the ledger SLICE to a tmp file and hands the child a PATH — the child can no longer over-read, because the bytes are not in its prompt** (owner proposal 2026-08-21) — today the prompt embeds/points at the whole `ROADMAP.md` + `TODO.md` and a prose instruction TELLS the child to slice; a prose instruction the child can get wrong is not a guard. <!-- id:e68f -->
  - **Acceptance**: a host-side slicer produces a FILE containing exactly the dispatched item's line, its `gated-on:`/`children-of:`/`children:` edge lines, and the repo-state header — and NOT the rest of the ledger; the assembled child prompt carries the PATH, not the ledger bytes. This DISSOLVES the prompt-size problem structurally (`prefer dissolving a problem structurally over guarding it`) and makes `id:b018`'s estimate trivially correct, since the slice size IS the prompt size.
  - **Done-check**: `tests/test_ledger_slice_dispatch_e68f.sh` green; then full `tests/run-tests.sh`.
  - **Sequencing**: touches `relay/scripts/relay-loop.js` — **sequence AFTER `id:087b` lands.** Supersedes what `id:b018` guards, but does NOT gate it. Relates `TODO.md:132` (the original 2026-06-22 statement of this defect). <!-- relates:b018 -->

- [x] [ROUTINE] **URGENT — prompt-size gate must measure the `id:e68f` SLICE when one exists (this repo was un-dispatchable)** — `id:b018` and `id:e68f` were ratified INDEPENDENT and are not: `oversizeDispatchReason` counted `roadmap_bytes + todo_bytes` unconditionally, so dotclaude-skills estimated ~301,349 tok against a 100,000 budget and the gate refused EVERY dispatch — on bytes the child no longer reads, because `sliceLedgerForUnit()` stamps `unit.slice_path` immediately BEFORE the gate's call site. See `TODO.md` for the full statement. <!-- id:35b7 --> <!-- relates:b018 --> <!-- relates:e68f -->
  - **Acceptance**: with `unit.slice_path` set the required-read set IS the slice — the gate sizes the MEASURED slice and stops counting the ledgers; with no slice it counts the ledgers exactly as before. Fail-open unchanged (an unmeasured slice never blocks). The printed REMEDY no longer prescribes `archive-done.sh` unconditionally — it names the slice lever first and marks archiving as conditional on the bulk being CLOSED.
  - **Tests**: `tests/test_prompt_size_gate_slice_35b7.sh` (`# roadmap:35b7`)
  - **Done-check**: `tests/run-tests.sh tests/test_prompt_size_gate_slice_35b7.sh` then full `make test`.
  - **Context**: `relay/scripts/prompt-size-gate.mjs` + the byte-equivalent inline copy in `relay/scripts/relay-loop.js`; `relay/scripts/ledger-slice.sh` gained an ADDITIVE stdout line `slice-bytes: <N>` (the path stays last, so the 18 `id:e68f` assertions and the existing parse are untouched). Byte count is MEASURED on the host, never a guessed allowance.
- [x] [ROUTINE] **`id:e68f`'s slicer bounded an item block by INDENTATION, so column-0 acceptance criteria were silently dropped** — the block now extends to the next COLUMN-0 checkbox line or the next `#`-heading, fenced blocks are never split mid-fence, and the owning section HEADING is stamped into the slice header (parked/exempt context, `id:356f`). Promoted from TODO with the same id (single-id-two-views). <!-- id:b015 -->
  - **Acceptance**: column-0 prose / un-indented bullets / fenced blocks belonging to the item are INCLUDED; a fenced sample checkbox does not terminate the block; the block never bleeds into the adjacent next item; `slice-bytes` stays MEASURED (`wc -c`) with the path as the last stdout line.
  - **Tests**: `tests/test_ledger_slice_block_bounds_b015.sh` (`# roadmap:b015`)
  - **Done-check**: `tests/run-tests.sh tests/test_ledger_slice_block_bounds_b015.sh` then full `make test`.
  - **Context**: `relay/scripts/ledger-slice.sh` only (`relay-loop.js` untouched). Size impact measured across all 82 ROADMAP items: max 99,907 → 100,012 B (+0.1%, the heading line), median 3,366 → 3,471 B; exactly one item (`id:0e56`) grew materially (+1,166 B) — genuinely its own previously-dropped content.
- [x] [ROUTINE] **Suppression must DEMOTE the verdict, not DROP the unit — a stuck item starves every lower verdict class indefinitely** — observed live on loderite, run `relay-20260820-180056-4594`: `actionable_routine_ids: ['57d1']` (one item, whose child died on prompt size) pinned the repo at `execute` while both anti-spin mechanisms (`id:1432` no-work suppression, `relay-loop.js:1235`; `id:365b` >3× circuit breaker, `:1196`) dropped the unit — pinned to `execute` AND `execute` suppressed, so the repo could reach NOTHING while 9 promotable items sat unreachable. <!-- id:bc2b -->
  - **Acceptance**: on `1432`/`365b` suppression the verdict is re-classified with that verdict CLASS EXCLUDED and the next-ranked class is dispatched; if none applies, idle exactly as today. No threshold heuristic, no new state (suppression state already persists per-run). Exclusion may only DEMOTE — never promote a repo above the verdict it would otherwise get. Precedent to mirror: the ratified `chain_ended ∧ substantive_unaudited` branch at rank 2 (`id:8123`), which exists because review was starving behind open `[ROUTINE]` work — identical shape, identical remedy.
  - **Done-check**: `tests/test_suppression_demotes_bc2b.sh` green; then full `tests/run-tests.sh`.
  - **Sequencing**: touches `relay/scripts/relay-loop.js` — **sequence AFTER `id:087b` lands.** Relates `id:4a76`, `id:1432`, `id:365b`, `id:8123`.

- [x] [ROUTINE] **`drained` must distinguish FINISHED from HUMAN-BLOCKED — two repos with 31 open items between them read as done** — `classify-verdict.sh:235` emits `human` ONLY when `surface > 0`; a repo whose ENTIRE open backlog is human-lane but whose `surface == 0` falls through to `idle`, and `control-board.sh:162` maps `idle → design-drained`. Measured 2026-08-21: helferli = 18 open, ALL `[INPUT — …]`, zero poolable → `design-drained`, "Waiting on a human: _(none)_"; csgebra = 13 open, all executable items `gated-on:` `940f` `[INPUT — decision]` → same false-clean. <!-- id:4a76 -->
  - **Acceptance**: fix UPSTREAM in `classify-verdict.sh`, NOT in the board (the board renders faithfully; fixing it there would leave `/relay human`'s scope and the pool's Skipped rollup still wrong). Emit `human` when `roadmap_open > 0` AND every executable lane is zero AND ≥1 open item sits in a human lane (`[INPUT — *]`, `@manual`) — `classify-repo.sh` supplies the count. `human → needs-feedback` already lands in the right board section, so `control-board.sh` needs NO change. The existing `surface > 0 → human` branch (`id:5eb3`) must keep firing unchanged, and a genuinely empty repo must still be `idle`.

...
- [ ] [INPUT - decision] 🔴 **URGENT -- 46 tests are RED on unmodified `main`: `node --check` cannot parse `relay-loop.js` since today's Node upgrade, because it mixes top-level `await` and `return`.** -- detail: `docs/ledger-notes/62c9.md` <!-- id:62c9 -->

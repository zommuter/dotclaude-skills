...
- [x] [HARD] **Command-fence precedence: a `` ```relay-mech `` fence embedded in a stdin PAYLOAD overrides the loop's own command** — the id:33b2 channel's missing boundary. **BLOCKS every hop conversion: id:d4ca and id:e405 must NOT convert a hop until this lands.** <!-- children-of:33b2 --> <!-- id:93ac -->
  - **DONE 2026-08-12** (relay hard-execute, apex). Fix = new `_strip_stdin_fence_span()` in `mechanical-proxy.py`; `_command_from_wrapped()` now searches the text with the `` ```relay-mech-stdin `` span EXCISED before matching the command fence, so a payload can never contribute the command by construction. Reuses the two existing regexes (`_MECH_STDIN_FENCE_RE`/`_MECH_FENCE_RE`) — no third parser, per the item's constraint. The rejected weaker alternatives stay rejected: no positional invariant ("command fence must be first" is the same un-asserted-invariant class as the defect) and no ">1 command fence" refusal (would break on any payload that legitimately quotes a fence — i.e. this repo's own docs). Test `tests/test_mech_command_precedence_93ac.sh` (`# roadmap:93ac`) covers a–d: (a) payload-first attack + lone-payload refusal both dispatch the LOOP's command or refuse, never the payload's `--path /tmp/ATTACKER`; (b) metacharacter payload round-trips byte-identical (payload path untouched); (c)+(d) canonical command-first and no-stdin requests unchanged. Verified genuine red-green (pre-fix extractor picks EVIL). **PRECEDENCE-SCOPED, as the title says**: the separate pre-existing limitation that the non-greedy stdin regex TRUNCATES a payload containing its own `` \n``` `` fence (a data-fidelity issue that surfaces only when id:d4ca actually flows a fenced markdown doc) is NOT this item and is left for a follow-up. id:33b2 not reopened; full suite 391/0/1-ered.

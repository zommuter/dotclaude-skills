...
- [x] ⭐ HIGH PRIORITY: cut relay status/overhead cost (~35% of spend, low-concurrency, on the critical path) [HARD — strong model] <!-- id:c3a6 -->
  ...
  - **Forward — integrator frequency** (NOT tier): INVESTIGATED 2026-06-17 (id:c563) — `integrate()` (relay-loop.js:940/953) already early-returns to `state.blocked` BEFORE the Sonnet agent (line 979) for null / `contract_met=false`, so the integrator agent spawns ONLY for genuine merges. Event-log 2026-06-16: 168 dispatches → 119 merges + 8 conflict-handbacks + 41 silent early-returns; **no wasteful integrators — the 'skip no-op integrates' lever does not exist** (regression-guarded by `tests/test_relay_integrator_noop_guard.sh`). Remaining lever = merge BATCHING only (high blast-radius, merge-to-canonical) → its own design session; not started. <!-- id:c563 -->

...
- [ ] [INPUT — decision] **A relay round that dispatches NOTHING leaves no trace of its verdict or reason — emit a `verdict`-kind event per repo per round** <!-- id:c7dc --> — 🚧 @container GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:258d, id:37f2, id:e87d — pick those, not this. id:c7dc spans 4 files (classify-repo.sh/discover-repo.sh/discover-chunk.sh/relay-loop.js) + a new dual-mode test; no sub-piece is independently acceptance-complete — split into 3 dependent seams.
...
- [x] [ROUTINE] **DISSOLUTION — the parent creates + provisions the worktree BEFORE dispatch, and the child is never told the main-checkout path** <!-- children-of:f91a --> <!-- id:34b7 -->
...
- [ ] **[ROUTINE]** discover-repo.sh + discover-chunk.sh: carry verdict/priority_rank on the no-unit paths (blocked/AMBIGUOUS/idle/substitutive) (after id:258d — seam 1, DONE) — seam of id:c7dc (auto, id:3801) <!-- id:37f2 -->
...
- [ ] **[ROUTINE]** relay-loop.js: emit pushEvent('verdict', {repo, round, verdict, priority_rank, reason, sig, cached}) once per repo per round, including reused-cache repos (after id:37f2 — seam 2) — seam of id:c7dc (auto, id:3801) <!-- id:e87d -->
  - **Acceptance**: pushEvent('verdict', ...) fires exactly once per repo per round for every unit/surfaced/skipped entry produced this round (dispatched, blocked, AMBIGUOUS, idle, substitutive) AND for cache-reused repos (reusedUnits/reusedIdle, id:c3a6) — with `cached:true` for a reused verdict and `cached:false` for a freshly-derived one; the round number is present; reachable via the existing pendingEvents -> snapshotState -> event-append sink (no new file).
  - **Done-check**: New tests/test_verdict_event_c7dc.sh (# roadmap:c7dc), hermetic behavioural fixture in the style of tests/test_classify_repo_unit.sh PLUS source-shape assertions in the style of tests/test_dispatch_event_sig.sh for the pushEvent('verdict',...) call site and its reachability from every no-unit/reused path; `node --check relay/scripts/relay-loop.js` and `node relay/scripts/lint-workflow-templates.mjs relay/scripts/relay-loop.js` both pass; new test green, then tick id:c7dc's checkbox and run full `make test`.
  - **Context**: relay/scripts/relay-loop.js (discovery-consumption block ~lines 1396-1530, and the `round++` loop ~line 3107), new tests/test_verdict_event_c7dc.sh

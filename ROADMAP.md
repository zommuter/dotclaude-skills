...
- [ ] [ROUTINE] Refine `roadmap-lint.sh` c095 heading-as-item detection — only flag a `## …[LANE]…` heading as MISSING-id when its children are BARE status markers (no own class tag + id) <!-- id:dfe4 -->
...
- [ ] [ROUTINE] Add a host-gate to `mechanical-daemon.sh` — skip+defer any recipe whose `.host` != `uname -n`, never execute it <!-- id:26c2 -->

...
- [ ] **[ROUTINE]** flake-log width=1 confirmation run #2 (post-id:81d5) (after id:(seam 1, independent — no id yet)) — seam of id:372a (auto, id:3801) <!-- id:f2ef -->
...
## Apex `hard` dispatch requires `--afk`; the gate stops keying on a model-id string (handoff C2/C3 2026-08-23)

ONE unit, ONE commit — two owner-ratified TODO items that rewrite the SAME block. Promoted
with their existing ids (single-id-two-views); `id:7986` is primary, `id:da51` rides along.

- [x] **[ROUTINE]** **Apex `hard` dispatch MUST require `--afk`, and the HARD-execute gate must key on `STRONG_TIER === 'opus'` instead of the model-id literal `claude-opus-4-8`** — combines `id:7986` (owner 2026-08-22, after a live pool spent Opus on three `[HARD]` units against his cap) with `id:da51` (`routed:fba9` from csgebra; owner-ratified 2026-08-23). Today `relay-loop.js` has NO consumer of `A.afk` at all, so the owner's rule is prose only; and the hard gate compares `STRONG_MODEL !== 'claude-opus-4-8'`, so bumping the pin to the live apex model would make the inequality TRUE and defer every hard unit while reporting "requires apex Opus" — a message false precisely when the pin is correct. The two must land together or the next reader inherits the same contradiction. **SETTLED, do NOT re-open:** (1) `handoff` is PERMITTED ALWAYS at apex, exactly like `review` — ONLY `hard` is gated on `--afk` (owner 2026-08-22; this OVERRIDES the contrary recommendation recorded when `id:7986` was filed, and the SKILL.md wording must not imply handoff is gated). (2) The gate asks `STRONG_TIER === 'opus'`; the model id becomes a pure OUTPUT value, bumped to `claude-opus-5` (owner 2026-08-23). (3) Runtime auto-detection of the live apex model id is OUT OF SCOPE (filed separately as `id:f6a1`). <!-- id:7986 --> <!-- ref:da51 -->
  - **Acceptance**:
    1. NEW pure module `relay/scripts/apex-gate.mjs` exports `enforceApexGate(units, { strongTier, afk }) -> { plan, hardDeferred }`, following the `round-plan.mjs` (id:dc5b) / `handback-summary.mjs` (id:1735) extractable-module pattern. `hardDeferred` entries carry the unit WHOLE plus a `gateReason` string. Semantics: a `hard` unit is withheld when `strongTier !== 'opus'` (reason names APEX, never `--afk`) or when `!afk` (reason names `--afk`); `review`/`handoff`/`execute` are NEVER withheld by this gate; `plan` order is preserved. The module observes NO model id and contains no `claude-*` literal.
    2. `relay/scripts/relay-loop.js` carries a byte-equivalent INLINE copy (the Workflow sandbox cannot `import`) and calls it in the dispatch path, replacing the `if (STRONG_MODEL !== 'claude-opus-4-8')` block at `:2306-2325`. `A.afk` is read (a `const AFK = !!A.afk` beside `ALLOW_INTENSIVE` is the idiom).
    3. Deferred hard units reach the RENDERED Queued section carrying their `gateReason` — the `state.queued` `hardDeferred.map(...)` at `:2482` must interpolate `u.gateReason`. Mirror the proven `intensive:<res>` surfaced-as-skipped shape at `:2484`; do NOT invent a new one. Never dropped, never silently idle (id:8c85 vanish class).
    4. EVERY `claude-opus-4-8` literal in `relay-loop.js` is gone: `:40` (the pin → `claude-opus-5`), `:95`, `:2307`, `:2313`, `:2338` (comments), `:3147` (the `fable-standin` suffix — re-key it on `STRONG_TIER === 'opus'`).
    5. `relay/SKILL.md`: the Workflow-launch arg list (`:275-281`) documents `args.afk`; the HARD-execute gate paragraph (`:329-334`) names `--afk` as a condition AND states verbatim that "`review` and `handoff` dispatch regardless of `--afk`"; the `--afk` flags-table row (`:847`) says the flag gates apex `hard` dispatch (it currently claims it only surfaces the `/loop` nudge); the model map (`:876`) and the `strong_model` example (`:829`) read `claude-opus-5`. No `claude-opus-4-8` left in the file.
    6. The FRONT DOOR passes `args.afk` to the Workflow (SKILL.md step 3) — today it never does, which is half of why the flag is inert.
    7. THREE SIBLING TESTS PIN THE OLD BEHAVIOUR AND MUST BE UPDATED IN THE SAME COMMIT, or the suite goes red on a correct fix: `tests/test_relay_loop_structure.sh:204-207` asserts the literal `if (STRONG_MODEL !== 'claude-opus-4-8') {`; `tests/test_fable_standin_marker.sh:33-35` asserts fable-standin is gated on `claude-opus-4-8`; `tests/test_strong_tier_knob.sh:42-43` asserts SKILL.md maps opus → `claude-opus-4-8`. Re-point each at the new shape, do not delete the assertion.
    8. Docs-only follow-through in the same commit: `relay/references/conventions.md:46,103,108` mention `claude-opus-4-8` as an example model id — refresh to `claude-opus-5`. `relay/scripts/prompt-size-gate.mjs:31` names it in a tier-derivation COMMENT only (no behaviour) — refresh. `RELAY_LOG.md`, `CHANGELOG.md`, `ROADMAP.archive.md`, `TODO.archive.md` are HISTORY — leave them alone.

...
- [ ] [INPUT - decision] **Both ledger ratchets are INERT on the install path, their baseline key has no repo dimension, and their length metric is locale-dependent -- fix all three or none.** -- detail: `docs/ledger-notes/4839.md` <!-- relates:0d7c --> <!-- id:4839 --> — 🚧 GATED (auto, id:3801; route:hard-split): DECOMPOSED into seams id:eccb, id:c655, id:aa5e, id:cb9a — pick those, not this. id:4839 spans 3 ordering-constrained dimensions + 2 aggravations + 4 hermetic tests in one heavily-invariant script; too large for one session, split into 4 ordered seams
...
- [ ] [ROUTINE] 🔴 **`SH_ASSIGN_RE` truncates a multi-line shell assignment at the first quote, silently dropping a read's notes operand -- the missed-union direction is the UNSAFE one the code names.** -- detail: `docs/ledger-notes/e047.md` <!-- children-of:1447 --> <!-- id:e047 -->
...
- [ ] [ROUTINE] 🔴 **`md-merge.py update-ids` SILENTLY no-ops when an id is found but the op changes nothing -- a missed `regex_sub` or an empty `append` both write back unchanged and exit 0.** -- detail: `docs/ledger-notes/3bd4.md` <!-- id:3bd4 -->
...
- [ ] **[ROUTINE]** ratify-queue.sh: self-verifying list — report a remote-confirmed pending entry as landed, not pending (Acceptance D) — seam of id:7408 (auto, id:3801) <!-- id:4d65 -->
...
- [ ] [ROUTINE] **The parked `id:3bd4`/`id:4f0f` attempt HANGS the four md-merge tests named for those ids -- >90s vs seconds green on `main` at the same load.** -- detail: `docs/ledger-notes/be51.md` <!-- relates:3bd4 --> <!-- relates:4f0f --> <!-- id:be51 -->
- [ ] [ROUTINE] **The parked `id:4d65` attempt is 11 PASS / 1 FAIL, and the failure INVERTS the item: `list --tsv` drops a genuinely-pending entry.** -- detail: `docs/ledger-notes/ceca.md` <!-- relates:4d65 --> <!-- id:ceca -->

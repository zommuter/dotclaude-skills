...
- [x] ⭐ HIGH PRIORITY: cut relay status/overhead cost (~35% of spend, low-concurrency, on the critical path) [HARD — strong model] <!-- id:c3a6 -->
  ...
  - **Forward — D1 attribution still to confirm on a real run** (`relay-econ.py` before/after): verify Discover-phase Opus spend collapses and report which cost bucket actually moved (shards are phase `Discover`, not necessarily the econ `status` bucket). <!-- id:9cb1 -->
  - **Forward — integrator frequency** (NOT tier): 109 serialized Sonnet integrations on 2026-06-16 (`relay-loop.js:455`) — the cost is count×serialization, not model. Investigate batching / skipping no-op integrates. High blast-radius (merge-to-canonical path) → its own session. <!-- id:c563 -->

...
- [ ] [HARD — meeting] Babysitter loop can't survive a hard session kill — `CronCreate durable:true` is a no-op locally (`[session-only]`); only a cloud `/schedule` routine is outage-proof. Decide: fix/file durable persistence vs re-point the `loop-hint.sh` nudge to `/schedule` for true cross-session-kill resilience vs document the `/loop`(in-session)-vs-`/schedule`(outage-proof) distinction. Design + rationale: **TODO id:98f0**. → /meeting. <!-- id:98f0 -->

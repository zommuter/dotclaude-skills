...
- [ ] [ROUTINE] `relay-state-write.sh` `toml-set` must emit VALID TOML for a bare string value (smart-quote) — a hyphenated `status = handed-off` currently produces invalid TOML that breaks the pool's relay.toml read (routed:dc81) <!-- id:abbd -->
...
- [ ] [HARD — pool] Port the 000d/ad74 per-round backstop logic into `classify-verdict.sh` so the guards fire 0× natively (re-opens id:b50e deletion) <!-- id:c79e -->

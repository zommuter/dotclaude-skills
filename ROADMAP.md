...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract — RESCOPED 2026-07-28 after an adversarial review refuted this item's own fix direction and its RED spec** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  ...
  - **FIX DIRECTION REVERSED (2026-07-28 adversarial review).** The original text said "either Skill-only (drop the Reads) or drop the Skill". Evidence says **drop the SKILL and keep the ~9.5k Reads**: `SKILL.md:32` claims `/relay executor` loads *only* `references/executor-contract.md` (12,590 B ≈ ~3.1k tok), yet the measured Skill load was **+26,674 tok** ≈ the ENTIRE relay doc set (SKILL.md 64,376 + contract 12,590 + conventions 8,796 + hard-lanes 22,181 = 107,943 B ≈ ~27k tok). If that holds, "Skill-only" — the direction this item leaned toward — would have made the problem WORSE, and the real lever is ~26.7k, roughly 3× this item's original ~9.5k headline.
  - **VERIFY BEFORE ACTING — this is byte-math inference, NOT transcript-verified.** First step of the item: check a dead child's transcript Skill-result token count against the 107,943-byte sum. If the Skill load turns out to be only the contract (~3.1k), the original "drop the Reads" direction is right instead. **Do not implement on the inference alone**; state which way the measurement went in the close note.

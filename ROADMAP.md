...
- [ ] [ROUTINE] **Lint for VACUOUS FIXTURES — a defect-fix test that proves nothing because its fixture never reaches the guarded code path** <!-- id:292b -->
...
- [x] [ROUTINE] **Record the verified pool ∥ meeting parallel-safety topology as a durable convention in `ARCHITECTURE.md`** <!-- routed:832e --> <!-- id:f657 -->
  - **Acceptance**: `ARCHITECTURE.md` gains ONE short subsection stating the convention that a scoped `/relay --afk --only <repo>` pool and a `/meeting` may run concurrently on the SAME repo without ledger collision, naming the three mechanisms that make it safe WITH their ids — distinct claim keys (`id:0ee1`), ledger-only writes are not lease-gated (`id:c144`), and flock + atomic commit on the shared ledgers — and the two interactions that are EXPECTED (not defects): the pool can close an item the meeting is mid-reasoning about, and a handback can carry an owner question the meeting answers in-turn. It must NOT restate the mutable checkbox state of `id:0ee1`/`id:c144` (durable convention only — the class `ARCHITECTURE.md` owns, hence no checkbox there).
  - **Tests**: `tests/test_architecture_pool_meeting_convention_f657.sh` (`# roadmap:f657`) — currently RED (no such subsection today). A doc-content contract (a legitimate DOC assertion per `lint-source-grep-assertions.py`'s own carve-out): asserts `ARCHITECTURE.md` contains a pool/meeting parallel-safety section naming both `id:0ee1` and `id:c144`.
  - **Done-check**: `tests/run-tests.sh tests/test_architecture_pool_meeting_convention_f657.sh`, then tick the box and run full `make test` green.
  - **Context**: `ARCHITECTURE.md`; TODO id:f657; the observation is cartulary's (`routed:832e`, provenance `docs/meeting-notes/2026-08-12-1135-a13-record-id-word-tuple-and-namespace-split.md`), recorded as reported — this repo has not independently re-run the 4-round/57-agent drain.

- [ ] [ROUTINE] **`roadmap-lint` recognizes an explicit OWNER-HOLD marker and suppresses its false DEAD-GATE** (SCOPED to the linter only) <!-- id:d119 -->
  - **Acceptance**: `relay/scripts/roadmap-lint.sh`'s DEAD-GATE rule (id:49e0) learns an owner-hold marker grammar (PROPOSED spelling `<!-- owner-hold:REASON -->` — confirm in REVIEW_ME before finalizing): an OPEN, active-section ROADMAP item carrying that marker is treated as INTENTIONALLY held, so NO DEAD-GATE finding fires for it even when it also carries a `gated-on:` whose target is TODO-only/archived/absent; an item with a dead gate and NO owner-hold marker STILL fires DEAD-GATE unchanged (WARN default, ERROR under `--strict`). **Explicitly OUT of scope**: migrating `id:540f`/`id:c179`'s `gated-on:e62c,b0b1` to the new marker, and any change to `classify-repo.sh` dispatch suppression — those are a SEPARATE coordinated step (removing `gated-on:b0b1` without the dispatch gate also honouring owner-hold would make those items dispatchable, which the owner forbids). This item builds ONLY the report-only linter's recognition; it is on no dispatch path.

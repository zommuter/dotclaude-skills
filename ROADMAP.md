...
- [ ] [ROUTINE] **Executor dispatch DOUBLE-LOADS the executor contract — RESCOPED 2026-07-28 after an adversarial review refuted this item's own fix direction and its RED spec** (promoted from TODO `routed:d2a1`) <!-- children-of:93cc --> <!-- id:9eb7 -->
  ...
  - **ITEM STAYS OPEN**: step (1) has landed, steps (2) + the skill-split trade have not. **The countermand is an INSTRUCTION, not a guarantee** — the same limit as id:6f1c, and both 07-28 children silently ignored the dispatch's existing "Read conventions.md" instruction. **Real acceptance = a measured drop in child context on the next dispatch**: check a child's `usage.cache_creation_input_tokens` at its post-Skill turn; expect the ~26.4k entry to be ABSENT and a ~5.5k contract Read in its place. If children still load the orchestrator, the countermand loses to the CLAUDE.md pointer and step (2) becomes mandatory rather than optional.

...
- [ ] **id:be0e — ratify the scope substitution (relay execute+review 2026-07-20).** The executor found the item's literally-cited symptom (the "multiple lane brackets" ERROR) was ALREADY fixed (id:1781/ad8a), so it instead fixed a *different* real, currently-reproducible false-positive that matches the item's **stated principle**: the `**[ROUTINE]` markdown-bold wrapper tripping `TAG-NOT-FIRST` (fixed by stripping leading `*`/`_` before the position check; RED test `tests/test_roadmap_lint_head_anchor.sh`, suite 274/0, merged `relay-ckpt-20260720-1404`). **Your call:** (a) ratify the substitution as shipped, and (b) decide whether the originally-cited multi-bracket ERROR case still deserves its *own* explicit fixture. Surfaced retroactively per the balanced-brief correction — the round-1 reviewer documented this scope change in its findings but did not open a box (chidiai case `2026-07-20-relay-opus-review-agent-returned-ship`, the calibration anchor for that fix). Tick to ratify; add a note to request the extra fixture.

...
prose were FIXED here rather than parked: 228 punctuation em dashes became `--`,
16 range en dashes became `-`, and 13 venue-keyed lane tags had their em-dash delimiter
migrated to the spaced hyphen. Nothing else changed: no word, figure, marker or line
break was altered, and the section is otherwise the ROADMAP.md block.
...
lines, and a note has none): 7 meeting-venue tags became `[INPUT - meeting]`, 2 pool-venue
tags became `[HARD]`, 1 decision-gate tag became `[INPUT - decision]`.

THREE things remain deliberately unchanged, each declared rather than silently left.
(1) TWO `[HARD - hands]` tags are NOT converted: `lane-convert.sh` never auto-converts that
lane, because it fragments across four destinations (`[MECHANICAL]`, `[INPUT - access]`,
`[INPUT - decision]`, `[INPUT - meeting]`) by per-item human judgment. Converting them here
would be a guess. (2) ONE `[HARD - strong model]` tag is NOT converted: the canonical
mapping has no rule for it, and inventing one in a note is the wrong place to extend the
vocabulary. (3) ONE backticked gawk regex literal (a two-character class quoted from live
code, in the Run-33 CLEAN paragraph) keeps its em dash: the surrounding sentence calls it a
"backwards-range", which is only true of the character actually in the source, so rewriting
it would make the quotation false.
The `## From TODO` section below is PRE-EXISTING and was not touched by this pass; its
own dashes are untouched and remain as relocated.
...
## From ROADMAP
  ...
  - **Run log** (recurring item -- stays open by design):
    ...
    - Run 25 (2026-06-21-1842): first-seen change since Run 24's own audit merge `99a1f2e`
      ...
      auto-gated `[HARD - strong model]` → `[INPUT - decision]` route:human. `git diff
    ...
    - Run 40 (2026-06-22-1601): first **CODE** window since Run 39's LEDGER-ONLY runs --
      ...
      `[INPUT - meeting]→[HARD - hands]` retag of 7809/98f0 landed in ROADMAP and that new items
      ...
      `[INPUT - meeting]`; corrected to 7 open HARD with e149/0994 added + the lane fix; (b) the
    ...
    - Run 45 (2026-06-23-0939): first-seen code since Run 44's own audit commit `0e60f1f`
      ...
      id:1d64 margin-aware quota-stop staleness, id:3c0f `[HARD]` token sync,

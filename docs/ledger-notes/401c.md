...
16 range en dashes became `-`, and 11 venue-keyed lane tags had their
em-dash delimiter migrated to the spaced hyphen (`[HARD - pool]`). Nothing else changed:
no word, figure, marker or line break was altered, and the section is otherwise the
ROADMAP.md block. TWO things were deliberately NOT changed, each declared rather than
silently left. (1) The lane NAMES stay venue-keyed (`[HARD - meeting]`, not
`[INPUT - meeting]`): this is an append-only audit LOG, and renaming a lane inside a
record of what a past run actually classified would falsify the record. The delimiter
migration is a spelling change and is safe here; the vocabulary rename is a claim about
history and is the owner's call. (2) ONE backticked gawk regex literal (a two-character
class quoted from live code, in the Run-33 CLEAN paragraph) keeps its em dash: the
surrounding sentence calls it a "backwards-range", which is only true of the character
actually in the source, so rewriting it would make the quotation false.

...
**APPENDED AND EDITED 2026-09-12 (id:401c Run 73), declared here because this note's header
asserts its prose is reproduced verbatim.** Two changes, nothing else touched: the Run 73
entry was APPENDED to the `## From ROADMAP` run log (that log is a recurring item's designed
growth point), and the `## From TODO` mirror line's stale "Latest ✓ Run 70 2026-07-06" was
refreshed to Run 73 -- it had been stale across Runs 71 and 72. Refreshing that mirror is the
same coherence-drift fix Runs 21-30 recorded, and the `## From TODO` section is otherwise
still untouched by any pass.

**EDITED ON RELOCATION (2026-09-02, `id:f193`) -- the verbatim claim above is amended
for the `## From ROADMAP` section only.** Fleet-rule violations found in the relocated
prose were FIXED here rather than parked. Re-measured against the pre-relocation
ROADMAP.md block (`ef7c7d0f^`, 1,037 continuation lines), which held 240 em dashes and
16 en dashes: 228 punctuation em dashes became `--`, 11 were the delimiter of a
single-lane venue-keyed tag and became the spaced hyphen (`[HARD - meeting]`), 1 was
kept (see (5) below), and all 16 range en dashes became `-`. Nothing else changed in
that pass: no word, figure, marker or line break was altered. (The figure "13 venue-keyed
lane tags" stood here until 2026-09-02; it was wrong, because it counted this header's
own two example tags as log content. The true count is 11.)

**AMENDED 2026-09-02, owner ruling -- the lane NAMES were converted too.** The relocation
pass had left the venue-keyed lane NAMES in place, on the reasoning that renaming a lane
inside a record of what a past run actually classified would falsify the record. The owner
overruled that, and the ruling is the stronger argument: the `HARD` lane means "needs the
apex model", so a meeting-lane item tagged `HARD` asserts something that was never true of
it. Leaving the old name preserves a MISCLASSIFICATION, it does not preserve history. The
rename is therefore a CORRECTION, applied per the canonical mapping in
`relay/scripts/lane-convert.sh` (which cannot do this itself -- it only rewrites checkbox
lines, and a note has none).

**The ruling reaches a tag that ASSERTS an item's lane. It does NOT reach either side of a
narrated transition between two lanes of the OLD vocabulary** (owner's refinement, same
day). "Item X is `[HARD - meeting]`" is a claim about X that is now wrong, so it is
corrected. "X was retagged from A to B" is a claim about an EVENT, and rewriting either
endpoint would make the sentence describe an event that did not occur. The refinement is
forced, not stylistic: the destination of the one retag narrated here is `[HARD - hands]`,
the single lane with NO new-vocabulary form, so that arrow cannot be expressed in the new
vocabulary at all. CONSEQUENCE, stated so a future reader does not read it as a
contradiction: the same item appears as `[INPUT - meeting]` where a run ASSERTS its lane
(Runs 34 and 53) and as `[HARD - meeting]` inside the Run 40 sentence that NARRATES its
retag to `[HARD - hands]`. Both are correct under the rule.

WHAT WAS CONVERTED, all of it measured in the `## From ROADMAP` section:
(a) 4 fully-spelled `[HARD - meeting]` lane ASSERTIONS became `[INPUT - meeting]` (Run 34
twice, Run 53 twice).
(b) 106 SHORTHAND lane annotations in the per-run cross-ledger censuses became their new
names: 32 `[pool]` to `[HARD]`, 42 `[meeting]` to `[INPUT - meeting]`, 32 `[decision-gate]`
to `[INPUT - decision]`. These were missed by the first conversion pass, which touched only
the fully-spelled form and so left the log contradicting itself: Run 40 called two items
`[INPUT - meeting]` and then listed the same items as `[meeting]` two lines later.
(c) 38 census lines had their umbrella category word `HARD` changed to `non-ROUTINE`.
**REVERTED 2026-09-02, owner-directed, after three independent checks.** All 38 are back to
`HARD`. Three reasons, none of them tidiness: `non-ROUTINE` appears in neither
`relay/references/hard-lanes.md` nor `lane-convert.sh`'s mapping, so it was INVENTED
vocabulary; the original justification that "no claim moved" was FALSE, since `HARD` asserts
a lane while `non-ROUTINE` asserts only that a lane is not ROUTINE, which is strictly weaker;
and the criterion that separated these 38 from the 24 bare-id census lines left alone was
visible adjacency to a converted bracket, which is a surface test rather than the ratified
rule. Nine of those 24 carry the same contradiction in prose form ("3 executable HARD --
dba3 decision-gated"), which the surface test cannot see. The local mismatch between an
umbrella `HARD` and an adjacent `[INPUT - *]` tag is now left standing as an honest artifact
of a vocabulary that changed under the log; it also marks where the vocabulary boundary
falls, which is real information.

(d) **REVERTED in the same pass: 11 occurrences that had re-labelled `id:7809` and
`id:98f0`.** Two Run 34 prose sentences returned to `[HARD - meeting]`, and 9 census
shorthand entries returned to `[meeting]`. The owner's rename ruling rests on the premise
that a meeting-lane item tagged `HARD` asserts something never true of it -- and for these
two items that premise is false in the other direction: they are `[HARD - hands]` items
(`ROADMAP.archive.md:533-534`), and this log itself records at Run 40 that the meeting label
was a MISLABEL, later retagged to hands. Converting therefore did not correct a wrong `HARD`;
it restated a known-wrong venue in confident current vocabulary, and landed on `[INPUT -
meeting]`, one of the four destinations the converter refuses to choose between for a hands
item. Since hands has no new-vocabulary form, these passages cannot be expressed in the new
vocabulary at all, which is the same reason the Run 40 arrow stays old-vocab.

Both reverts were verified mechanically: 45 lines changed, NO digit anywhere in the file
altered, and no line differing outside the three intended token classes.

FIVE things remain deliberately unchanged, each declared rather than silently left.
(1) `[HARD - hands]` is NEVER converted: 2 fully-spelled tags and 25 shorthand `[hands]`
annotations stay. `lane-convert.sh` refuses that lane because it fragments across four
destinations (`[MECHANICAL]`, `[INPUT - access]`, `[INPUT - decision]`, `[INPUT - meeting]`)
by per-item human judgment. Converting them here would be a guess.
(2) Two NARRATED transitions keep the vocabulary of their time, per the rule above: the
Run 25 arrow `[HARD - strong model]` to `[HARD - decision gate]`, and the Run 40 arrow
`[HARD - meeting]` to `[HARD - hands]` together with the `[HARD - meeting]` quoted in the
same Run 40 sentence as the label the d5e0 count line was then carrying. A partial
conversion of these four tags was applied earlier the same day and has been REVERTED; it
had produced a mixed-vocabulary arrow describing a transition that never occurred.
(3) Two REFERENTIAL tag mentions, which name a string rather than assert an item's lane:
`id:3c0f` `[HARD - pool]` token sync (Run 45) names the literal token a code change
synchronized, and `[HARD -- pool|meeting|hands]` (Run 30) quotes the multi-lane tag syntax
that `id:78ff` introduced into `gather-human-backlog.sh`.
(4) The census category word `HARD` is left in the 24 census lines of Runs 1 to 33, which
enumerate bare ids and carry no lane tag (e.g. "0 open ROUTINE / 3 executable HARD --
dba3/401c/3346"). The conversion created no contradiction inside them, so nothing here
forced a change -- but under the new vocabulary their category claim is no longer true of
`dba3` or `3346` either. Rewriting them is a prose-taxonomy edit beyond a tag rename, so it
is SURFACED for the owner rather than taken. Left for the same reason: the quotation "3 open
ROADMAP items, all HARD" and the two clauses reporting what a past d5e0 edit was corrected
TO (Run 34 "corrected to 5", Run 40 "corrected to 7 open HARD"), which narrate the content
of an edit instead of asserting a lane.
(5) ONE backticked gawk regex literal (a two-character class quoted from live code, in the
Run 30 CLEAN paragraph) keeps its em dash: the surrounding sentence calls it a
"backwards-range", which is only true of the character actually in the source, so rewriting
it would make the quotation false. (This header said "Run-33" until 2026-09-02; the literal
is in Run 30.)

Outside the tags listed above and the one umbrella word, the `## From ROADMAP` section is
byte-identical to its pre-conversion state at `ef7c7d0f`; that was verified by placeholder
substitution on both sides, not by inspection.
The `## From TODO` section below is PRE-EXISTING and was touched by neither pass; its own
dashes and lane words remain as relocated.

## From TODO

(code review + security + design coherence) — ROADMAP id:401c. Three passes: (1) correctness bugs / shell quoting / race conditions in scripts + Python helpers; (2) injection/path/secrets/permission security; (3) design coherence for unreviewed decisions (sensibility, feasibility, contradictions, gates that can never fire). Every finding fixed inline, tracked, or explicitly accepted. **First run ✓ 2026-06-12** (16 findings — see `docs/meeting-notes/2026-06-12-1811-strong-model-audit.md`). **Latest ✓ Run 73 2026-09-12** (mirror refreshed by Run 73; it had read "Run 70 2026-07-06" across Runs 71 and 72). **Run 70's own entry, kept for the history** (window since Run 69's `8d8d40b`; substantive surface = the 2026-07-06 session batch — lane-convert `--reorder`/roadmap-lint TAG-NOT-FIRST (id:4b37), classify-repo strong_model fable-gate (id:5884), md-merge archive-anchor (id:14d0), relay-state-write toml-set smart-quote (id:abbd), relay-loop integrate `-c` disambiguation + resolveScopeRepo single-repo scope (id:25aa/7633), review.md step-3 tier enumeration (id:f032)). **Pass-1: 1 HIGH FIXED INLINE** — `lane-convert.sh` reorder_rest AND `roadmap-lint.sh` TAG-NOT-FIRST both trimmed leading whitespace with `${var# }` (literal space) under a `[[:space:]]*` loop guard → a TAB after the lane tag / after the checkbox spins FOREVER (reproduced: `timeout` exit 124). Fixed both to `${var#[[:space:]]}` (bracket-expr, matches the guard); added regression case (g) to `tests/test_lane_reorder.sh`. **Pass-2 security CLEAN** — toml-set smart-quote wraps bare strings in `"…"`; an embedded-quote/backslash value would emit invalid TOML but NO caller passes such (values are tags/ISO-dates/model-ids/bools) → accepted. No injection/path/secret holes in the changed scripts; resolveScopeRepo copy byte-identical to pool-args.mjs. **Pass-3: d5e0 count prose FIXED INLINE** (stale "11 open after 2026-07-02" → 8 open after the 2026-07-06 batch; ZERO , b50e CLOSED NO-GO + c79e filed) + 1 FILED (id:5dc3 REVIEW_ME): roadmap-lint's c095 heading-as-item detector false-positives on 3 descriptive `## [MECHANICAL]/[ROUTINE]` handoff-section headers whose children are full id'd `[x]` items (not bare status markers). orphan-scan --cross-ledger 0; relay-doctor report-only (2 parked orphans + inbox dead-letter routed:ab31 SURFACED not auto-filed — targets this repo but describes loderite's npm version-check, ambiguous applicability); suite 186/0/0. See `RELAY_LOG.md` 2026-07-06 Run 70. (Run 69 2026-06-30-1855 window `8d8d40b..HEAD` was Pass-1/2 CLEAN + 2 ledger drifts fixed (id:1bbd lane-anchor twin; d5e0 count 7→3); suite 135/0/0; see `docs/meeting-notes/2026-06-30-1855-strong-model-audit.md`.) (Run 68 2026-06-26-0926 was REAL CODE — window `5e1a216..HEAD`, 1 cross-ledger drift fixed (id:5c00), suite effectively 107 green.) (Run 67 2026-06-23-2145 was LEDGER-ONLY clean, window `a689119..HEAD`.) (Run 66 2026-06-23-1724h was LEDGER-ONLY clean, window `9330f72..HEAD`.) (Run 65 2026-06-23-1724g was LEDGER-ONLY clean, window `69dd4d8..HEAD`.) (Run 64 2026-06-23-1724f was LEDGER-ONLY clean, window `c8127e0..HEAD`.) (Run 63 2026-06-23-2110 was LEDGER-ONLY clean, window `a360ac6..HEAD`.) (Run 62 2026-06-23-2102 was LEDGER-ONLY clean, window `91d639a..HEAD`.) (Run 61 2026-06-23-2054 was LEDGER-ONLY clean, window `9da3a6f..HEAD`.) (Run 60 2026-06-23-2044 was LEDGER-ONLY clean, window `73e4903..daf5694`.) (Run 59 2026-06-23-2040 was LEDGER-ONLY clean, window `c8d4469..HEAD`.) (Run 58 2026-06-23-2037 was LEDGER-ONLY clean, window `9b2abd7..HEAD`.) (Run 57 2026-06-23-2031 was LEDGER-ONLY clean, window `9782379..HEAD`.) (Run 56 2026-06-23-2024 was LEDGER-ONLY clean, window `578c854..HEAD`.) (Run 55 2026-06-23-1724e was LEDGER-ONLY clean, window `2cd8d6e..HEAD`, coherence pass on new design items id:74c7/d23f.) (Run 54 2026-06-23-1909 was LEDGER-ONLY clean, window `e905c84..HEAD`.) (Run 53 2026-06-23-1724d was LEDGER-ONLY clean, window `8052b4f..HEAD`.) (Run 52 2026-06-23-1724c was LEDGER-ONLY clean-by-vacuity, window `9dfce93..HEAD`.) (Run 51 2026-06-23-1724b was LEDGER-ONLY clean-by-vacuity, window `b46be9a..HEAD`.) (Run 50 2026-06-23-1724 was LEDGER-ONLY clean-by-vacuity, window `12b151e..HEAD`.) (Run 49 2026-06-23-1749 was LEDGER-ONLY clean-by-vacuity.) (Run 48 2026-06-23-1730 was REAL CODE — `80a8441..HEAD` Run 47's id:ad74+id:09a3 ship; **1 HIGH inline fix**: the id:ad74 JS INTENSIVE promote backstop was a NO-OP, rewrote to flip idle→execute, suite 89/0/0; see `docs/meeting-notes/2026-06-23-1730-strong-model-audit.md`.) (Run 46 2026-06-23-0945 was LEDGER-ONLY clean-by-vacuity.) (Run 45 2026-06-23-0939 covered the REAL CODE: id:000d/1d64/3c0f/69ef + 1 HIGH inline fix of the dead `is_finished` demote guard — see `docs/meeting-notes/2026-06-23-0939-strong-model-audit.md`.) Recurring: reviewer re-opens the ROADMAP checkbox after each significant Sonnet batch. <!-- id:401c -->
  ...
  - **Run log** (recurring item -- stays open by design):
    ...
    - Run 34 (2026-06-22-0712): first-seen change since Run 33's own audit merge `62b58fa`
      ...
      **Design-coherence pass (substantive this run, unlike the pure-vacuity LEDGER runs):**
      ...
      executable HARD -- 401c [HARD] / 3346 / dba3 [INPUT - decision] / 7809 / 98f0; de4e DEFERRED
    ...
    - Run 35 (2026-06-22-0722): first-seen change since Run 34's own merge `40bc011`
      ...
      **LEDGER-ONLY window** (Runs 11/12/16/17 class). Sole first-seen change = the Run 34
      ...
      Cross-ledger coherent (0 open ROUTINE / 5 executable HARD -- 401c [HARD] / 3346
      [INPUT - meeting] / dba3 [INPUT - decision] / 7809 [meeting] / 98f0 [meeting]; de4e DEFERRED
    ...
    - Run 36 (2026-06-22-0737): first-seen change since Run 35's own merge `69c0bc5`
      ...
      **LEDGER-ONLY window**: `git diff --name-only 69c0bc5..HEAD -- '*.sh' '*.py' '*.js'`
      ...
      to Run 36. Cross-ledger coherent (0 open ROUTINE / 5 executable HARD -- 401c [HARD] /
      3346 [INPUT - meeting] / dba3 [INPUT - decision] / 7809 [meeting] / 98f0 [meeting]; de4e DEFERRED
      non-executable; all five open in both ROADMAP+TODO; d5e0 agrees, no count drift this
    ...
    - Run 37 (2026-06-22-0942): first-seen change since Run 36's own audit merge `b93f024`
      ...
      **SUBSTANTIVE CODE window** (breaks the Run 31/32/33/35/36 ledger-only streak): 403
      ...
      ROUTINE / 5 executable HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / 7809
      [INPUT - meeting] / 98f0 [meeting]; de4e DEFERRED non-executable; all five open in both
    ...
    - Run 38 (2026-06-22-0953): first-seen change since Run 37's own audit merge `8258aa3`
      ...
      **clean: LEDGER-ONLY window** (Run 11/12/16/17/31/32/33/35/36 class). Sole first-seen
      ...
      coherent (0 open ROUTINE / 5 executable HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3
      [INPUT - decision] / 7809 [meeting] / 98f0 [meeting]; de4e DEFERRED non-executable; all five
    ...
    - Run 39 (2026-06-22-1004): first-seen change since Run 38's own audit merge `0174a69`
      ...
      **1 coherence drift fixed inline (recurring Run 4/8/17/35/36/37/38 mirror class)** -- the
      ...
      Cross-ledger coherent (0 open ROUTINE / 5 executable HARD -- 401c [HARD] / 3346 [INPUT - meeting]
      / dba3 [INPUT - decision] / 7809 [meeting] / 98f0 [meeting]; de4e DEFERRED non-executable;
    ...
    - Run 40 (2026-06-22-1601): first **CODE** window since Run 39's LEDGER-ONLY runs --
      ...
      (0 open ROUTINE / 7 open HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] /
    ...
    - Run 41 (2026-06-22-1757): first-seen change since Run 40's own audit merge `f3c26f8`
      ...
      coherent (0 open ROUTINE / 7 open executable-or-gated HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3
      [INPUT - decision] / e149/7809/98f0/0994 [hands]; de4e DEFERRED non-executable; all open in both
    ...
    - Run 42 (2026-06-22-1818): first-seen change since Run 41's own audit merge `b65ba59`
      ...
      Cross-ledger coherent (0 open ROUTINE / 7 open executable-or-gated HARD -- 401c [HARD] /
      3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149/7809/98f0/0994 [hands]; de4e DEFERRED
    ...
    - Run 44 (2026-06-23-0701): first-seen code since Run 43's own audit commit `c66c6f4`
      ...
      **CLEAN -- no inline code/security fix.** Pass-1: bae5's lock booleans diff the SAME
      ...
      executable-or-gated HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] /
      e149/7809/98f0/0994 [hands]; de4e DEFERRED non-executable; all open in both ROADMAP+TODO).
    ...
    - Run 45 (2026-06-23-0939): first-seen code since Run 44's own audit commit `0e60f1f`
      ...
      **Mirror drift fixed inline (Run 4/8/17/… class)** -- TODO id:401c MIRROR line read "Latest
      ...
      closed this window; open executable-or-gated HARD: 09a3 [HARD] / 3346 [INPUT - meeting] / dba3
      [INPUT - decision] / e149/7809/98f0/0994 [hands]; de4e DEFERRED non-executable; the d5e0 prose
    ...
    - Run 50 (2026-06-23-1724): first-seen change since Run 49's own audit merge `12b151e`
      ...
      [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149 / 7809 / 98f0 / 0994 [hands]; de4e
    ...
    - Run 51 (2026-06-23-1724b): first-seen change since Run 50's own audit merge `b46be9a`
      ...
      401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149 / 7809 / 98f0 / 0994 [hands]; de4e
      DEFERRED non-executable; all open in both ROADMAP+TODO; d5e0 agrees). orphan-scan
    ...
    - Run 53 (2026-06-23-1724d): first-seen change since Run 52's own audit merge `8052b4f`
      ...
      TODO discussion item, id:9000 (`[INPUT - meeting]` inter-session coordination channel, +1);
      ...
      `[INPUT - meeting]` lane correct for a discussion placeholder; every cross-reference
      ...
      coherent (0 open ROUTINE / 7 open executable-or-gated HARD -- 401c [HARD] / 3346 [INPUT - meeting]
      / dba3 [INPUT - decision] / e149 / 7809 / 98f0 / 0994 [hands]; de4e DEFERRED non-executable;
    ...
    - Run 55 (2026-06-23-1724e): window = since Run 54's audit merge `2cd8d6e`
      ...
      Cross-ledger coherent (0 open ROUTINE / 7 open executable-or-gated HARD -- 401c [HARD]
      / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149 / 7809 / 98f0 / 0994 [hands]; de4e
    ...
    - Run 56 (2026-06-23-2024): window = since Run 55's audit merge `578c854`
      ...
      ROUTINE / 7 open executable-or-gated HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3
      [INPUT - decision] / e149 / 7809 / 98f0 / 0994 [hands]; de4e DEFERRED non-executable;
    ...
    - Run 65 (2026-06-23-1724g): window = since Run 64's audit merge `69dd4d8`
      ...
      HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149 / 7809 / 98f0 / 0994
    ...
    - Run 67 (2026-06-23-2145): window = since Run 66's audit commit `a689119`
      ...
      executable-or-gated HARD -- 401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision] / e149 / 7809 /
    ...
    - Run 69 (2026-06-30-1855): window = `8d8d40b..HEAD` (HEAD `7527cb1`), since Run 68 --
      ...
      **Pass-1 code review CLEAN** + **Pass-2 security CLEAN** (no correctness/injection defects --
      ...
      re-derived to 3 open executable-or-gated (401c [HARD] / 3346 [INPUT - meeting] / dba3 [INPUT - decision];
      de4e [INPUT - meeting] DEFERRED). orphan-scan --cross-ledger now 0; roadmap-lint 0; gaming-scan
    ...
    - Run 73 (2026-09-12-2142): the strict window since Run 72's HEAD (`1b7e9bb..HEAD`) is
      **4,940 commits / 649 code files / 81,429 insertions** (~28 kLOC outside `tests/` and
      `tracker/fixtures/`). A 3-pass adversarial audit of that is NOT soundly performable in
      one turn, so this run took the **Run 70 precedent explicitly**: declare the window,
      audit a bounded surface, name the residue. Surface audited = every PRODUCTION file
      ADDED since 2026-09-10, i.e. never audited before (~1,570 lines): `decision-brief/
      docket.sh` (737), `relay/scripts/review-box-tick.py` (500), `expires-on-scan.sh` (197),
      `lib-clean-tree.sh` (138). **Pass-1: 1 MEDIUM FIXED INLINE** -- `docket.sh
      draft-answer` built its proposal with `${existing%%<!-- id:$item_id -->}`, which strips
      a SUFFIX, so on a line whose anchored marker is not line-final the strip silently
      no-ops and the append emits a SECOND `<!-- id:XXXX -->`; md-merge.py and
      lib-typed-edges.sh both REFUSE a multi-marker line (id:6059), leaving the item
      unaddressable by every anchored-id writer. Silent ledger damage from the one write path
      in a skill contracted never to write unconfirmed. The pre-existing `marker_count != 1`
      guard counts markers, not their POSITION, so it passes. Reachable: 14 open TODO.md + 22
      open ROADMAP.md items are in that shape here, `id:32c3` among them AND an `[INPUT -
      decision]` item, i.e. exactly what draft-answer is pointed at; reproduced with a
      line-final control that came out at 1 marker against the defect case's 2. Fixed by
      REFUSING (new exit 7) rather than guessing an insertion point -- the id:6d7e reasoning
      `review-box-tick.py` already applies. RED spec first
      (`tests/test_decision_brief_draft_answer_marker_final.sh`), negative case VERIFIED by
      `make verify-negatives` (green-now OK, red-there OK, matched the LAST of 3 FAIL lines).
      **Pass-2 security CLEAN** across all four files (json.dumps for the md-merge payload,
      option-safe greps, list-argv subprocess, collector stderr captured not swallowed).
      **Pass-3: 2 TRACKED, 2 ACCEPTED** -- TRACKED: `roadmap-tick.sh` has NO
      `relay:recurring-audit` carve-out although `gather-repo-state.sh` and
      `backtest-historical.py` both gate on it, so under v12/id:5b12 the driver ticks this
      very item and **401c needs RE-OPENING after this run**; and `review-box-tick.py`'s
      docstring claim that its heading regex is identical to md-merge's is FALSE for a
      titleless `###` (md-merge tests its opener rstripped at `:849` but its section END on
      the raw line at `:854`, so a bare `###` closes a section it cannot open) -- not live
      today (zero such headings, measured), so forward-robustness, and the fix is a
      shared-writer contract decision rather than a local patch. ACCEPTED: a doubled coverage
      count on a whitespace-only collector stderr; `--answer` mandatory but not written
      (matches the hard-lanes.md marker contract). **RESIDUE, stated so this line is not read
      as coverage: everything in `1b7e9bb..HEAD` outside those four files is UNAUDITED**,
      including ~3.3 kLOC of MODIFIED production code from the same two days
      (`relay-loop.js` +333, `todo-conformance.sh` +292, `meeting/append.sh` +187,
      `worktree-retire.sh` +167); windows before Run 70 stay uncertified; `id:da95`
      untouched. Suite 656/0/0 + 1 expected-red. See
      `docs/meeting-notes/2026-09-12-2142-strong-model-audit.md`.

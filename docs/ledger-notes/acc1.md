...
SKILL step-4/5 reminder or a self-check). <!-- id:acc1 -->

## Original title (verbatim, before the `id:64f9` rewrite)

/meeting must HARD-enforce the same-message transcript+AskUserQuestion rule at decision points — model ended a decision point on bare prose this session, user had to prompt 'why not continue with AskUserQuestion now'. format.md §Interactive mode already states the rule ('never end a turn on bare prose and send the question in a subsequent turn'); strengthen enforcement (e.g.

...
## Amendment 2026-07-02 (rename mapping — `[HARD — hands]` has no auto-default)
Correcting Decision 1's mapping (owner, during wave-2b handoff): `[HARD — hands]→[INPUT — access] OR [MECHANICAL]` was **too coarse**. `[HARD — hands]` (hardware/sudo/secret/on-device/rehearsal) fragments across **FOUR** destinations by per-item judgment — `[MECHANICAL]` (machine runs it) / `[INPUT — access]` (credential/physical) / `[INPUT — decision]` (human decides, e.g. it-infra fd30 post-gate decisions) / `[INPUT — meeting]` (design judgment, e.g. a rehearsal needing interpretation). Defaulting to `[INPUT — access]` would silently mislabel every judgment-bearing hands item. So the B1 rename converter (id:4f02) auto-converts ONLY the three unambiguous 1:1 renames (`pool→HARD`, `meeting→INPUT-meeting`, `decision gate→INPUT-decision`) and **FLAGS every `[HARD — hands]` item for judgment, converting none**. hard-lanes.md's north-star table records the four-way fan-out.

## Amendment 2026-07-02 (human lanes closed to `{meeting, decision, access, unspecific}` — NO `[INPUT — run]`; automation-first)
Owner decision (resolving the "human runs/observes a procedure" gap that 77f3/b8ae exposed): do **NOT** mint an `[INPUT — run]` lane. A recurring "a human runs/observes X and reports" task is almost always an **un-written test wearing a human costume**, and a comfortable lane for it would legitimize the chore instead of pressuring automation. Rule:
1. **Automation-first triage** — for any `[HARD — hands]` (or would-be run/observe) item, the FIRST question is "can this be BDD-automated?" If yes → it is `[ROUTINE]`/`[MECHANICAL]` **+ a test** (or an `@manual` Gherkin scenario as the explicit placeholder for the test-not-yet-written — the relay's existing handoff-C4 / REVIEW_ME machinery). Reaching for a "run" lane must fire the question *why isn't this a test?*
2. **Human capability lanes are closed to `{meeting, decision, access}`** — irreducible human *inputs* (design judgment / a pick / a credential-or-physical action).
3. **`[INPUT — unspecific]`** is the catch-all ONLY for a genuinely-undecidable residual — never a default, used after the automation-first triage and the four-way judgment both fail to place it.
Applied: 77f3 (post-Fable *checks*) + b8ae (*observe* the re-enqueue log line — already partially covered by `test_relay_loop_structure.sh` 13–14) are **automation candidates**, not human-lane residents. This sharpens M3 (id:3ef7): its per-`[HARD — hands]` judgment leads with "BDD-automate?" before any `[INPUT]`/`[MECHANICAL]` placement, and only falls to `[INPUT — unspecific]` when truly undecidable.

...
- **D3 — detection rule.** Pre-registered acceptance band (pass-rate floor + latency/throughput band), flag only on **2 consecutive** out-of-band runs. tokens/sec is the weak silent-swap hedge. *Out of scope:* auto-rollback. **PRE-REGISTERED 2026-06-17** (`docs/meeting-notes/2026-06-17-0905-model-probe-tos-and-band.md` D2): per tier `(model, battery_version)`; seed = first 5 cold runs/tier; **pass-rate gate: ≥2 items below baseline pass-count AND 2 consecutive runs** (c-chart, limit = baseline+2 defects); **throughput advisory: median ± 30%** (tighten to ±3·MAD after ≥20 runs); **latency: never gates**; battery edit resets baseline; no human override of the rule.
...
- [x] **Blocking pre-check:** confirm the probe's invocation path against current Anthropic usage policy / Claude Code terms. **RESOLVED 2026-06-17** (`docs/meeting-notes/2026-06-17-0905-model-probe-tos-and-band.md`): both paths in-bounds; **chosen = path A (`claude -p`) run as a dedicated OS user with an empty/minimal `~/.claude`** (harness-free by isolation, subscription serving path, no per-token cost). Path B (API key + `--bare`) retained as latent fallback behind the same swappable function. Verdict scoped to subscription path. <!-- id:2d01 -->
...
- [ ] **Seed + close:** run 5 cold probe runs/tier under the pre-registered band formula (D3 above; committed 2026-06-17-0905 note); fill per-tier baselines; then close id:dba3 with the investigation finding written into this note. Gated on id:c345 + id:040a landing first. <!-- id:23e9 -->

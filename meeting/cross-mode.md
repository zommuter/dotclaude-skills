# Cross-mode — `/meeting --cross`

Loaded by canonical `/meeting` only when the skill argument is `--cross`. Everything in canonical setup (steps 1–7) has already run: metadata is captured, format spec / personas / profile are loaded, broker is probed. This file owns the full cross flow; canonical SKILL.md is not read further after loading this.

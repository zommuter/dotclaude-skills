...
5. **Dispatch by class:**
   - Class 1 → proceed to implementation in normal mode (no plan mode, no meeting). **(D7 — relay-managed repos, `<root>/ROADMAP.md` exists):** the inline implementation follows the `fables-executor` contract — "C1s work as if picked by `/fables-executor`". Concretely (contract rules 2–4): definition of done = the item's previously-failing tests pass + a refactor pass + the FULL suite (`make test` or equivalent) is green; **never weaken, skip, delete, or rewrite a test to make it pass**; append a one-paragraph self-report to `<root>/RELAY_LOG.md` and commit it. On `/opusplan` the post-`ExitPlanMode` tier IS Sonnet (the executor tier), so treat C1 work exactly as a `/fables-executor` session would. (Unlike a relay child, a direct `/meeting` session still pushes + runs git-diary-workflow at end — the `fable-ckpt-*` tag remains the next `/fables-turn review`'s job, not this session's.) In non-relay repos (no ROADMAP.md), proceed in normal mode as before.

...
1b. **Arg-guard (id:7681)**: before treating any leading-dash token in the invocation as
   ...
   **Capture the mode flags** the cleaned invocation carries: `--cross` selects cross-project
   mode (step 1 / `cross-mode.md`); `--fabled` sets the session to run the opt-in closing
   Fable pass at end-of-meeting (id:7e87 — End-of-meeting step 0f). Neither is part of the
   meeting subject.
...
2-setup-claim. **Advisory claim at setup (relay-managed repos only, `<root>/ROADMAP.md` exists).** For relay-managed repos, `/meeting` acquires ONE advisory repo claim at SETUP — early in setup, immediately after root resolution and metadata capture — so the relay pool (which already honors claims via `claim.sh peek`) skips the repo for the meeting's duration:
   - **Acquire**: `~/.claude/skills/relay/scripts/claim.sh acquire "meeting:<root-basename>" --repo <root-basename> --run meeting-<captured session id> --mode meeting`. **Distinct key (id:0ee1, branch b, 2026-07-20)** — the advisory claim lives on `meeting:<root-basename>`, NOT on the bare `<root-basename>` hard-lease key, so a parallel executor's `claim.sh acquire <root-basename> --mode execute` is never refused by a live meeting (the meeting is ledger-only per the SCOPE INVARIANT). `--repo <root-basename>` is mandatory — without it the claim is invisible to `relay-loop.js`'s repo-field matchers.
   ...
   - **Released at the END of the meeting on ALL exit paths**: `~/.claude/skills/relay/scripts/claim.sh release "meeting:<root-basename>" --run meeting-<captured session id>`. The claim's mtime+TTL is the backstop if a release path is missed.
...
7c. **Same-turn protocol guard (ENFORCED — `routed:29bc`).** From here until the meeting note is written, a `Stop` hook (`~/.claude/hooks/meeting-question-guard.py`) **BLOCKS** any turn that ends on ≥800 characters of prose without an `AskUserQuestion` call in that same turn. It arms itself from the transcript — there is nothing to write at setup. **NOBODY is exempt (changed 2026-09-01, owner-decided, provisional).** Fable-class used to be exempt automatically, inferred from the session model — but the class was never the meeting PHASE, so a `--fabled` pass on an Opus facilitator inferred `default` and blocked during the one step `format.md` §Interactive mode says must NOT call `AskUserQuestion`. The exemption is now commented out in the hook (restorable verbatim), so `--class fable` still writes a marker but no longer exempts. **Consequence to watch:** a genuine Fable-class meeting now blocks while `format.md` still tells it to answer in inline prose. If that stalls, either restore the exemption or retire that `format.md` rule — one or the other, not both. If a turn genuinely has no decision point and the block fires: `bash ~/.claude/hooks/meeting-guard-marker.sh disable`.

## With a subject argument

1. **Warrantability self-check** (see format spec). If the request clearly passes the "When to call a meeting" criteria, say so in one line and proceed — no prompt. If it looks like a bug fix, one-liner, or already-decided feature, **do not end the turn on prose**: state the concern and pose it through the harness-class protocol in the SAME message — `AskUserQuestion` on Sonnet/Opus/Haiku (`[hold the meeting anyway / just do it directly / something else]`, with the reason it might be overkill in the tl;dr), inline-prose numbered options on Fable-class. Rationale (id:2419): this was the ONE legitimate prose yield between `/meeting` and the meeting-note write, and therefore the only accepted false positive of the `meeting-question-guard` Stop hook. Closing it means every decision point in a meeting window is a question, which is what the guard already assumes.
...
3. **Plan-mode gate (`format.md` §Plan-mode gate — check your model identity):** on **Sonnet/Haiku**, call `EnterPlanMode` and accumulate the transcript in the plan file the system creates; on **Opus/Fable**, SKIP `EnterPlanMode` and accumulate the transcript in visible chat (§Interactive mode). The meeting note is the durable record either way, and the per-decision `AskUserQuestion` points + the closure gate still apply.
...
2. **Pre-classify** with `classify.sh` — run:
   ...
   - **Skip**: items that are purely date-triggered (activation date in text); items explicitly deferred/reopen-gated with unmet conditions; `RELAY` lines (the ROADMAP.md mirror — executor work, never meeting-worthy); **`POOL` lines** (bare `[HARD]`, old `[HARD — pool]` — relay-executor work; nudge `/relay` instead), **`HANDS` lines** (`[INPUT — access]`/`[INPUT — author]`, old `[HARD — hands]` — human-manual work), **`HUMAN` lines** (`[INPUT — decision]` — a human call with NO design session, id:1f1c; route to `/relay human` instead), **`EXEC` lines** (`[ROUTINE]` — executor-tier work; nudge `/relay` instead. Skipped **always**, id:4e3b — including an item with a link + `## Decisions`: `[ROUTINE]` means a cheap executor suffices, so the pool is its home, and granting it the C1 inline-implementation path would give the cheaper lane MORE meeting access than bare `[HARD]`, which is already skipped when impl-ready), and **`MECH` lines** (`[MECHANICAL]` — daemon-run, no LLM). POOL/EXEC/MECH/HANDS/HUMAN are NOT meeting candidates — surface them under a separate "not meeting-worthy" note, never in the C1/C2/C3 buckets (this is the fix for the /meeting over-claim where a settled, pool-executable `[HARD — pool]` item was recommended for a redundant design meeting, 2026-07-14). The full lane→class mapping (both vocabularies) is documented in `classify.sh`'s header and contracted in `relay/references/hard-lanes.md`.
3. **Print the classified bucket summary** as visible text (group by class, show each item one-liner). If an item has `GATED` in the 5th TSV field, append `[GATED]` to its one-liner so the gate condition is visually obvious; if it has `HARD-NOLANE`, append `[needs a lane]` (id:78ff — the item carries a `[HARD …]`/`[INPUT — …]` bracket whose lane is not in `hard-lanes.md`, and must be re-tagged before it can be routed; note bare `[HARD]` is a VALID lane — the pool lane — and is never flagged). Print `POOL`/`EXEC`/`MECH`/`HANDS`/`HUMAN` items under a separate **"↪ not meeting-worthy (apex-pool / executor / daemon / human-manual / human-decision)"** note, NOT in the pick-eligible buckets. **`MECH` must always be printed, never silently omitted** (id:4e3b): a `[MECHANICAL]` item is pool-inert AND human-inert (`gather-human-backlog.sh` keeps it out of every human bucket) and its host daemon is not built, so this note is currently the ONLY surface on which a human ever sees one — dropping it would strand it invisibly (the id:4347 no-silent-swallow anti-pattern). Pick `head -1` of the highest-class non-empty **meeting bucket** (priority: 1 > 2 > 3); POOL/EXEC/MECH/HANDS/HUMAN/RELAY are never picked. Show one-line rationale.

   **Relay-ready nudge (D4 — relay-managed repos only):** if `<root>/ROADMAP.md` exists AND has at least one open (`- [ ]`) `[ROUTINE]` item (`grep -qE '^- \[ \].*\[ROUTINE\]' <root>/ROADMAP.md`), print ONE surface-only line above the bucket summary: `↪ <root> has N open [ROUTINE] item(s) — run \`/relay review <root-basename>\` to verify executor work and re-dispatch.` Name the repo explicitly (bare `/relay review` defaults to `--all`). This is a **surface-only nudge** — never invoke `/relay` inline: the meeting's execution tier is not guaranteed to be the strong reviewer tier (under `/opusplan` the post-`ExitPlanMode` tier is Sonnet, the executor tier; on Opus/Fable the plan-mode switch is skipped entirely per the `format.md` §Plan-mode gate), so strong-model relay work must run in its own strong session. Skip silently when no ROADMAP.md (non-relay repo) or no open `[ROUTINE]` items. *(Deferred under id:5ab4's friction gate: inline dispatch, Fable-vs-Sonnet auto-detection, window-check fold-in, strong-Agent-spawn.)* Note: `classify.sh` routes `[HARD]` items **by lane** — `[HARD — meeting]` → C3 (design work, never C1/C2), while `[HARD — pool]` → `POOL` and `[HARD — hands]` → `HANDS` are skipped from the meeting buckets entirely (executor / human-manual). A bare `[HARD]` with no lane classifies C3 but is flagged `HARD-NOLANE` (id:78ff loud reject).
4. Ask via AskUserQuestion: `[do this / pick something else]` — no "not yet" option.
5. **Dispatch by class:**
   - Class 1 → proceed to implementation in normal mode (no plan mode, no meeting). **(D7 — relay-managed repos, `<root>/ROADMAP.md` exists):** the inline implementation follows the `/relay executor` contract — "C1s work as if picked by `/relay executor`". Concretely (contract rules 2–4): definition of done = the item's previously-failing tests pass + a refactor pass + the FULL suite (`make test` or equivalent) is green; **never weaken, skip, delete, or rewrite a test to make it pass**; append a one-paragraph self-report to `<root>/RELAY_LOG.md` and commit it. The executing tier varies by harness — Sonnet under `/opusplan` (post-`ExitPlanMode`), Opus in a direct Opus session, Fable under Fable — so treat C1 work exactly as a `/relay executor` session would **regardless of tier** (C1 uses no plan mode either way). (Unlike a relay child, a direct `/meeting` session still pushes + runs git-diary-workflow at end — the `fable-ckpt-*` tag remains the next `/relay review`'s job, not this session's.) In non-relay repos (no ROADMAP.md), proceed in normal mode as before.
   - Class 2 → **plan-mode gate (`format.md` §Plan-mode gate):** on **Sonnet/Haiku**, call `EnterPlanMode` and use Claude Code's native explore → design → present → ExitPlanMode workflow; on **Opus/Fable**, run the same explore → design → present → implement flow in **normal mode** (no `EnterPlanMode`/`ExitPlanMode`) — present the plan as visible chat and run the **closure gate** (below) for approval before implementing, since the `ExitPlanMode` approval that normally gates C2 is skipped here. No persona scaffolding either way. After the plan is approved and implemented: write a **Class 2 planning record** to `<root>/docs/meeting-notes/YYYY-MM-DD-HHMM-<slug>.md` using the Class 2 template defined in `format.md` (distinct from Class 3: `**Mode:**` field, no `## Discussion`, uses `## Context / ## Plan / ## Implementation findings / ## Decisions / ## Action items`). Content synthesises the plan file; plan file is left to Claude Code's auto-cleanup. No allowlist changes needed (Read+Write paths already covered).
...
0. **Closure gate (id:af5a — run this FIRST, before step 1 and ANY durable write).** Do NOT unilaterally decide the agenda is exhausted — the user may have an amendment, a late objection, or a topic the agenda missed, and once the note is written and the ledgers committed, raising it means re-opening a recorded decision rather than shaping it. Ask whether the meeting can be closed:
   - **Sonnet/Opus/Haiku:** `AskUserQuestion` — `[wrap up and record / amend a decision / add an agenda item]`.
   - **Fable-class:** inline-prose numbered options (`format.md` §Interactive mode) — never `AskUserQuestion`.
   On **"amend a decision"** → re-open it as an **Amendment session** in the transcript (§Constraints); **APPEND a superseding entry that cites what it supersedes — never silently rewrite the ratified original** (decision-provenance). Then re-run this gate. On **"add an agenda item"** → conduct it as a new agenda item (full discussion → decision), then re-run this gate. On **"wrap up"** → **if `--fabled` was passed, run the `--fabled` closing pass (step 0f) FIRST** (its findings may re-open a decision via this same gate); otherwise proceed to step 1. (This is also the sole closure checkpoint for a Class-2 dispatch whose `ExitPlanMode` approval was skipped on Opus/Fable per the plan-mode gate.)
0f. **`--fabled` closing Fable pass (id:7e87 — opt-in; unifies id:8df5; design D3 `docs/meeting-notes/2026-07-20-2304-fabled-meeting-flow-and-unknown-switch-guard.md`).** Run this **only** when the invocation passed `--fabled` (step 1b) — it is **opt-in, never default**. It is a single closing adversarial pass whose findings are **advisory only, never a gate** ([[feedback-fable-optional-not-gate]] — Opus decisions are final; this is a free second opinion).
   1. **Availability** — reuse the tested config reader, do NOT re-derive it: run `~/.claude/skills/relay/scripts/fable-config.sh check`. `available` (exit 0, the Max default unless relay.toml declares `[relay] fable_available = false`) or an explicit user "Fable is available" override → Fable is available. `unavailable` (exit 1) → skip straight to step 2 (LOUD degrade) — there is nothing left to probe (Fable is a fixed part of the Max subscription; retired 2026-07-28, id:aa26).
   2. **LOUD degrade** — if Fable is unavailable, do **NOT** silently skip (a silent degrade literally re-creates id:7681): surface the exact line **Fable unavailable — `--fabled` pass skipped** to the user AND record that exact line in the meeting note (a `> **--fabled:**` blockquote carrying it, near the Decisions section), then proceed to step 1. **Never silent.**
   3. **Digest AT CLOSING TIME** — if available, build a shared repo-state **digest** *now*, at closing time, so it carries the final state: the captured metadata, the orphan/classify scan outputs already surfaced this session, and — critically — **the ratified decisions VERBATIM** (copy the `## Decisions` text as ratified, including each `**Decision provenance:**` line). The digest is the subagent's entire context; it is built at closing time precisely so late amendments are included.
   4. **ONE Fable-5 subagent (subagent-NOT-driver)** — spawn exactly **one** closing subagent via the Task tool pinned to `model: claude-fable-5`, fed the digest, with a **design-critique** framing ("You are 🜛 Fable-5, a closing adversarial reviewer. Critique the *design* of these ratified decisions — find holes, unstated assumptions, and failure modes"). The design-critique framing sidesteps the `reasoning_extraction` refusal (id:aa68/id:a4f5); Fable is a **subagent, never the meeting driver**. It returns findings as prose; it decides nothing.
   5. **Advisory handling — findings VERBATIM before any decision prompt (id:8c6f, `routed:4d2b`).** Emit the closing subagent's findings as complete, verbatim prose, as visible chat content, BEFORE presenting any decision prompt — the same verbatim-before-prompt obligation step 5 of `## Session flow` already carries for `AskUserQuestion` (`format.md` §Interactive mode, mirrored above at this file's step 5: *"output the complete, verbatim discussion text ... as visible chat content, then immediately call `AskUserQuestion` in the same response"*). **Summarising the findings, or renumbering them to `F1..Fn` in place of the prose, does NOT satisfy this** — the user must see the subagent's own words, not your paraphrase of them. Only after the verbatim findings are visible: present them to the user as **advisory**. For any finding the owner accepts, re-open the affected decision through the closure-gate **"amend a decision"** path above (append a superseding entry) — the owner ratifies, Fable does not. Findings the owner declines are noted and dropped. Record the pass (verdict + findings) in the meeting note.
   6. **Pre-registered escalation trigger (gates per-decision (B) + full multi-pass (C, id:8df5)).** Count **only** closing-pass findings that **force reopening/amending an already-ratified decision** this session. **Hardening-only findings do NOT count** (the metric must discriminate). If a session reaches **≥2** such forced-amendment findings, that is the pre-registered evidence to build the per-decision pass (fire Fable before each `AskUserQuestion`) and the full multi-pass (id:8df5) — surface the count so the trigger is auditable; until then v1 stays closing-only. Then re-run the closure gate (step 0).
1. Call `ExitPlanMode` **only if you entered plan mode** (per the `format.md` §Plan-mode gate — skipped on Opus/Fable, in which case there is nothing to exit).
...
- No file edits except the plan file (when plan mode is active) and the final meeting note.
- No implementation work mid-meeting even if asked — defer until the meeting's decisions are settled (after `ExitPlanMode` when plan mode is active; after the closure gate otherwise). **Both constraints hold whether or not plan mode is active** — on Opus/Fable there is no read-only guard enforcing them, so honor them as prose (exactly as every other non-plan-mode part of the skill already does).

...
2a. **Relay-pool claim hold (id:d748 — relay-managed repos only, `<root>/ROADMAP.md` exists).** Steps **2b** (TODO.md) and **2e** (REVIEW_ME/ROADMAP/TODO) write the SHARED LEDGERS a live relay pool also merges into. The meeting NOTE (step 2) and the whole read/think phase are NEVER blocked — only this shared-ledger WRITE-BACK is gated. Before the 2b/2e writes, take the repo claim so you don't collide with a running pool:
